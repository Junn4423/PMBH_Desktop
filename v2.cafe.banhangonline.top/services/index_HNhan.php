...
switch ($vtable) {
	...
	// lay loai san pham
	case 'Mb_loaiSanPham':
		include_once("../cafe/clsall/sl_lv0006.php");
	...
	case 'Mb_sanPham':
		include_once("../cafe/clsall/sl_lv0007.php");
	...
	case 'Mb_Cthd':
		...
		break; // Add missing break for Mb_Cthd case
	case 'Mb_thanhtoan':
		include_once("../cafe/clsall/sl_lv0014.php");
		$sl_lv0014 = new sl_lv0014($_SESSION['ERPSOFV2RRight'], $_SESSION['ERPSOFV2RUserID'], 'Jo0100');
		...
		switch ($vfun) {
			case 'thanhToan_contract':
				$result = $sl_lv0014->thanhToan('', $mahd, '', '', '', $_SESSION['ERPSOFV2RUserID'] ?? 'admin');
				$vOutput = ['success' => ($result ? true : false), 'message' => ($result ? 'Thanh toán thành công' : 'Thanh toán thất bại')];
				break;
			default:
				break;
		break;
