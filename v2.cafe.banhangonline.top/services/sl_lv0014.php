...
";
	...
	{
		echo '123';
		require_once("../server/clsall/wh_lv0034.php");
		...
		$vsql="update sl_lv0014 set lv019='1',lv030=IF(lv030<1,1,(lv030+1)) where lv001 in ('$cthdId') and (select A.lv011 from sl_lv0013 A where A.lv001=sl_lv0014.lv002)<=0";
		echo $vsql;
		$vresult=db_query($vsql);
		$vsql2 = " select A.sl001 from sl_lv0013 A join sl_lv0014 B on A.lv001 = B.lv002 where B.lv001 = '$cthdId' limit 1";
		...
		if($lvwh_lv0010->LV_CheckDataBEPBAR($cthdId,$cthdId))
		{
	...
	function thanhToan($cusid, $donhangid, $txtlv004, $txtlv008, $txtlv010, $userID)
	{
		...
		if(trim($cusid)=='' || $cusid=='Mã KH'){
			$vsql="update sl_lv0013 set lv002='',lv003='',lv011=2,lv005=concat(curdate(),' ',curtime()),lv032='".$userID."' where lv001='".$donhangid."' and (lv011=0 or lv011=1)";
			$vresult=db_query($vsql);
			// echo "1+ '$vsql'";
			// echo "1+ '$vsql'";
		}
		else
		{
			...
			$vsql="update sl_lv0013 set  lv011=2,lv005=concat(curdate(),' ',curtime()),lv032='".$userID."' where lv001='".$donhangid."' and (lv011=0 or lv011=1)";
			$vresult=db_query($vsql);
			// echo "2+ '$vsql'";
		}
		$lvwh_lv0034=new wh_lv0034($_SESSION['ERPSOFV2RRight'],$_SESSION['ERPSOFV2RUserID'],'wh0034');
	...
	function xoaCthd($maCt)
	{
		...
		if (empty($maCt)) {
			return [
				'success' => false,
				'message' => 'Mã chi tiết đơn hàng trống!'
			];
		}

		// Nên escape chuỗi cho an toàn
		$maCt = addslashes(string: $maCt); // Hoặc dùng prepared statement thì tốt hơn

		$sql = "DELETE FROM sl_lv0014 WHERE lv001 = '$maCt'";
		$result = db_query($sql);

		if (!$result) {
			$success = false;
			$errorMessage = "Lỗi xóa đơn hàng: " . db_error(); // nếu db_error() có tồn tại
		}
	...
	function taoCtHd($maHD, $maSp, $soLuong)
	{
