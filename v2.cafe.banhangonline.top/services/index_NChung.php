...
case 'DonBan':
    ...
    case 'BaoCaoBanHang':
        include_once("../cafe/clsall/sl_lv0214.php");
    ...
    case 'Mb_Users':
        include_once("../cafe/clsall/lv_lv0007.php");
    ...
    case 'SystemLogs':
        ...
        try {
            ...
            $parseDate = function ($value, $fallback = null) {
                ...
                $timestamp = strtotime($stringValue);
                if ($timestamp === false) return $fallback;
                return date('Y-m-d H:i:s', $timestamp);
            };
            ...
            $conditions = [];
            if ($fromDate !== null) $conditions[] = "lv003 >= '" . esc_str($fromDate) . "'";
            if ($toDate !== null) $conditions[] = "lv003 <= '" . esc_str($toDate) . "'";
            if ($userFilter !== '') $conditions[] = "lv002 = '" . esc_str($userFilter) . "'";
            if ($actionFilter !== '') $conditions[] = "lv004 = '" . esc_str($actionFilter) . "'";
            if ($keyword !== '') {
                $kw = esc_str('%' . $keyword . '%');
            ...
            $totalRows = 0;
            if ($countRes && $countRow = db_fetch_array($countRes, MYSQLI_ASSOC)) {
    ...
    case 'CancelLogs':
        ...
        try {
            ...
            if ($toDate !== null) $conditions[] = "lv003 <= '" . esc_str($toDate) . "'";
            if ($userFilter !== '') $conditions[] = "lv002 = '" . esc_str($userFilter) . "'";
            if ($reasonCode !== '') $conditions[] = "lv005 LIKE '%\"cancelReasonCode\":\"" . esc_str($reasonCode) . "\"%'";
            if ($keyword !== '') {
                $kw = esc_str('%' . $keyword . '%');
