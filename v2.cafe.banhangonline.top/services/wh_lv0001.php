<?php
/////////////coding wh_lv0001///////////////
class   wh_lv0001 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
////////////
	...
	var $ArrView=array("lv001"=>"0","lv002"=>"0","lv003"=>"0","lv004"=>"0","lv005"=>"0","lv006"=>"0","lv007"=>"0","lv008"=>"0");	
	...
	{
		$lvsql="select * from  wh_lv0001 Where lv001='$vlv001'";
	...
	function LV_AprAll()
	{
		...
		{
			$wh=$vrow['lv001'];
			if($wh!="") $this->LV_ApprovalOne($wh);
		}
	}
	function LV_Approval($lvarr)
	{
		$vlist=explode("@",$lvarr);
		foreach($vlist as $wh)
		{
	...
	function LV_ApprovalOne($vlv003)
	{
		$vDateStart=$vDateEnd=GetServerDate();
		$lvsql=" select MP.lv002,(if(ISNULL(PriceReReceiptQty),0,PriceReReceiptQty)+if(ISNULL(PriceInReceiptQty),0,PriceInReceiptQty))/(if(ISNULL(ReReceiptQty),0,ReReceiptQty)+if(ISNULL(InReceiptQty),0,InReceiptQty)) Price,
		if(ISNULL(ReReceiptQty),0,ReReceiptQty)-if(ISNULL(ReOutlv004),0,ReOutlv004)+if(ISNULL(InReceiptQty),0,InReceiptQty)-if(ISNULL(InOutlv004),0,InOutlv004) sl1,MP.lv005,if(ISNULL(ReReceiptQty),0,ReReceiptQty)-if(ISNULL(ReOutlv004),0,ReOutlv004)+if(ISNULL(InReceiptQty),0,InReceiptQty)-if(ISNULL(InOutlv004),0,InOutlv004) sl2,MP.lv007,MP.lv005,MP.lv005 from (
		select A.*,
		(select sum(A1.lv004) from wh_lv0009 A1 inner join wh_lv0008 A11 on A1.lv002=A11.lv001 where A1.lv003=A.lv002 and A11.lv009<'$vDateStart' and A11.lv002='$vlv003') ReReceiptQty,
		(select sum(A1.lv004*A1.lv008) from wh_lv0009 A1 inner join wh_lv0008 A11 on A1.lv002=A11.lv001 where A1.lv003=A.lv002 and A11.lv009<'$vDateStart' and A11.lv002='$vlv003') PriceReReceiptQty,
		(select sum(A1.lv004) from wh_lv0009 A1 inner join wh_lv0008 A11 on A1.lv002=A11.lv001 where A1.lv003=A.lv002 and A11.lv009>='$vDateStart' and A11.lv009<='$vDateEnd 23:00:00' and A11.lv002='$vlv003') InReceiptQty,
		(select sum(A1.lv004*A1.lv008) from wh_lv0009 A1 inner join wh_lv0008 A11 on A1.lv002=A11.lv001 where A1.lv003=A.lv002 and A11.lv009>='$vDateStart' and A11.lv009<='$vDateEnd 23:00:00' and A11.lv002='$vlv003') PriceInReceiptQty,
		(select sum(A2.lv004) from wh_lv0011 A2 inner join wh_lv0010 A21 on A2.lv002=A21.lv001 where A2.lv003=A.lv002 and A21.lv009<'$vDateStart' and A21.lv002='$vlv003') ReOutlv004 ,
		(select sum(A2.lv004) from wh_lv0011 A2 inner join wh_lv0010 A21 on A2.lv002=A21.lv001 where A2.lv003=A.lv002 and A21.lv009>='$vDateStart' and A21.lv009<='$vDateEnd 23:00:00' and A21.lv002='$vlv003') InOutlv004   
		from wh_lv0012 A inner join sl_lv0007 B on A.lv002=B.lv001 where A.lv003='$vlv003'  and B.lv015>=0 ) MP";
		$vresult=db_query($lvsql);
		while($vrow=db_fetch_array($vresult))
		{
			$sql="update wh_lv0012 set lv004='".$vrow['sl1']."',lv006='".$vrow['sl2']."',lv008='".$vrow['Price']."' where lv002='".$vrow['lv002']."' and lv003='".$vlv003."'";
			db_query($sql);
		}
		$vsql="delete from wh_lv0012 where (lv002 not in (select A.lv003 from wh_lv0009 A)) and (lv002 not in (select A.lv003 from wh_lv0011 A)) ";
		$vresult=db_query($vsql);
	//	return $vReturn;
	}
	function LV_ApprovalItem($vlv003,$vListItemID)
	{
		$vDateStart=$vDateEnd=GetServerDate();
		$lvsql=" select MP.lv002,(if(ISNULL(PriceReReceiptQty),0,PriceReReceiptQty)+if(ISNULL(PriceInReceiptQty),0,PriceInReceiptQty))/(if(ISNULL(ReReceiptQty),0,ReReceiptQty)+if(ISNULL(InReceiptQty),0,InReceiptQty)) Price,
		if(ISNULL(ReReceiptQty),0,ReReceiptQty)-if(ISNULL(ReOutlv004),0,ReOutlv004)+if(ISNULL(InReceiptQty),0,InReceiptQty)-if(ISNULL(InOutlv004),0,InOutlv004) sl1,MP.lv005,if(ISNULL(ReReceiptQty),0,ReReceiptQty)-if(ISNULL(ReOutlv004),0,ReOutlv004)+if(ISNULL(InReceiptQty),0,InReceiptQty)-if(ISNULL(InOutlv004),0,InOutlv004) sl2,MP.lv007,MP.lv005,MP.lv005 from (
		...
		(select sum(A2.lv004) from wh_lv0011 A2 inner join wh_lv0010 A21 on A2.lv002=A21.lv001 where A2.lv003=A.lv002 and A21.lv009<'$vDateStart' and A21.lv002='$vlv003') ReOutlv004 ,
		(select sum(A2.lv004) from wh_lv0011 A2 inner join wh_lv0010 A21 on A2.lv002=A21.lv001 where A2.lv003=A.lv002 and A21.lv009>='$vDateStart' and A21.lv009<='$vDateEnd 23:00:00' and A21.lv002='$vlv003') InOutlv004   
		from wh_lv0012 A inner join sl_lv0007 B on A.lv002=B.lv001 where A.lv003='$vlv003' and B.lv001 = '$vListItemID'   ) MP";
		$vresult=db_query($lvsql);
		while($vrow=db_fetch_array($vresult))
		{
			$sql="update wh_lv0012 set lv004='".$vrow['sl1']."',lv006='".$vrow['sl2']."',lv008='".$vrow['Price']."' where lv002='".$vrow['lv002']."' and lv003='".$vlv003."'";
	...
	function LV_Insert()
	{		
		//Tat chuc nang kiem tra quyen
		//if($this->isAdd==0) return false;
		$lvsql="insert into wh_lv0001 (lv001,lv002,lv003,lv004,lv005,lv006,lv007,lv008) values('$this->lv001','$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006','$this->lv007','$this->lv008')";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'wh_lv0001.insert',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Update()
	{
		...
		echo("toi dc dayy r");
		$lvsql="Update wh_lv0001 set lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004',lv005='$this->lv005',lv006='$this->lv006',lv007='$this->lv007',lv008='$this->lv008' where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
		if ($vReturn) {
			echo "Update dữ liệu thành công!";
		} else {
			echo "Lỗi khi update dữ liệu!";
		}
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'wh_lv0001.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Delete($lvarr)
	{
		...
		$lvsql = "DELETE FROM wh_lv0001  WHERE wh_lv0001.lv001 IN '$lvarr'";// and (select count(*) from wh_lv0001 B where  B.lv002= wh_lv0001.lv001)<=0  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'wh_lv0001.delete',sof_escape_string($lvsql));
	...
	function mb_Delete($lvarr)
	{
		//Tat kiem tra quyen
		//if($this->isDel==0) return false;
		$lvsql = "delete from wh_lv0001 WHERE lv001='$lvarr'";
		$vReturn= db_query($lvsql);
		if ($vReturn) {
			echo "Xoas dữ liệu thành công!";
		} else {
			echo "Lỗi khi xoas dữ liệu!";
		}
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'wh_lv0001.delete',sof_escape_string($lvsql));
		return $vReturn;
	}	
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM wh_lv0001 WHERE 1=1 ".$this->GetCondition();
	...
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT * FROM wh_lv0001 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('$this->Dir'+'wh_lv0001/?lang=".$this->lang."&func='+value+'&ID=".base64_encode($this->lv002)."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td align=@#05>@02</td>";
		$sqlS = "SELECT * FROM wh_lv0001 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
