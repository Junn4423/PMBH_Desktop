...
{
	//echo defined('DB_DATABASE');
	global $db_link;
	if($db_link) return $db_link;
	$db_link=mysqli_connect(DB_SERVER,DB_USER,DB_PWD);
	
	// Nếu không truyền $dbName, sử dụng DB_DATABASE từ CouchDB (nếu có)
	if($dbName == "" && defined('DB_DATABASE')) {
		$dbName = DB_DATABASE;
	}
...
function db_query($db_query, $dbName=""){
	global $db_link;
	$result=mysqli_query(db_connect($dbName),$db_query);
	return $result;
}
