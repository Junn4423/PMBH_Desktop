<?php
/////////////coding sl_lv0014///////////////
class sl_lv0014 extends lv_controler
{
	public $lv001 = null;
	public $lv002 = null;
	public $lv003 = null;
	...
	///////////
	public $DefaultFieldList = "lv001,lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv011,lv012,lv013,lv014,lv015,lv016,lv017,lv018,lv019,lv020,lv021,lv022,lv023,lv024";
	////////////////////GetDate
	...
	function LV_GetItemAdward($vContracID, $vProgId, $vItemID)
	{
		...
		$vsql = "select BB.lv003,CC.lv007 Price,(select DD.lv007 from sl_lv0007 DD where DD.lv001='$vItemID') PriceF from sl_lv0060 BB left join sl_lv0007 CC on BB.lv003=CC.lv001 where BB.lv002='$vProgId' and BB.lv007=1";
	...
	function LV_Insert()
	{
		...
		$vReturn = db_query($lvsql);
		if ($vReturn)
			$this->InsertLogOperation($this->DateCurrent, 'sl_lv0014.insert', sof_escape_string($lvsql));
		return $vReturn;
	}
	...
	{
		$lvsql = " select '$vTemID' lv002,A.lv004 lv003,A.lv005 lv004,B.lv004 lv005,A.lv006 lv006,B.lv008 lv007,B.lv006 congthu,concat(CURRENT_DATE(),' ',CURRENT_TIME()) lv014 from sl_lv0072 A inner join sl_lv0007 B on A.lv004=B.lv001 where A.lv003='$vlv002' and A.lv002='$vRoomID'";
		$vReturn = db_query($lvsql);
		while ($vrow = db_fetch_array($vReturn)) {
			$this->lv002 = $vrow['lv002'];
			$this->lv003 = $vrow['lv003'];
	...
	function LV_InsertOther($vlv002, $vreplace)
	{
		...
		$vReturn = db_query($lvsql);
		if ($vReturn)
			$this->InsertLogOperation($this->DateCurrent, 'sl_lv0014.InsertOther', sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Update()
	{
	...
	{
		$vsql = "select B.lv006,A.lv002 ContractID,C.lv007 RoomID from sl_lv0014 A inner join sl_lv0007 B on A.lv003=B.lv001 left join sl_lv0013 C on C.lv001=A.lv002 where A.lv001='" . $vDetailID . "' and (select AA.lv011 from sl_lv0013 AA where AA.lv001=A.lv002)<=0";
		$vresult = db_query($vsql);
		$vrow = db_fetch_array($vresult);
		if ($vrow) {
	...
	{
		$vsql = "select B.lv006,A.lv002 ContractID from sl_lv0014 A inner join sl_lv0007 B on A.lv003=B.lv001 where A.lv001='" . $vDetailID . "' and (select AA.lv011 from sl_lv0013 AA where AA.lv001=A.lv002)<=0";
		$vresult = db_query($vsql);
		$vrow = db_fetch_array($vresult);
		if ($vrow) {
			$this->lv002 = $vrow['ContractID'];
	...
	function LV_Delete($lvarr)
	{
		...
		$vReturn = db_query($lvsql1); {
			...
			$vReturn = db_query($lvsql);
			if ($vReturn)
				$this->InsertLogOperation($this->DateCurrent, 'sl_lv0014.delete', sof_escape_string($lvsql));
		}
		return $vReturn;
	}
	function LV_DeleteNoApr($lvarr)
	{
		if ($this->isDel == 0)
			return false;
		$lvsql1 = "insert into sl_lv0014_1 (lv024,lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv011,lv012,lv013,lv014,lv015,lv016,lv017,lv018,lv019,lv020,lv021)   (select lv001,lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv011,lv012,lv013,lv014,lv015,lv016,lv017,lv018,lv019,lv020,concat(IF(ISNULL(lv021),'',lv021),' Đã xoá') lv021 from sl_lv0014 where  lv018=0 And  lv001 IN ($lvarr) and (select B.lv011 from sl_lv0013 B where  B.lv001= sl_lv0014.lv002)<=0)  ";
		$vReturn = db_query($lvsql1); {
...
";
	...
	private function sqlconditionext($vFile, $vSelectID)
	{
		...
		switch ($vFile) {
			...
			case 'lv009':
				...
				break;
			case 'lv099':
				echo $vsql = "select A.lv001,A.lv002,IF(A.lv001='$vSelectID',1,0) lv003 from  sl_lv0059 A  where A.lv008=1 and (date(A.lv003)<=CURRENT_DATE() and date(A.lv004)>=CURRENT_DATE()) and (time(A.lv003)<=CURRENT_TIME() and time(A.lv004)>=CURRENT_TIME())";
				break;
	...
	function kiemTraSanPham($maHD, $maSP)
	{
		$lvsql = "select lv001 from sl_lv0014 where lv002= '{$maHD}' and lv003= '{$maSP}'";
		$vresult = db_query($lvsql);
		if ($vresult) {
			$vrow = db_fetch_array($vresult);
			if ($vrow) {
				echo "lv001: " . $vrow['lv001']; // Kiểm tra đầu ra
				return $vrow['lv001'];
			}
		}
	...
	function MB_Insert()
	{
		...
		if (!$this->LV_CheckLocked($this->lv002))
			return false;
		$sqlSelect = "select lv005, lv007, lv008 from sl_lv0007 where lv001='{$this->lv003}'";
		// echo $sqlSelect;
		$result = db_query($sqlSelect);
		if ($result && $row = db_fetch_array($result)) {
			$this->lv005 = $row['lv005'];
			$this->lv006 = $row['lv007'];
			$this->lv007 = $row['lv008'];
		}
		$this->lv013 = ($this->lv013 != "") ? recoverdate(($this->lv013), $this->lang) : $this->DateDefault;
		$this->lv014 = ($this->lv014 != "") ? recoverdate(($this->lv014), $this->lang) : $this->DateDefault;
		$lvsql = "insert into sl_lv0014 (lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv012,lv014) values('$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006','$this->lv007','$this->lv008','$this->lv012','$this->DateCurrent()')";
		$vReturn = db_query($lvsql);
		if ($vReturn)
			$this->InsertLogOperation($this->DateCurrent, 'sl_lv0014.insert', sof_escape_string($lvsql));
		return $vReturn;
	}
	function MB_UpdateQty($maCTHD, $vQty)
	{
		if (!$this->LV_CheckLocked($this->lv002))
			return false;
		// Lấy giá trị hiện tại của lv004
		$sqlSelect = "select lv004 from sl_lv0014 where lv001 = '{$maCTHD}'";

		$result = db_query($sqlSelect);

		if ($result && $vrow = db_fetch_array($result)) {
			$oldQty = $vrow['lv004'];
		}
		// Cập nhật lv004 với giá trị mới và lưu giá trị cũ vào lv103
		$lvsql = "update sl_lv0014 set lv103 = '{$oldQty}', lv004 = '{$vQty}' where lv001 = '{$maCTHD}'";
		$vReturn = db_query($lvsql);
		if ($vReturn) {
			$this->InsertLogOperation($this->DateCurrent, 'sl_lv0014.update', sof_escape_string($lvsql));
		}
	...
	function themCtdh($maHD, $maSP, $soLuong)
	{

		//Kiểm tra sản phẩm đã tồn tại trong hóa đơn chưa
		$sanPhamTonTai = $this->kiemTraSanPham($maHD, $maSP);
		...
		if ($sanPhamTonTai != '') {
			...
			$this->lv003 = $maSP;
			$this->lv004 = $soLuong;
			return $this->MB_UpdateQty($sanPhamTonTai, $soLuong);
		} else {
			// Nếu chưa tồn tại, thêm mới
			$this->lv002 = $maHD;
			$this->lv003 = $maSP;
			$this->lv004 = $soLuong;
			return $this->MB_Insert();
		}
	}
	function layMonNuocTuBanDangBan() {
		// Lấy danh sách loại sản phẩm của món nước từ bảng sl_lv0070
		$rowConf = db_fetch_array(db_query("SELECT lv021 FROM sl_lv0070 LIMIT 1"), MYSQLI_ASSOC);
		$listLoaiNuoc = explode(',', $rowConf['lv021']); // convert to mảng
		
	
		$listLoaiNuocStr = "'" . implode("','", array_map('trim', $listLoaiNuoc)) . "'";
		$lvsql = "
			SELECT cthd.lv001 as idCthd, ban.lv001 as idBan, sp.lv002 as tenNuoc, cthd.lv004 as soLuong, cthd.lv006 as gia, cthd.lv014 as thoiGian, ban.lv002 as banOrder
			FROM sl_lv0013 hd 
			JOIN sl_lv0014 cthd ON hd.lv001 = cthd.lv002 
			JOIN sl_lv0007 sp ON cthd.lv003 = sp.lv001
			JOIN sl_lv0009 ban ON hd.lv007 = ban.lv001 
			JOIN sl_lv0006 loaisp ON sp.lv003 = loaisp.lv001
			WHERE hd.lv011 = 0 
				AND cthd.lv019 = 0 
				AND sp.lv003 IN ($listLoaiNuocStr)
		";
		...
		while ($vrow = db_fetch_array($vresult, MYSQLI_ASSOC)) {
			$vArrRe[] = [
				'tenNuoc' => $vrow['tenNuoc'],
				'soLuong' => $vrow['soLuong'],
				'gia' => $vrow['gia'],
				'thoiGian' => $vrow['thoiGian'],
				'banOrder' => $vrow['banOrder'],
				"idBan" => $vrow["idBan"],
				"idCthd" => $vrow["idCthd"]
			];
		}
	...
	function layMonAnTuBanDangBan() {
		// Lấy danh sách loại sản phẩm của món ăn từ bảng sl_lv0070
		$rowConf = db_fetch_array(db_query("SELECT lv022 FROM sl_lv0070 LIMIT 1"), MYSQLI_ASSOC);
		$listLoaiAn = explode(',', $rowConf['lv022']); // convert to mảng
	
		$listLoaiAnStr = "'" . implode("','", array_map('trim', $listLoaiAn)) . "'";
	...
	function layHoaDonTuBanDangBan()
	{
		$lvsql = "SELECT cthd.lv001 as idCthd, ban.lv001 as idBan, sp.lv002 as tenNuoc, cthd.lv004 as soLuong, cthd.lv006 as gia,  cthd.lv014 as thoiGian, ban.lv002 as banOrder
			FROM sl_lv0013 hd 
			JOIN sl_lv0014 cthd ON hd.lv001 = cthd.lv002 
			JOIN sl_lv0007 sp ON cthd.lv003 = sp.lv001
			JOIN sl_lv0009 ban ON hd.lv007 = ban.lv001 
			WHERE hd.lv011 = 0  AND cthd.lv019 = 0";
		$vresult = db_query($lvsql);
	...
	function layHoaDonTuBanDaXong()
	{
		$lvsql = "SELECT cthd.lv001 as idCthd, ban.lv001 as idBan, sp.lv002 as tenNuoc, cthd.lv004 as soLuong, cthd.lv006 as gia,  cthd.lv014 as thoiGian, ban.lv002 as banOrder
			...
			JOIN sl_lv0007 sp ON cthd.lv003 = sp.lv001
			JOIN sl_lv0009 ban ON hd.lv007 = ban.lv001 
			WHERE hd.lv011 = 0  AND cthd.lv019 = 1";
		$vresult = db_query($lvsql);
	...
	// function updateTrangThaiMon($itemId, $status)
	// {
	// 	// Kiểm tra tham số đầu vào
	// 	if (empty($itemId) || ($status !== '0' && $status !== '1')) {
	// 		return false;
	// 	}

	// 	// Cập nhật trạng thái món trong bảng sl_lv0014
	// 	$sql = "UPDATE sl_lv0014 SET lv019 = '$status' WHERE lv001 = '$itemId'";
	...
	function xoaCthd($idCt)
	{
		// Cập nhật trạng thái món trong bảng sl_lv0014
		$sql = "DELETE FROM sl_lv0014 WHERE lv001 = '$idCt'";
		// Thực thi câu lệnh SQL sử dụng hàm db_query thay vì $this->db->query
		$result = db_query($sql);
		return $result;
	}
	function thanhToan($cusid, $donhangid, $txtlv004, $txtlv008, $txtlv010, $userID)
	{
		require_once("../cafe/clsall/wh_lv0010.php");
		require_once("../cafe/clsall/wh_lv0011.php");
		require_once("../cafe/clsall/wh_lv0034.php");
		// Debug statements removed - they break JSON response
		$cusid=$userID;
		...
		if(($lvwh_lv0034->lv003!=NULL && trim($lvwh_lv0034->lv003)!=''))
		{
			...
			if($lvwh_lv0010->LV_CheckData($donhangid,$vWarehouseID))
			{
				$lvwh_lv0010->lv001=str_replace(" ","",'PX-'.str_replace(":","",str_replace("/","",$lvwh_lv0010->DateCurrent))."-".rand(0,100));
				$lvwh_lv0010->lv002=$vWarehouseID;
				$lvwh_lv0010->lv003=$lvwh_lv0010->LV_UserID;
				$lvwh_lv0010->lv004=$txtlv004;
				$lvwh_lv0010->lv005='CONTRACT';
				$lvwh_lv0010->lv006=$donhangid;
				$lvwh_lv0010->lv007=0;	
				$lvwh_lv0010->lv008=$txtlv008;
				$lvwh_lv0010->lv009=$this->DateCurrent;
				$lvwh_lv0010->lv010=$txtlv010;
				$lvwh_lv0010->lv011=$lvsl_lv0013->LV_UserID;
				$vresult=$lvwh_lv0010->LV_Insert();
				if($vresult)
				{
					$vresult1=$lvwh_lv0011->LV_InsertTempSL($lvwh_lv0010->lv001,$donhangid,$vWarehouseID);
				}
				
			}
		}
		// Return the result of the update query (already executed above)
		return $vresult;
	}
