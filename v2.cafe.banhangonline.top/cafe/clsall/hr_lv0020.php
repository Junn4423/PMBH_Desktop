...
///////////
	public $DefaultFieldList="lv001,lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv011,lv012,lv013,lv014,lv015,lv016,lv017,lv018,lv019,lv020,lv021,lv043,lv022,lv023,lv024,lv025,lv026,lv027,lv028,lv029,lv030,lv044,lv031,lv032,lv033,lv034,lv035,lv036,lv037,lv038,lv039,lv040,lv041,lv042";	
////////////////////GetDate
...
////////////
	...
	var $ArrPush=array();
	var $ArrFunc=array();
	var $ArrGet=array("lv001"=>"2","lv002"=>"3","lv003"=>"4","lv004"=>"5","lv005"=>"6","lv006"=>"7","lv007"=>"8","lv008"=>"9","lv009"=>"10","lv010"=>"11","lv011"=>"12","lv012"=>"13","lv013"=>"14","lv014"=>"15","lv015"=>"16","lv016"=>"17","lv017"=>"18","lv018"=>"19","lv019"=>"20","lv020"=>"21","lv021"=>"22","lv022"=>"23","lv023"=>"24","lv024"=>"25","lv025"=>"26","lv026"=>"27","lv027"=>"28","lv028"=>"29","lv029"=>"30","lv030"=>"31","lv031"=>"32","lv032"=>"33","lv033"=>"34","lv034"=>"35","lv035"=>"36","lv036"=>"37","lv037"=>"38","lv038"=>"39","lv039"=>"40","lv040"=>"41","lv041"=>"42","lv042"=>"43","lv043"=>"44","lv044"=>"45");
	...
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT * FROM hr_lv0020 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('hr_lv0020/?lang=".$this->lang."&func='+value,'','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$lvTd="<td align=@#05>@02</td>";
		$lvImg="<td  ><img name=\"imgView\" border=\"0\" style=\"border-color:#CCCCCC\" title=\"\" alt=\"Image\" width=\"96px\" height=\"128px\" src=\"../../images/employees/@01/@02\" /></td>";
		$sqlS = "SELECT * FROM hr_lv0020 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
	...
	{
		$vArrRe = [];
		$sql = "SELECT * FROM hr_lv0022";
		$rs = db_query($sql);
		while ($row = db_fetch_array($rs, MYSQLI_ASSOC)) {
			$vArrRe[] = $row;
		}

		$config = include('config_field_links.php');
		resolve_foreign_keys($vArrRe, config: $config);

		return $vArrRe;
	}
	//Cong Cao
	function hr_LoadNhanSu()
	{
		$vArrRe = [];
		$sql = "SELECT * FROM hr_lv0020";
		$rs = db_query($sql);
		while ($row = db_fetch_array($rs, MYSQLI_ASSOC)) {
			$vArrRe[] = $row;
		}
	...
	/*function LV_InsertPersonnel()
	{
		// Hàm xử lý ngày bên trong luôn, tương tự hàm update
		$recoverdate = function ($dateStr) {
		...
		// Danh sách các field ngày cần chuyển
		$dateFields = ['lv011', 'lv015', 'lv021', 'lv030', 'lv044'];

		// Gán lại định dạng cho từng field
		foreach ($dateFields as $field) {
			$this->$field = !empty($this->$field) ? $recoverdate($this->$field) : $this->DateDefault;
		}

		// Câu lệnh SQL insert
		$lvsql = "INSERT INTO hr_lv0020 (
		lv001, lv002, lv005, lv006, lv007, lv008, lv009, lv010,
		...
		lv041, lv042, lv043, lv044
	) VALUES (
		'$this->lv001','$this->lv002','$this->lv005','$this->lv007','$this->lv008',
		'$this->lv009','$this->lv010','$this->lv011','$this->lv017','$this->lv018',
		'$this->lv019','$this->lv020','$this->lv021','$this->lv022','$this->lv023',
		'$this->lv024','$this->lv025','$this->lv031','$this->lv034','$this->lv035',
		'$this->lv036','$this->lv037','$this->lv038','$this->lv039','$this->lv040',
		'$this->lv041','$this->lv042'
	)";
		echo $lvsql; 
		$vReturn = db_query($lvsql);

		if ($vReturn) {
			$this->InsertLogOperation($this->DateCurrent, 'hr_lv0020.insert', sof_escape_string($lvsql));
		}
	...
	function LV_InsertPersonnel()
	{
		...
		$fields = [
			'lv001', 'lv002', 'lv005', 'lv007', 'lv008', 'lv009', 'lv010', 'lv011',
			'lv017', 'lv018', 'lv019', 'lv020', 'lv021', 'lv022', 'lv023', 'lv024',
			'lv025', 'lv031', 'lv034', 'lv035', 'lv036', 'lv037', 'lv038', 'lv039',
			'lv040', 'lv041', 'lv042'
		];

		// Xử lý định dạng ngày (chỉ cho những trường có định dạng dd/mm/yyyy)
		$dateFields = ['lv011', 'lv021'];
		$recoverDate = function ($dateStr) {
			if (!$dateStr || $dateStr == '') return null;
			$parts = explode('/', $dateStr);
			if (count($parts) === 3) {
		...
		$values = implode(", ", array_map(function ($field) {
			return "'" . sof_escape_string($this->$field) . "'";
		}, $fields));

		$sql = "INSERT INTO hr_lv0020 ($columns) VALUES ($values)";
		// echo $sql; // Debug nếu cần

		$result = db_query($sql);

		if ($result) {
			$this->InsertLogOperation($this->DateCurrent, 'hr_lv0020.insert', sof_escape_string($sql));
		}
	...
	function LV_UpdatePersonnel()
	{
		// Hàm xử lý ngày nằm bên trong luôn
		$recoverdate = function ($dateStr) {
			if (!$dateStr || $dateStr == '') return null;
