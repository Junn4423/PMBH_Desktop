...
////////////
	...
	function LV_LoadGroupSanPham($vWhID,&$strthanh,&$strsanpham,&$i,$widthsp="67%",$widthgroup="32%")
	{
		...
		$strsanpham='<div style="width:'.$widthsp.';float:left;overflow: auto;height:400px;padding:0px!important"  class="thanhprocess">
							...
							if($lsGroup=='')
								...
								$vsql="select * from sl_lv0006  where lv001 in ($lsGroup) and  lv004=0 and (ISNULL(lv003) or lv003='' or lv001=lv003) order by lv005 asc";
									...
									$strsanpham=$strsanpham.'<div style="float:left;width:97%">
										...
										<div>
											...
											if($lsItem=='')
												...
												$vsql1="select * from sl_lv0007 where lv003='".$vrow['lv001']."' and lv001 in ($lsItem) order by lv010,lv002 asc";
	...
	function LV_GetNhomSanPham($vListItem)
	{
		...
		if($vListItem=='')
			...
			$vsql="select distinct lv003 from sl_lv0007 where lv001 in ($vListItem)";
		$vresult=db_query($vsql);
		while($vrow=db_fetch_array($vresult))
		{
			if($vReturn=="")
	...
	{
		$vsql="select distinct lv003 from sl_lv0006 where lv001 in ($vGroupItem)";
		$vresult=db_query($vsql);
		while($vrow=db_fetch_array($vresult))
		{
			if($vReturn=="")
	...
	function LV_Insert()
	{
		...
		if ($this->lv005 != "") {
			if (preg_match('/^\d{4}-\d{2}-\d{2}( \d{2}:\d{2}:\d{2})?$/', $this->lv005)) {
				// already ISO datetime (with optional time) — use as-is
				$this->lv005 = $this->lv005;
			} else {
				$this->lv005 = recoverdate(($this->lv005), $this->lang);
			}
		} else {
			$this->lv005 = $this->DateDefault;
		}
	...
	function LV_GetDetail($vContractID,$vBangID,$vLangArr,&$vSum)
	{
		...
		';		
		$lvsql="select A.*,D.lv002 UnitName,C.lv002 Name,C.lv009 ColorName from wh_lv0005 A inner join wh_lv0004 B on A.lv002=B.lv001 inner join sl_lv0007 C on A.lv003=C.lv001 left join sl_lv0005 D on C.lv004=D.lv001 where 1=1 and B.lv001='$vContractID'";	
		$vresult=db_query($lvsql);
		$i=1;
		$vSum=0;
		$vSumTT=0;
	...
	{
		$vsql="select lv001,lv002 from sl_lv0005";
		$vresult=db_query($vsql);
		while($vrow=db_fetch_array($vresult))
		{
	...
	function LV_GetDetailFull($vContractID,$vBangID,$vLangArr,&$vSum)
	{
		...
		';		
		$lvsql="select A.*,D.lv002 UnitName,C.lv002 Name,C.lv009 ColorName,C.lv004 DonVi,C.lv005 DonViKhac,C.lv006 QuiDoi from wh_lv0005 A inner join wh_lv0004 B on A.lv002=B.lv001 inner join sl_lv0007 C on A.lv003=C.lv001 left join sl_lv0005 D on C.lv004=D.lv001 where 1=1 and B.lv001='$vContractID'";	
		$vresult=db_query($lvsql);
		$i=1;
		$vSum=0;
		$vSumTT=0;
		...
		while($vrow = db_fetch_array ($vresult))
		{
			...
			<td align="center"><input onfocus="this.select()"  id="soluongkiem_'.$vrow['lv001'].'" style="min-width:53px;width:100%;text-align:center;" type="textbox" value="'.$vrow['lv006'].'" onblur="changeSLKKFull(\''.$vrow['lv003'].'\',this,\''.$vrow['lv001'].'\')" title="'.$this->obj_conf->lv004.'"/></td>
			<td align="center"><input onfocus="this.select()"  id="soluongthuc_'.$vrow['lv001'].'" style="min-width:53px;width:100%;text-align:center;" type="textbox" value="'.$vrow['lv008'].'" onblur="changepriceFull(\''.$vrow['lv003'].'\',this,\''.$vrow['lv001'].'\')" title="'.$this->obj_conf->lv004.'"/></td>
			<td align="center">
...
//////////////////////Buil list////////////////////
	...
	function MB_Delete($maPhieuKiem)
	{
		$vsql = "delete from wh_lv0004 where lv001='$maPhieuKiem'";
		$vresult = db_query($vsql);
		if($vresult)
		{
			return true;
	...
	function generateMaPhieuKiem(){
		$month = date('m');
		$year = date('Y');
		$vsql = "SELECT MAX(SUBSTRING_INDEX(lv001, '-', -1)) AS maxIndex 
				FROM wh_lv0004 WHERE lv001 LIKE 'INV-$month/$year-%'";
		$vresult = db_query($vsql); 
		$lastIndex = 0;
		if ($vresult) {
		...
		return "INV-$month/$year-$formattedIndex";
	}
	function MB_Load($maKho)
	{
		$vsql = "select * from  wh_lv0004 where lv002='$maKho'";
		$vresult = db_query($vsql);
		return $vresult;
	}
	function MB_LoadByID($maPhieuKiem)
	{
		$vsql = "select * from  wh_lv0004 where lv001='$maPhieuKiem'";
		$vresult = db_query($vsql);
		return $vresult;
	}
}
