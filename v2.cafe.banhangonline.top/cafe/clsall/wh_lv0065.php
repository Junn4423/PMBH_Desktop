<?php
/////////////coding wh_lv0065///////////////
class   wh_lv0065 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
///////////
	public $DefaultFieldList="lv904,lv994,lv995,lv002,lv003,lv004,lv006,lv011,lv024,lv010";	
////////////////////GetDate
	...
	protected $objhelp='wh_lv0065';
////////////
	var $ArrOther=array();
	var $ArrPush=array();
	var $ArrFunc=array();
	var $ArrGet=array("lv001"=>"2","lv002"=>"3","lv003"=>"4","lv004"=>"5","lv005"=>"6","lv006"=>"7","lv007"=>"8","lv008"=>"9","lv009"=>"10","lv010"=>"11","lv011"=>"12","lv012"=>"13","lv013"=>"14","lv014"=>"15","lv015"=>"16","lv016"=>"17","lv017"=>"18","lv018"=>"19","lv019"=>"20","lv020"=>"21","lv021"=>"22","lv022"=>"23","lv023"=>"24","lv024"=>"25","lv904"=>"905","lv994"=>"995","lv995"=>"996");
	var $ArrView=array("lv001"=>"0","lv002"=>"0","lv003"=>"0","lv004"=>"10","lv005"=>"0","lv006"=>"10","lv007"=>"0","lv008"=>"10","lv009"=>"0","lv010"=>"0","lv011"=>"10","lv012"=>"10","lv013"=>"2","lv014"=>"2","lv015"=>"0","lv016"=>"0","lv017"=>"10","lv018"=>"10","lv019"=>"10","lv020"=>"0","lv021"=>"0","lv022"=>"0","lv023"=>"0","lv024"=>"10","lv904"=>"2","lv994"=>"0","lv995"=>"0");	
	...
	$vHeaderReportInventory="
		<table width=\"100%\" align=\"center\" cellpadding=\"0\" cellspacing=\"0\" border=\"1\" class=\"tblprint\">
			<tr height=\"30px\" class=\"tblcaption\">
				...
				<td rowspan=\"2\" class=\"htable\" width=\"10%\" style=\"cursor:pointer\">Tổng chi mua hàng</td>
				<td class=\"htable\" width=\"*%\" style=\"cursor:pointer\" colspan=\"4\">Nội dung mua hàng</td>
			</tr>
			<tr>
		...
		if(count($this->vArrDay)>0){
			foreach($this->vArrDay as $vDateCal)
			{
				...
				{
					case 1:
						$vsql="select A.lv003,sum(A.lv004) lv004,'' lv006,'' lv011,sum(A.lv004*A.lv008) thanhtien,C.lv002 Names from wh_lv0009 A inner join sl_lv0007 C on A.lv003=C.lv001 inner join wh_lv0008 B on A.lv002=B.lv001 and substr(B.lv009,1,10)='".$vDateCal."' group by A.lv003";
						break;
					case 2:
						$vsql="select C.lv003,sum(A.lv004) lv004,'' lv006,'' lv011,sum(A.lv004*A.lv008) thanhtien,D.lv002 Names from wh_lv0009 A inner join sl_lv0007 C on A.lv003=C.lv001 inner join wh_lv0008 B on A.lv002=B.lv001 and substr(B.lv009,1,10)='".$vDateCal."' left join sl_lv0006 D on C.lv003=D.lv001 group by C.lv003";
						break;
					case 3:
						$vsql="select B.lv010,sum(A.lv004) lv004,'' lv006,'' lv011,sum(A.lv004*A.lv008) thanhtien,C.lv002 Names from wh_lv0009 A  inner join wh_lv0008 B on A.lv002=B.lv001 and substr(B.lv009,1,10)='".$vDateCal."' inner join hr_lv0020 C on B.lv003=C.lv001 group by B.lv003";
						break;
					case 4:
						$vsql="select B.lv002,sum(A.lv004) lv004,'' lv006,'' lv011,sum(A.lv004*A.lv008) thanhtien,C.lv002 Names from wh_lv0009 A inner join wh_lv0008 B on A.lv002=B.lv001 and substr(B.lv009,1,10)='".$vDateCal."' inner join wh_lv0003 C on B.lv006=C.lv001  group by B.lv006";
						break;
					default:
						$vsql="select A.lv003,A.lv004,A.lv008 lv006,A.lv012 lv011,(A.lv004*A.lv008) thanhtien,C.lv002 Names from wh_lv0009 A inner join sl_lv0007 C on A.lv003=C.lv001 inner join wh_lv0008 B on A.lv002=B.lv001 and substr(B.lv009,1,10)='".$vDateCal."'";
						break;
				}
				$bResultS = db_query($vsql);
				while($vrow =db_fetch_array($bResultS))
				{
					$vContract=$vContract+round($vrow['thanhtien'],0);
					if($vNumline==0)
					{
						...
						$vLineRun = str_replace("@10", $this->FormatView(round($vrow['thanhtien'],0),20),$vLineRun);
					}
					else
					{
						...
						$vLineRun1 = str_replace("@10", $this->FormatView(round($vrow['thanhtien'],0),20),$vLineRun1);
						$strExpportAll1 = $strExpportAll1.$vLineRun1;
					}
					$vNumline++;
				}
				$vSumContract=$vSumContract+$vContract;
				$vLineRun = str_replace("@03", $this->FormatView(round($vContract,-3),20), $vLineRun);
				if($vNumline==0)
				{
		...
		$vincrease++;
		$vOrder++;
		$vRowLast=str_replace("@03",$this->FormatView(round($vSumContract,-3),10),$vRowLast);
	...
	{
		$strCondi="";
		if($this->datefrom!="") $strCondi=$strCondi." and B.lv004>= '$this->datefrom 00:00:00'";
		if($this->dateto!="") $strCondi=$strCondi." and B.lv004<= '$this->dateto 23:59:59'";
...
//////////////////////Buil list////////////////////
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT A.*,B.lv004 lv904,substr(B.lv004,12,5) lv994,substr(B.lv005,12,5) lv995 FROM sl_lv0014 A inner join sl_lv0013 B on A.lv002=B.lv001 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		...
		while ($vrow = db_fetch_array ($bResult)){
			$strL="";
			$vorder++;
			$vThanhTien=$vrow['lv004']*$vrow['lv006']+$vrow['lv004']*$vrow['lv006']*$vrow['lv008']/100-$vrow['lv004']*$vrow['lv006']*$vrow['lv011']/100;
			$vSumLv024=$vSumLv024+$vThanhTien;
			...
			for($i=0;$i<count($lstArr);$i++)
			{
				...
				{
					case 'lv024':
						$vTemp=str_replace("@02",str_replace("@02",$this->getvaluelink($lstArr[$i],$this->FormatView($vThanhTien,(int)$this->ArrView[$lstArr[$i]])),str_replace("@01",$vrow['lv001'] ,$lvHref)),$this->Align($lvTd,(int)$this->ArrView[$lstArr[$i]]));
						break;
					default:
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		function Export(vFrom,value)
		{
			...
			window.open('".$this->Dir."wh_lv0065/?lang=".$this->lang."&childdetailfunc='+value+'&datefrom='+datefrom+'&dateto='+dateto,'','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		while ($vrow = db_fetch_array ($bResult)){
			...
			$vSumLv024=$vSumLv024+$vThanhTien;
			$vSumLv004=$vSumLv004+$vrow['lv004'];
			$this->Amount=$this->Amount+$vThanhTien;
			for($i=0;$i<count($lstArr);$i++)
			{
				switch($lstArr[$i])
				{
					...
					case 'lv024':
						$vTemp=str_replace("@02",$this->getvaluelink($lstArr[$i],$this->FormatView($vThanhTien,(int)$this->ArrView[$lstArr[$i]])),$this->Align($lvTd,(int)$this->ArrView[$lstArr[$i]]));
						break;
					default:
