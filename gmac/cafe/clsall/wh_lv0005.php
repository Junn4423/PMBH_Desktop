<?php
/////////////coding wh_lv0005///////////////
class   wh_lv0005 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
///////////
	public $DefaultFieldList="lv001,lv002,lv003,lv004,lv008,lv005,lv006,lv007,lv099";	
////////////////////GetDate
	...
	public $paging=null;
	public $lang=null;
	protected $objhelp='wh_lv0005';
////////////
	var $ArrOther=array();
	var $ArrPush=array();
	var $ArrFunc=array();
	var $ArrGet=array("lv001"=>"2","lv002"=>"3","lv003"=>"4","lv004"=>"5","lv005"=>"6","lv006"=>"7","lv007"=>"8","lv008"=>"9","lv009"=>"10","lv010"=>"11","lv011"=>"12","lv012"=>"13","lv013"=>"14","lv099"=>"100");
	var $ArrView=array("lv001"=>"0","lv002"=>"0","lv003"=>"0","lv004"=>"10","lv005"=>"0","lv006"=>"10","lv007"=>"0","lv008"=>"10","lv009"=>"0","lv010"=>"10","lv011"=>"0","lv012"=>"0","lv013"=>"10","lv099"=>"0");	
	...
	{
		$lvsql="select lv007 from wh_lv0004 B where  B.lv001='$this->lv002'";
	...
	{
		$lvsql="select * from  wh_lv0005 Where lv001='$vlv001'";
	...
	function LV_Insert()
	{
		...
		if(!$this->LV_CheckLocked($this->lv002)) return false;
		 $lvsql="insert into wh_lv0005 (lv001,lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv011,lv012) values('$this->lv001','$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006','$this->lv007','$this->lv008','$this->lv009','$this->lv010','$this->lv011','$this->lv012')";
		$vReturn= db_query($lvsql);
	...
	function LV_Update()
	{
		...
		if(!$this->LV_CheckLocked($this->lv002)) return false;
		  $lvsql="Update wh_lv0005 set lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004',lv005='$this->lv005',lv006='$this->lv006',lv007='$this->lv007',lv008='$this->lv008',lv009='$this->lv009',lv010='$this->lv010',lv011='$this->lv011',lv012='$this->lv012' where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'wh_lv0005.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_UpdateOther()
	{
		if($this->isEdit==0) return false;
		if(!$this->LV_CheckLocked($this->lv002)) return false;
	    $lvsql="Update wh_lv0005 set lv008='$this->lv008',lv010='$this->lv010',lv012='$this->lv012' where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'wh_lv0005.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	...
	{
		$lvsql="select lv007 from  wh_lv0004 Where lv001='$vlv002'";
		$vresult=db_query($lvsql);
		if($vresult){
		$vrow=db_fetch_array($vresult);
			if($vrow)
	...
	function LV_Delete($lvarr)
	{
		...
		$lvsql = "DELETE FROM wh_lv0005  WHERE wh_lv0005.lv001 IN ($lvarr) and (select lv007 from wh_lv0004 B where  B.lv001= wh_lv0005.lv002)<=0  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'wh_lv0005.delete',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_DeleteOrther($lvarr)
	{
		if($this->isDel==0) return false;
		$lvsql = "DELETE FROM wh_lv0005  WHERE wh_lv0005.lv002 IN ($lvarr) and (select lv007 from wh_lv0004 B where  B.lv001= wh_lv0005.lv002)<=0  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'wh_lv0005.delete',sof_escape_string($lvsql));
		return $vReturn;
	}
	...
	{
		$lvsql="select lv001 from  wh_lv0005 Where lv002='$vContracID' and lv003='$vItemID'";
		$vresult=db_query($lvsql);
		$vrow=db_fetch_array($vresult);
		if($vrow)
		{
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM wh_lv0005 WHERE 1=1 ".$this->GetCondition();
...
//////////////////////Buil list////////////////////
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT *,lv003 lv099 FROM wh_lv0005 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		...
		while ($vrow = db_fetch_array ($bResult)){
			$strL="";
			$vorder++;
			$vsumsl4=$vsumsl4+$vrow['lv004'];
			$vsumsl6=$vsumsl6+$vrow['lv006'];
			$vsumsl8=$vsumsl8+$vrow['lv008'];
			$vsumsl10=$vsumsl10+$vrow['lv010'];
			for($i=0;$i<count($lstArr);$i++)
			{
		...
		$strF=$strF."</tr>";
		$strF=str_replace("<!--lv004-->",$this->FormatView($vsumsl4,10),$strF);
		$strF=str_replace("<!--lv006-->",$this->FormatView($vsumsl6,10),$strF);
		$strF=str_replace("<!--lv008-->",$this->FormatView($vsumsl8,10),$strF);
		$strF=str_replace("<!--lv010-->",$this->FormatView($vsumsl10,10),$strF);
...
//////////////////////Buil list////////////////////
	function LV_BuilListInput($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		<table  align=\"center\" class=\"lvtable\">
		<!--<tr ><td colspan=\"".(2+count($lstArr))."\" class=\"lvTTable\">".$this->ArrPush[0]."</td></tr>-->
		<tr ><td colspan=\"".(count($lstArr))."\">"."<a class=\"lvtoolbar\" href=\"javascript:Save();\" tabindex=\"47\"><img src=\"../images/controlright/save_f2.png\"  alt=\"Save\" title=\"<?php echo $vLangArr[1];?>\"
name=\"save\" border=\"0\" align=\"middle\" id=\"save\" /> <?php echo $vLangArr[2];?></a>"."</td>
<td colspan=\"2\" align=right>".$this->ListFieldSave($lvFrom,$lvList,$maxRows,$lvOrderList,$lvSortNum)."</td>
</tr>
@#01
<tr>
...
<tr>
    <td colspan=\"".(count($lstArr))."\">"."<a class=\"lvtoolbar\" href=\"javascript:Save();\" tabindex=\"47\"><img
                src=\"../images/controlright/save_f2.png\" alt=\"Save\" title=\"<?php echo $vLangArr[1];?>\"
                name=\"save\" border=\"0\" align=\"middle\" id=\"save\" /> <?php echo $vLangArr[2];?></a>"."</td>
...
$lvTd="<td>@02";
    $lvTdInput="
<td><input id=\"@03\" name=\"@03\" value=\"@02\" tabindex=2 onKeyUp=\"return
        CheckKeyCheckOther(event,2,'$lvChk',$lvFrom, '$lvChk' , '$lvChkAll' ,@13,'@14')\"></td>";
...
for($i=0;$i<count($lstArr);$i++) { $vTemp=str_replace("@01","",$lvTdH); $vTemp=str_replace("@02",$this->
    ...
    $strL="";
    $vorder++;
    for($i=0;$i<count($lstArr);$i++) { switch($lstArr[$i]) { case 'lv008' : case 'lv010' : case 'lv012' :
        $vTemp=str_replace("@02",str_replace("@02",$this->
        getvaluelink($lstArr[$i],$this->FormatView($vrow[$lstArr[$i]],(int)$this->ArrView[$lstArr[$i]])),str_replace("@01",$vrow['lv001']
        ,$lvHref)),str_replace("@14","txt".$lstArr[$i],str_replace("@03","txt".$lstArr[$i].$vorder,str_replace("@13",$vorder,$lvTdInput))));
        break;
        case 'lv001':
        case 'lv002':
        $vTemp=str_replace("@02",str_replace("@02",$this->getvaluelink($lstArr[$i],$this->FormatView($vrow[$lstArr[$i]],(int)$this->ArrView[$lstArr[$i]])),str_replace("@01",$vrow['lv001']
        ...
        function Export(vFrom, value) {
            window.open('".$this->Dir."wh_lv0005/?lang=".$this->lang."&childdetailfunc=' + value +
                '&ID=".base64_encode($this->lv002)."', '',
        ...
        $lvSelect="<ul id=\"menu-nav\" onkeyup=\"return CheckKeyCheckTab(event,$lvFrom,".count($lstArr).")\">
            <li class=\"menusubT\"><img src=\"$this->Dir../images/lvicon/config.png\" border=\"0\"
                class=\"lv_funcshowimg\"/><span class=\"lv_funcshowtext\">".$this->ArrFunc[11]."</span>
                <ul id=\"submenu-nav\">
                    @#01
                </ul>
            </li>
        ...
        for ($i=0;$i<count($lstArr);$i++) { $strTempChk=str_replace("@01",$i,$lvChk.$this->
            ...
            $lvTable="<div align=\"center\">
                <div ondblclick=\"this.innerHTML='' \">".$this->Get_TitleHeader()."</div>
            </div>
            <div align=\"center\">
            ...
            $strF="<tr>
                ...
                $lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
                $lvTd="<td align=@#05>@02</td>";
                $sqlS = "SELECT *,lv003 lv099 FROM wh_lv0005 WHERE 1=1 ".$this->GetCondition()." $strSort LIMIT $curRow,
                $maxRows";
                $vorder=$curRow;
                $bResult = db_query($sqlS);
                $this->Count=db_num_rows($bResult);
                ...
                for($i=0;$i<count($lstArr);$i++) { $vTemp=str_replace("@01","",$lvTdH); $vTemp=str_replace("@02",$this->
                    ...
                    for($i=0;$i<count($lstArr);$i++) { $vTemp=str_replace("@02",$this->
                        getvaluelink($lstArr[$i],$this->FormatView($vrow[$lstArr[$i]],(int)$this->ArrView[$lstArr[$i]])),$this->Align($lvTd,(int)$this->ArrView[$lstArr[$i]]));
                        $strL=$strL.$vTemp;
                        }
            ...
            $strF=str_replace("
            <!--lv004-->",$this->FormatView($vsumsl4,10),$strF);
            $strF=str_replace("
            <!--lv006-->",$this->FormatView($vsumsl6,10),$strF);
            $strF=str_replace("
            <!--lv008-->",$this->FormatView($vsumsl8,10),$strF);
            $strF=str_replace("
            <!--lv010-->",$this->FormatView($vsumsl10,10),$strF);
            $strF=str_replace("
            <!--lv003-->",'<p style="text-align:center;padding:5px">Tổng:</p>',$strF);
            $lvTable=str_replace("@#02",$strF,$lvTable);
            $strTrH=str_replace("@#01",$strH,$lvTrH);
            return str_replace("@#01",$strTrH.$strTr,$lvTable);
            }
            ...
            //////////////////////Buil list////////////////////
            //////////////////////Buil list////////////////////
            function LV_BuilListReportOther($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow,
            $maxRows,$paging,$lvOrderList,$vTax)
            {
            ...
            return
            ($lvopt==0)?$row['lv002']:(($lvopt==1)?$row['lv001']."(".$row['lv002'].")":(($lvopt==2)?$row['lv002']."(".$row['lv001'].")":$row['lv001']));
            }

            }
            function MB_LoadID($vlv001)
            {
            $lvsql="select * from wh_lv0005 Where lv002='$vlv001'";
            $vresult=db_query($lvsql);
            return $vresult;
            }
            function MB_Insert(){
            //if($this->isAdd==0) return false;
            if(!$this->LV_CheckLocked($this->lv002)) return false;
            $lvsql="insert into wh_lv0005 (lv001,lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv011,lv012)
            values('$this->lv001','$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv008','$this->lv007','$this->lv008','$this->lv009','$this->lv010','$this->lv011','$this->lv012')";
            $vReturn= db_query($lvsql);
            if($vReturn) $this->InsertLogOperation($this->DateCurrent,'wh_lv0005.insert',sof_escape_string($lvsql));
            return $vReturn;
            }
            ...
            // if($this->isEdit==0) return false;
            if(!$this->LV_CheckLocked($this->lv002)) return false;
            $lvsql="Update wh_lv0005 set
            lv007='$this->lv007',lv008='$this->lv008',lv006='$this->lv008'
            where lv002='$this->lv002' and lv003='$this->lv003'";
            $vReturn= db_query($lvsql);
            if($vReturn) $this->InsertLogOperation($this->DateCurrent,'wh_lv0005.update',sof_escape_string($lvsql));
            return $vReturn;
            }
            function MB_Delete($maPhieu, $maSanPham)
            {
            //if(!$this->LV_CheckLocked($this->lv002)) return false;
            $lvsql= 'delete from wh_lv0005 where lv002="'.$maPhieu.'" and lv003="'.$maSanPham.'"';
            $vReturn= db_query($lvsql);
            if($vReturn) $this->InsertLogOperation($this->DateCurrent,'wh_lv0005.delete',sof_escape_string($lvsql));
            return $vReturn;
            }
