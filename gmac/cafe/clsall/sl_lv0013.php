...
";
	...
	// ham dung de gop ban
	function Mb_GopBan($idDonHang, $idBanGop)
	{
		...
		if (!db_query($updateLv024)) {
			$success = false;
			$errorMessage .= "Lỗi khi cập nhật lv024.\n";
		}
		...
		foreach ($mang as $id) {
            ...
            WHERE lv002 = '$vMaDonHang'";
				...
				if (!db_query($insertCthd)) {
					$success = false;
					$errorMessage .= "Lỗi khi chèn dữ liệu từ hóa đơn cũ ($vMaDonHang).\n";
				}
				...
				if (!db_query($deleteCthd)) {
					$success = false;
					$errorMessage .= "Lỗi khi xóa hóa đơn cũ ($vMaDonHang).\n";
				}
				...
				if (!db_query($updateOldInvoice)) {
					$success = false;
					$errorMessage .= "Lỗi khi cập nhật trạng thái hóa đơn cũ ($vMaDonHang).\n";
				}
			}
		}
		...
		return [
			'success' => $success,
			'message' => $success ? 'Gộp bàn thành công!' : 'Gộp bàn thất bại!',
		];
	}
	function Mb_TaoHoaDon($idBan)
	{
		$success = true; // Biến để theo dõi trạng thái thực hiện
		$errorMessage = '';
		...
		$lvsql = "INSERT INTO sl_lv0013 (
			...
			'$maHoaDon', '$ngayHienTai', '$idBan', '$trangThai', '$ngTao'
