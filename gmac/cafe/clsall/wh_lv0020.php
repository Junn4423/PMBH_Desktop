<?php
/////////////coding wh_lv0020///////////////
class   wh_lv0020 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
////////////
	...
	var $ArrView=array("lv001"=>"0","lv002"=>"0","lv003"=>"0","lv004"=>"2","lv005"=>"10","lv006"=>"10","lv007"=>"22","lv008"=>"0","lv009"=>"10");	
	...
	{
		$lvsql="select * from  wh_lv0020 Where concat(lv001,'-',lv002,'-',lv003)='$vlv001'";
	...
	function LV_Insert()
	{
		...
		$lvsql="insert into wh_lv0020 (lv001,lv002,lv003,lv004,lv005,lv006,lv007,lv008) values('$this->lv001','$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006','$this->lv007','$this->lv008')";
	...
	function LV_Update()
	{
		...
		$this->lv004 = ($this->lv004!="")?recoverdate(($this->lv004), $this->lang):$this->DateDefault;
		//$this->lv007 = ($this->lv007!="")?recoverdate(($this->lv007), $this->lang):$this->DateDefault;
		 $lvsql="Update wh_lv0020 set lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004',lv005='$this->lv005',lv006='$this->lv006',lv008='$this->lv008' where  concat(lv001,'-',lv002,'-',lv003)='$this->lv001';";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'wh_lv0020.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Delete($lvarr)
	{
		if($this->isDel==0) return false;
		$lvsql = "DELETE FROM wh_lv0020  WHERE concat(lv001,'-',lv002,'-',lv003) IN ($lvarr)";// and (select count(*) from wh_lv0020 B where  B.lv002= wh_lv0020.lv001)<=0  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'wh_lv0020.delete',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_Get_slt_lot($vLotId,$vItemId,$vWhId)
	{
		...
		$vresult=db_query($vsql);
		$vrow=db_fetch_array($vresult);
		$slxuat=(float)$vrow['sumslxuat'];
		return $slnhap-$slxuat;
	}
	function LV_Get_slt_lot_all($vLotId,$vItemId)
	{
	...
	function LV_Get_slt_nolot_all($vItemId)
	{

		$vsql="select sum(lv004) sumslnhap from wh_lv0009 A where  A.lv003='$vItemId'";
		$vresult=db_query($vsql);
		$vrow=db_fetch_array($vresult);
		$slnhap=$vrow['sumslnhap'];
		$vsql="select sum(lv004) sumslxuat from wh_lv0011 A where A.lv003='$vItemId'";
		$vresult=db_query($vsql);
		$vrow=db_fetch_array($vresult);
		$slxuat=$vrow['sumslxuat'];
		return $slnhap-$slxuat;
	}
	function LV_Get_slt_lot_once($vLotId,$vItemId,$vWhID)
	{

		$vsql="select sum(A.lv004) sumslnhap from wh_lv0009 A inner join wh_lv0008 B on A.lv002=B.lv001 where  A.lv003='$vItemId' and A.lv014='$vLotId'  and  B.lv002='$vWhID'";
		$vresult=db_query($vsql);
		$vrow=db_fetch_array($vresult);
		$slnhap=$vrow['sumslnhap'];
		$vsql="select sum(A.lv004) sumslxuat from wh_lv0011 A inner join wh_lv0010 B on A.lv002=B.lv001 where A.lv003='$vItemId' and A.lv014='$vLotId'  and  B.lv002='$vWhID'";
		$vresult=db_query($vsql);
		$vrow=db_fetch_array($vresult);
		$slxuat=$vrow['sumslxuat'];
		return $slnhap-$slxuat;
	}
	function LV_Get_slt_nolot_once($vItemId,$vWhID)
	{

		$vsql="select sum(A.lv004) sumslnhap from wh_lv0009 A inner join wh_lv0008 B on A.lv002=B.lv001 where  A.lv003='$vItemId' and  B.lv002='$vWhID'";
		$vresult=db_query($vsql);
		$vrow=db_fetch_array($vresult);
		$slnhap=$vrow['sumslnhap'];
		$vsql="select sum(A.lv004) sumslxuat from wh_lv0011 A inner join wh_lv0010 B on A.lv002=B.lv001 where A.lv003='$vItemId' and  B.lv002='$vWhID'";
		$vresult=db_query($vsql);
		$vrow=db_fetch_array($vresult);
		$slxuat=$vrow['sumslxuat'];
		return $slnhap-$slxuat;
	}
	function LV_Get_Field_lot($vLotId,$vItemId,$vField)
	{
		$vsql="select $vField  from wh_lv0020  where lv002='$vItemId' and lv001='$vLotId'";
		$vresult=db_query($vsql);
		$vrow=db_fetch_array($vresult);
		$slnhap=$vrow[$vField];
		return $slnhap;
	...
	protected function GetCondition()
	{
		...
		$strCondi=$strCondi." and lv003 in ($strwh)";
		return $strCondi;
	}
		////////////////Count///////////////////////////
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM wh_lv0020 WHERE 1=1 ".$this->GetCondition();
		$bResultC = db_query($sqlC);
		$arrRowC = db_fetch_array($bResultC);
		return $arrRowC['nums'];
	}
	...
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT * FROM wh_lv0020 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		...
		while ($vrow = db_fetch_array ($bResult)){
			...
			{
				if($lstArr[$i]=="lv009")
					$vTemp=str_replace("@02",$this->getvaluelink($lstArr[$i],$this->FormatView($this->LV_Get_slt_lot($vrow['lv001'],$vrow['lv002'],$vrow['lv003']),(int)$this->ArrView[$lstArr[$i]])),$this->Align($lvTd,(int)$this->ArrView[$lstArr[$i]]));
				else
			...
			$strTr=$strTr.str_replace("@#01",$strL,str_replace("@02",$vrow['lv001']."-".$vrow['lv002']."-".$vrow['lv003'],str_replace("@03",$vorder,str_replace("@01",$vorder%2,$lvTr))));
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('$this->Dir'+'wh_lv0020/?lang=".$this->lang."&childfunc='+value+'&ID=".base64_encode($this->lv002)."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td align=@#05>@02</td>";
		$sqlS = "SELECT * FROM wh_lv0020 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
	...
	function Mb_LaySoLuongTonKhoNhieu($arrItemId, $vWhID) {
		// array(mã sản phẩm)
		if (empty($arrItemId)) return [];

		$arrItemId = array_map('addslashes', $arrItemId);
		$itemList = "'" . implode("','", $arrItemId) . "'";
	
		// Nhập kho
		$sqlNhap = "SELECT A.lv003 as maSP, SUM(A.lv004) as slNhap
					FROM wh_lv0009 A
					INNER JOIN wh_lv0008 B ON A.lv002 = B.lv001
					WHERE A.lv003 IN ($itemList) AND B.lv002 = '$vWhID'
					GROUP BY A.lv003";
	
		$resNhap = db_query($sqlNhap);
		$arrNhap = [];
		
		while ($row = db_fetch_array($resNhap)) {
			$arrNhap[$row['maSP']] = floatval($row['slNhap']);
		}
	
		// Xuất kho
		$sqlXuat = "SELECT A.lv003 as maSP, SUM(A.lv004) as slXuat
					FROM wh_lv0011 A
					INNER JOIN wh_lv0010 B ON A.lv002 = B.lv001
					WHERE A.lv003 IN ($itemList) AND B.lv002 = '$vWhID'
					GROUP BY A.lv003";
		...
		$arrXuat = [];
		while ($row = db_fetch_array($resXuat)) {
			$arrXuat[$row['maSP']] = floatval($row['slXuat']);
		}
	
		// Tổng hợp tồn kho từng mã
		$result = [];
		foreach ($arrItemId as $maSP) {
			$slNhap = isset($arrNhap[$maSP]) ? $arrNhap[$maSP] : 0;
			$slXuat = isset($arrXuat[$maSP]) ? $arrXuat[$maSP] : 0;
			$result[] = [
				'maSP' => $maSP,
				'soLuong' => $slNhap - $slXuat
			];
		}
		return $result;
	}
