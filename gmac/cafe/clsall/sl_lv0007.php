...
////////////
	...
	//end
						/////////////////////////////KeBao////////////////
	function mb_GetProductByID($maKho){
		$vsql="select * from  sl_lv0007 Where lv016='$maKho'";
		$vresult=db_query($vsql);
		return $vresult;
	}
						/////////////////////////////////END////////////////
	...
	{
		$lvsql="select * from  sl_lv0007 Where lv001='$vlv001'";
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('sl_lv0007/?lang=".$this->lang."&func='+value,'','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td align=@#05>@02</td>";
		$sqlS = "SELECT * FROM sl_lv0007 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
	...
	function LoadSanPhamTheoIdLoai($maLoai)
	{
		$lvsql="select * from  sl_lv0007 Where lv003='$maLoai'";
		$vresult=db_query($lvsql);
	...
	function LayAllNguyenLieu(){
		$vsql="Select * from sl_lv0007 where (lv003 in ('nuocngot','thuocla') or lv001 like 'NL%') ";
		//SELECT * FROM sl_lv0007 WHERE (lv003 IN ('nuocngot', 'thuocla') OR lv001 LIKE 'NL%') and lv016 = '$maKho'
		$vresult=db_query($vsql);
		return $vresult;
	}
	function MB_Delete($maSanPham){
		$vsql = "DELETE FROM sl_lv0007 WHERE lv001 = '$maSanPham'";
		$vresult = db_query($vsql);
		return $vresult;
	}
	function MB_LoadAll()
	{
		$lvsql="select * from  sl_lv0007 ";
		$vresult=db_query($lvsql);
	...
	function UploadImage($maSp, $imageFile) {
		...
		if (move_uploaded_file($imageFile["tmp_name"], $targetFile)) {
			...
			$lvsql = "UPDATE sl_lv0007 SET lv014 = '$imagePath' WHERE lv001 = '$maSp'";
			$vReturn = db_query($lvsql);
