...
//////////////////////Buil list////////////////////
	...
	function MB_Load_NVL()
	{
		$vsql="SELECT * FROM sl_lv0006 WHERE lv003 !='' or lv001 IN('nuocngot','thuocla')";
		$vresult=db_query($vsql);
		return $vresult;
	}
	function LoadLoaiSanPham()
	{
		...
		while ($vrow = db_fetch_array($vresult, MYSQLI_ASSOC)) {
			$vArrRe[] = [
				"idLoaiSp" => $vrow["lv001"],
				"tenLoaiSp" => $vrow["lv002"],
			];
		}
		return $vArrRe;
		
	}
		function MB_LoadDanhMucSP()
	{
		$vsql="select * from  sl_lv0006 where lv003=''";
		$vresult=db_query($vsql);
		return $vresult;
		
	}
	function MB_Delete($idLoai){
		$vsql = "DELETE FROM sl_lv0006 WHERE lv001 = '$idLoai'";
		$vresult = db_query($vsql);
		return $vresult;
	}
}
