...
if(isset($_GET['ajaxitemsend']))
{
	...
	if($vContractID!="" && $vContractID!=NULL)
	{
		...
		if($vresult)
		{
			...
			echo $lvwh_lv0008->LV_GetDetailMuaHang($vContractID,$vWHID,$vLangArr,$vSum);
			echo '[ENDHOPDONGMONEY]';
			echo '[HOPDONGSUM]';
			echo $lvwh_lv0008->FormatView($vSum,10);
			echo '[ENDHOPDONGSUM]';
			echo '[HOPDONGUNG]';
			echo 0;
			echo '[ENDHOPDONGUNG]';
...
{	
	...
	$vsum10=$lvwh_lv0008->LV_GetContractMoney($vdonhangid,10);
		...
		echo '[ENDSOLUONGKHO]';
		echo '[SOLUONGMUA]';
			echo $lvwh_lv0008->FormatView($vsum10,10);
		echo '[ENDSOLUONGMUA]';
...
if(isset($_GET['ajaxaproval']))
{
		...
		echo '[ENDDONHANGNHAN]';
		echo '[EMPTYCONTRACT]';
		echo $lvwh_lv0008->LV_GetDetailMuaHang('','',$vLangArr,$vSum);
...
<!-------------------SP------------------------->
				<div class="khungsp">
						<div id="kk_detailview" style="background:#fff;height:400px;overflow: scroll;width100%;">
							...
							<?php 
							$vLangArr=GetLangFile("$vDir../","SL0049.txt",$plang);
							echo $lvwh_lv0008->LV_GetDetailMuaHang($lvwh_lv0008->lv001,$vWHID,$vLangArr,$vSum);
							?>
							</div>
						</div>
