<?php
/////////////coding jo_lv0004///////////////
class   jo_lv0022 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
	...
	public $lv023=null;
///////////
	public $DefaultFieldList="lv001,lv099,lv015,lv002,lv003,lv022,lv016,lv017,lv009,lv010,lv030,lv021,lv008,lv039,lv040,lv023";
////////////////////GetDate
...
////////////
	...
	{
		$vReturn='';
		$lvsql="select lv001 from hr_lv0002 where lv102='$vlv100'";
		$vresult=db_query($lvsql);
		while($vrow=db_fetch_array($vresult))
		{
	...
	function LV_GetDep($vDepID)
	{
		...
		{
			$vsql="select lv001,lv102 from  hr_lv0002 where lv001 in ($vReturn) ";
			$bResult=db_query($vsql);
			while ($vrow = db_fetch_array ($bResult)){
				//$vReturn=$vReturn.",'".$vrow['lv001']."'";
				if(trim($vrow['lv102'])=='') $vReturn=$vReturn.",".$this->LV_GetChildDep($vrow['lv001']);
			}
		}
	...
	function LV_GetChildDep($vDepID)
	{
		...
		$vReturn="'".str_replace(",","','",$vDepID)."'";
		$vsql="select lv001,lv102 from  hr_lv0002 where lv002 in ($vReturn) ";
		$bResult=db_query($vsql);
		while ($vrow = db_fetch_array ($bResult)){
			if(trim($vrow['lv102'])=='') $vReturn=$vReturn.",".$this->LV_GetChildDep($vrow['lv001']);
		}
	...
	function LV_Aproval($lvarr)
	{
		...
		while ($vrow = db_fetch_array ($vReturn)){
				...
				$vTo=$vrow['lv032'].";".$vrow['lv031'];
				$vTo=str_replace(";;",";",$vTo);
				if($vTo!="") $this->LV_SendMail($vrow['lv037'].'<br>Đã duyệt cấp 3',$vrow['lv038'].' đã duyệt cấp 3','admin',$lvemail,$vTo);
				$lvsql="update jo_lv0004 set lv021=1,lv019=NOW(),lv028='' where lv001='".$vrow['lv001']."'";
	...
	{
		if($this->isApr==0) return false;
		$lvsql = "Update jo_lv0004 set lv021='1',lv019=NOW()  WHERE jo_lv0004.lv001 IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
		if($vReturn){
		 	$this->InsertLogOperation($this->DateCurrent,'jo_lv0004.unapproval',sof_escape_string($lvsql));
		 }
		return $vReturn;
	}	
	function LV_UnAproval($lvarr)
	{
		...
		while ($vrow = db_fetch_array ($vReturn)){
				...
				$vTo=$vrow['lv032'].";".$vrow['lv031'];
				$vTo=str_replace(";;",";",$vTo);
				if($vTo!="") $this->LV_SendMail($vrow['lv037'].'<br>Không duyệt cấp 3',$vrow['lv038'].' không duyệt cấp 3','admin',$lvemail,$vTo);
				$lvsql="update jo_lv0004 set lv021=-1,lv019=NOW(),lv028='' where lv001='".$vrow['lv001']."'";
				$vReturn1= db_query($lvsql);
				if($vReturn1){
				 	$this->InsertLogOperation($this->DateCurrent,'jo_lv0004.unapproval',sof_escape_string($lvsql));
			...
			for($i=0;$i<count($lstArr);$i++)
			{
				switch($lstArr[$i])
				{
					case 'lv030':
							if($this->GetApr()==1)
							{
								$lvTdTextBox="<td align=@#05 rowspan=\"@#99\"><input type=\"textbox\" value=\"@02\" @03 onblur=\"UpdateText(this,'".$vrow['lv001']."',30)\" style=\"width:125px\" tabindex=\"2\"  maxlength=\"32\"  /></td>";
