...
///////////Load max////////////
	...
	{
		$vCode=$_GET['code'];
		if($vCode=='' || $vCode==NULL)  $vCode=$_POST['code'];
		$vToken=$_GET['token'];
		if($vToken=='' || $vToken==NULL) $vToken=$_POST['token'];
	...
	{
		$lvsql="select count(*) Nums from  hr_lv0020 Where lv001='$vCode' and lv197='$vToken'";
		$vresult=db_query($lvsql);
		$vrow=db_fetch_array($vresult);
		if($vrow)
		{
			if($vrow['Nums']==0 ) return false;
			return true;
		}
		return false;
	}
...
//////////Insert Save Oper//////////////////////////////////	
	...
	public function FormatView($vValue,$vopt)
	{
		...
		switch($vopt)
		{
			...
			case 2:
			if($vValue=='' || $vValue=='0000-00-00') return '';
			return formatdate($vValue,$this->lang);
			case 22: return formatdate(substr($vValue,0,10),$this->lang).' '.substr($vValue,10,9);
			case 3: return '***';
			case 4:return formatdate($vValue,$this->langconver)." ".substr($vValue,11,8);
			case 5: $this->langconver='EN';if($vValue<0) return '('.LCurrency(-$vValue,$this->langconver).')'; else return LCurrency($vValue,$this->langconver);
