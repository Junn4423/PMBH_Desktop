<?php
/////////////coding jo_lv0004///////////////
class   jo_lv0008 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
	...
	public $lv023=null;
///////////
	public $DefaultFieldList="lv001,lv099,lv015,lv002,lv003,lv022,lv016,lv017,lv009,lv004,lv008,lv039,lv040,lv023";	
////////////////////GetDate
...
////////////
	...
	var $ArrPush=array();
	var $ArrFunc=array();
	var $ArrGet=array("lv001"=>"2","lv002"=>"3","lv003"=>"4","lv004"=>"5","lv005"=>"6","lv006"=>"7","lv007"=>"8","lv008"=>"9","lv009"=>"10","lv010"=>"11","lv011"=>"12","lv012"=>"13","lv013"=>"14","lv014"=>"15","lv015"=>"16","lv016"=>"17","lv017"=>"18","lv018"=>"19","lv019"=>"20","lv020"=>"21","lv021"=>"22","lv022"=>"23","lv023"=>"24","lv024"=>"25","lv025"=>"26","lv039"=>"40","lv040"=>"41","lv099"=>"100");
	var $ArrView=array("lv001"=>"0","lv002"=>"0","lv003"=>"0","lv004"=>"0","lv005"=>"2","lv006"=>"0","lv007"=>"2","lv008"=>"0","lv009"=>"0","lv010"=>"0","lv011"=>"0","lv012"=>"0","lv013"=>"0","lv014"=>"0","lv015"=>"0","lv016"=>"4","lv017"=>"4","lv018"=>"4","lv019"=>"4","lv020"=>"0","lv021"=>"0","lv022"=>"0","lv023"=>"4","lv024"=>"2","lv025"=>"0","lv039"=>"0","lv040"=>"0","lv099"=>"0");	
	...
	{
		$vReturn='';
		$lvsql="select lv001 from hr_lv0002 where lv100='$vlv100'";
		$vresult=db_query($lvsql);
		while($vrow=db_fetch_array($vresult))
		{
	...
	function LV_GetDep($vDepID)
	{
		...
		{
			$vsql="select lv001,lv100 from  hr_lv0002 where lv001 in ($vReturn) ";
			$bResult=db_query($vsql);
			while ($vrow = db_fetch_array ($bResult)){
				//$vReturn=$vReturn.",'".$vrow['lv001']."'";
				if(trim($vrow['lv100'])=='') $vReturn=$vReturn.",".$this->LV_GetChildDep($vrow['lv001']);
			}
		}
	...
	function LV_GetChildDep($vDepID)
	{
		...
		$vReturn="'".str_replace(",","','",$vDepID)."'";
		$vsql="select lv001,lv100 from  hr_lv0002 where lv002 in ($vReturn) ";
		$bResult=db_query($vsql);
		while ($vrow = db_fetch_array ($bResult)){
			if(trim($vrow['lv100'])=='') $vReturn=$vReturn.",".$this->LV_GetChildDep($vrow['lv001']);
		}
	...
	function LV_Aproval($lvarr)
	{
		...
		while ($vrow = db_fetch_array ($vReturn)){
				...
				$vTo=$vrow['lv032'];
				if($vTo!="") $this->LV_SendMail($vrow['lv035'],$vrow['lv038'],'admin',$lvemail,$vTo);
				$lvsql="update jo_lv0004 set lv004=1,lv007=NOW(),lv024='' where lv001='".$vrow['lv001']."'";
	...
	{
		if($this->isApr==0) return false;
		$lvsql = "Update jo_lv0004 set lv004='1',lv007=NOW()  WHERE jo_lv0004.lv001 IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
		if($vReturn){
		 	$this->InsertLogOperation($this->DateCurrent,'jo_lv0004.approval',sof_escape_string($lvsql));
		 }
	...
	function LV_UnAproval($lvarr)
	{
		...
		$lvsql = "Update jo_lv0004 set lv004='-1',lv007=NOW(),lv024=''  WHERE jo_lv0004.lv001 IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
			...
			for($i=0;$i<count($lstArr);$i++)
			{
				switch($lstArr[$i])
				{
					case 'lv009':
							if($this->GetApr()==1)
							{
								$lvTdTextBox="<td align=@#05 rowspan=\"@#99\"><input type=\"textbox\" value=\"@02\" @03 onblur=\"UpdateText(this,'".$vrow['lv001']."',9)\" style=\"width:125px\" tabindex=\"2\"  maxlength=\"32\"  /></td>";
