<?php
/////////////coding jo_lv0100///////////////
class   jo_lv0100 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
////////////
	...
	{
		$vArrRe=Array();
		$vsql="select * from  jo_lv0100 where 1=1 ".$this->GetCondition()."";
	...
	{
		$vArrShift=Array();
		$vsql="select * from  jo_lv0100 where lv001<>'' and lv005<>'' order by lv003 ASC";
		$vresult=db_query($vsql);
		$i=0;
		while($vrow=db_fetch_array($vresult))
		{
			...
			if($vrow['lv005']!="" && $vrow['lv005']!=NULL)
			{
				$vArrS=explode(",",$vrow['lv005']);
				foreach($vArrS as $Shift)
				{
					if($Shift!="")
					{
						$vArrShift['CA-'.$Shift][$vrow['lv004']]=$vrow['lv001'];
					}
				}
	...
	{
		$lvsql="select * from  jo_lv0100 Where lv001='$vlv001'";
	...
	//	$this->lv005 = ($this->lv005!="")?recoverdate(($this->lv005), $this->lang):$this->DateDefault;
		$lvsql="insert into jo_lv0100 (lv001,lv002,lv003,lv004,lv005) values('$this->lv001','$this->lv002','$this->lv003','$this->lv004','$this->lv005')";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'jo_lv0100.insert',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_Update()
	{
		if($this->isEdit==0) return false;
		 //$this->lv005 = ($this->lv005!="")?recoverdate(($this->lv005), $this->lang):$this->DateDefault;
		 $lvsql="Update jo_lv0100 set lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004',lv005='$this->lv005' where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'jo_lv0100.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Delete($lvarr)
	{
		if($this->isDel==0) return false;
		$lvsql = "DELETE FROM jo_lv0100  WHERE jo_lv0100.lv001 IN ($lvarr)";// and (select count(*) from jo_lv0100 B where  B.lv002= jo_lv0100.lv001)<=0  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'jo_lv0100.delete',sof_escape_string($lvsql));
		return $vReturn;
	}	
	/////lv admin deletet
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM jo_lv0100 WHERE 1=1 ".$this->GetCondition();
	...
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT * FROM jo_lv0100 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('$this->Dir'+'jo_lv0100/?lang=".$this->lang."&func='+value+'&ID=".base64_encode($this->lv002)."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td align=@#05>@02</td>";
		$sqlS = "SELECT * FROM jo_lv0100 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
...
//		if($this->isView==0) return false;
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td align=@#05>@02</td>";
		$sqlS = "SELECT * FROM jo_lv0100 WHERE 1=1  ".$this->RptCondition." order by lv001 LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
