<?php
/////////////coding tc_lv0004///////////////
class   tc_lv0004 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
////////////
	...
	var $ArrView=array("lv001"=>"0","lv002"=>"0","lv003"=>"0","lv004"=>"0","lv005"=>"0","lv006"=>"0","lv007"=>"0","lv008"=>"2");
	...
	{
		$vArrRe=Array();
		$vsql="select * from  tc_lv0004 where 1=1 ".$this->GetCondition()."";
	...
	function LV_LoadArray()
	{
		$vArrShift=Array();
		$vsql="select * from  tc_lv0004 where lv001<>'' order by lv003 ASC";
		$vresult=db_query($vsql);
		$i=0;
		while($vrow=db_fetch_array($vresult))
		{
			$vArrShift[$i][1][0]=$vrow['lv001'];
			$vArrShift[$i][2][0]=$vrow['lv002'];
			$vArrShift[$i][3][0]=$vrow['lv003'];
			$vArrShift[$i][3][1]=(int)str_replace(":","",$vrow['lv003']);
			$vArrShift[$i][4][0]=$vrow['lv004'];
			$vArrShift[$i][4][1]=(int)str_replace(":","",$vrow['lv004']);
			$vArrShift[$i][5][0]=$vrow['lv005'];
			$vArrShift[$i][6][0]=$vrow['lv006'];
			$vArrShift[$i][6][1]=(int)str_replace(":","",$vrow['lv006']);
			$vArrShift[$i][7][0]=$vrow['lv007'];
			$vArrShift[$i][7][1]=(int)str_replace(":","",$vrow['lv007']);
			$vArrShift[0]['CA-'.$vrow['lv001']][5]=$vrow['lv005'];
			$vArrShift[0]['IN-'.$vrow['lv001']][3]=$vrow['lv003'];
			$vArrShift[0]['OUT-'.$vrow['lv001']][4]=$vrow['lv004'];
			$vArrShift[0]['GAYO-'.$vrow['lv001']][6]=$vrow['lv006'];
			$vArrShift[0]['GAYT-'.$vrow['lv001']][7]=$vrow['lv007'];
			$vArrShift[0]['GAY-'.$vrow['lv001']][1]=0;
			//Xác dinh gio lam cho ca
			if($vArrShift[$i][3][1]>$vArrShift[$i][4][1])
			{
				//Ca quan ngày
				$vArrShift[0]['WD-'.$vrow['lv001']][0]=TIMEDIFF(TIMEADD(TIMEDIFF('24:00:00',$vrow['lv003']),TIMEDIFF($vrow['lv004'],'00:00:00')),$vrow['lv005']);
				//$vArrShift[0][0][0]=$vrow['lv001'];
				$vArrShift[0]['NS-'.$vrow['lv001']][0]=1;
			}
			else
				$vArrShift[0]['WD-'.$vrow['lv001']][0]=TIMEDIFF(TIMEDIFF($vrow['lv004'],$vrow['lv003']),$vrow['lv005']);
			
			if($vrow['lv006']!=NULL && $vrow['lv006']!="00:00:00")
			{
				$vArrShift[0]['GAY-'.$vrow['lv001']][1]=1;
				$vArrShift[0]['WD-'.$vrow['lv001']][0]=TIMEDIFF(TIMEADD(TIMEDIFF($vrow['lv004'],$vrow['lv003']),TIMEDIFF($vrow['lv007'],$vrow['lv006'])),$vrow['lv005']);
			}
			else
			{
				if($vrow['lv007']!=NULL && $vrow['lv007']!="00:00:00") $vArrShift[0]['GAY-'.$vrow['lv001']][1]=1;
			}
			//echo $vrow['lv001'].":".$vArrShift[0]['WD-'.$vrow['lv001']][0];
			//echo "<br/>";
			$i++;
		}
		return $vArrShift;
	...
	{
		$lvsql="select * from  tc_lv0004 Where lv001='$vlv001'";
	...
	function LV_Insert()
	{
		...
		$lvsql="insert into tc_lv0004 (lv001,lv002,lv003,lv004,lv005,lv006,lv007,lv008) values('$this->lv001','$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006','$this->lv007','$this->lv008')";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'tc_lv0004.insert',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_Update()
	{
		...
		$this->lv008 = ($this->lv008!="")?recoverdate(($this->lv008), $this->lang):$this->DateDefault;
		$lvsql="Update tc_lv0004 set lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004',lv005='$this->lv005',lv006='$this->lv006',lv007='$this->lv007',lv008='$this->lv008' where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'tc_lv0004.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Delete($lvarr)
	{
		if($this->isDel==0) return false;
		$lvsql = "DELETE FROM tc_lv0004  WHERE tc_lv0004.lv001 IN ($lvarr)";// and (select count(*) from tc_lv0004 B where  B.lv002= tc_lv0004.lv001)<=0  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'tc_lv0004.delete',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_GetTimeShift($vStart,$vEnd,$vState=0,$vFreeTime='00:00:00')
	{
		$vState=(int)$vState;
		if($vStart==$vEnd)
		{
		...
		else
		{
			...
			else
			{
				...
				else
				{
					$vGetS=$vStart;
					if($vState==1)
					$vGetS=CompareTime($this->StartTime,$vStart)?$this->StartTime:$vStart;								
					else
					$vGetS=CompareTime($this->StartTime,$vStart)?$this->StartTime:$vStart;
				}
			}
			...
			else
			{
				...
				{	$vGetE=$vEnd;
					if($vState==1)
					$vGetE=CompareTime($vEnd,$this->EndTime)?$this->EndTime:$vEnd;								
					else
					$vGetE=CompareTime($vEnd,$this->EndTime)?$this->EndTime:$vEnd;	
				}
			}
			$lvTime= TIMEDIFF($vGetE,$vGetS);
			if($vFreeTime=='00:00:00' || $vFreeTime=='00:00')
				return $lvTime;
			else
				return TIMEDIFF($lvTime,$vFreeTime);
			
		}
	}
	function GetHoursActual($vCalTime,$vMaxTime)	
	{
		
		$vHourShift="00:00:00";
		if(CompareTime($this->StartTime,$this->EndTime))
		{
			$vHourShift=TIMEDIFF("24:00:00",$this->StartTime)+TIMEDIFF($this->EndTime,"00:00:00");
		}
		else
		{
			$vHourShift=TIMEDIFF($this->EndTime,$this->StartTime);
		}
		$vTimeTemp=TIMEDIFF($vHourShift,$vCalTime);
			return TIMEDIFF($vMaxTime,$vTimeTemp);
	}
	/////lv admin deletet
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM tc_lv0004 WHERE 1=1 ".$this->GetCondition();
	...
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT * FROM tc_lv0004 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
	...
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('".$this->Dir."tc_lv0004/?lang=".$this->lang."&func='+value+'&ID=".base64_encode($this->lv002)."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
";
	...
	function LV_BuilListReportTC($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$vArr)
	{
...
//		if($this->isView==0) return false;
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td align=@#05>@02</td>";
		$sqlS = "SELECT * FROM tc_lv0004 WHERE 1=1  ".$this->RptCondition." order by lv001 LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
		...
		while ($vrow = db_fetch_array ($bResult)){
			...
			{
				if($lstArr[$i]=='lv005')
					$vTemp=str_replace("@02",$this->getvaluelink($lstArr[$i],$this->FormatView($vArr[$vrow['lv001']],(int)$this->ArrView[$lstArr[$i]])),$this->Align($lvTd,(int)$this->ArrView[$lstArr[$i]]));
				else
					$vTemp=str_replace("@02",$this->getvaluelink($lstArr[$i],$this->FormatView($vrow[$lstArr[$i]],(int)$this->ArrView[$lstArr[$i]])),$this->Align($lvTd,(int)$this->ArrView[$lstArr[$i]]));
				$strL=$strL.$vTemp;
			}
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td align=@#05>@02</td>";
		$sqlS = "SELECT * FROM tc_lv0004 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
	...
	function LV_BuilListReportNoLogo($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
