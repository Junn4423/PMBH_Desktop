<?php
/////////////coding sl_lv0102///////////////
class   sl_lv0102 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
////////////
	...
	function LV_getTangMini($vLangArr)
	{
		...
		{
			$vTakeAway='';
			if(strpos(' take a way, cafe mang đi, cafe mang di',strtolower($row['lv002']))>0) $vTakeAway=" takeaway";
	...
	function LV_GetBangMini($vTangID,$vTang,$lvNumRow,&$vautocrease,&$vorder,$vLangArr)
	{
		...
		while($row= db_fetch_array($lvResult))
		{
			...
			if($row['active']!="" && $row['active']!=NULL)
				...
				$vung=$this->LV_GetPTMoney($row['active'])-$this->LV_GetPCMoney($row['active']);
				$vLeft=$vLeft.'
				<div style="width:130px;float:left;height:160px;">
				...
				<div id="bangtitle_'.$vorder.'" class="'.(($active || $checks)?(($vsate==0)?(($vKM==1)?'bangtitlemini':'bangtitlemini_alarm'):'bangtitlewaitmini'):'bangtitlecurmini').'" @#@01><div style="font:10px arial;overflow:hidden;height:12px;padding-top:0px">'.(($active && $this->obj_conf->lv008==1)?'<span id="tongtienconlai1_'.$vorder.'"><strong>'.$this->FormatView($vsum-$vung,10).' đ</strong></span>':'<span id="tongtienconlai1_'.$vorder.'"><strong></strong></span>').'</div><div style="font-size:25px" title="'.$this->FormatView($vCurHD['lv004'],2).' '.substr($vCurHD['lv004'],11,8).'">'.$row['lv002'].'</div><div>'.(($active)?(($checks)?'<span class="bangtime">'.$vHDGop['name'].'</span>':' <span class="bangtime" id="bangtime_'.$vautocrease.'" title="'.$limittime.'">'.$this->sec_to_times($limittime).'</span>'):'<span class="bangtime">'.$vHDGop['name'].'</span>').'</div>
				</div>
				...
				'.$this->LV_GetDetailControlMini($row['active'],$vorder,$vLangArr,$vlistItem,$vCurHD['VAT']).'
					'.(($active && $vsate==0)?'<br><div id="lvloaddata_'.$row['active'].'"></div>':'').'
						...
						<div style="position:absolute;left:0px;bottom:0px;height:25px">
							...
							<div id="xuongbepbarid_'.$vorder.'" class="banhangmini" onclick="tratien(\''.$row['active'].'\',\''.$vorder.'\',5);closepopcalendar(\''.$vorder.'\');"  style="float:left;width:120px">'.'CHẤP NHẬN'.'</div>
							<div id="tratienid_'.$vorder.'" class="banhangmini" onclick="tratien(\''.$row['active'].'\',\''.$vorder.'\',2);closepopcalendar(\''.$vorder.'\');"  style="float:left;">'.$vLangArr[75].'</div>
				...
				<div class="gopban"  id="gopbang_'.$vorder.'" style="display:'.(($active || $checks)?'block':'none').'"><div style="float:left;padding-top:5px">'.(($checks || $active==false)?'':'<input type="button" value="'.$vLangArr[81].'" onclick="ActiveGop(this,\''.$row['active'].'\',\''.$vTang.'\')"/>').'</div><div style="float:left;"><input id="gopbangcheck_'.$vorder.'" type="checkbox" onclick="'.(($checks)?"setDonHang('".$vHDGop['lv001']."')":'').';setgopbang(this,\''.$row['lv001'].'\','.(($active)?1:0).')" '.(($checks)?'checked':'').'/></div></div>
				</div>
				';
		}
		$vLeft=$vLeft.'		
	...
	function LV_GetDetail($vContractID,$vBangID,$vLangArr,&$vSum)
	{
		...
		$vstr='
			<table class="lvtable">
			<tr class="lvhtable"><td class="lvhtable">'.$vLangArr[18].'</td><td style class="lvhtable" style="width:10px">M</td><td style class="lvhtable" style="width:10px">K</td><td  class="lvhtable" style="width:20px">R</td>'.(($this->obj_conf->lv012==1)?'<td class="lvhtable">Xóa</td>':'').'<td class="lvhtable">Tên</td><td class="lvhtable">SLượng</td><td class="lvhtable">Giá<td class="lvhtable">Ggiá(%)</td><td class="lvhtable">Ghi chú</td><td class="lvhtable">Tách Bill</td></tr>
		';		
		$lvsql="select A.*,C.lv002 Name,C.lv009 ColorName from sl_lv0014 A inner join sl_lv0013 B on A.lv002=B.lv001 inner join sl_lv0007 C on A.lv003=C.lv001 where 1=1 and B.lv001='$vContractID'";	
		$vresult=db_query($lvsql);
		$i=1;
		$vSum=0;
		$vSumTT=0;
		...
		while($vrow = db_fetch_array ($vresult))
		{
			...
			$vstr=$vstr.'<tr class="lvlinehtable'.($i%2).(($vrow['lv004']==0)?' textunderline':'').'"  ><td style="color:'.$vrow['ColorName'].'">'.$i.'</td><td style="color:'.$vrow['ColorName'].'">'.$vrow['lv018'].'</td><td style="color:'.$vrow['ColorName'].'">'.$vrow['lv019'].'</td><td style="color:'.$vrow['ColorName'].'">'.$vrow['lv017'].'</td>'.(($this->obj_conf->lv012==1)?'<td style="color:'.$vrow['ColorName'].';text-align:center"><img src="../images/icon/delete.png"  onclick="delline(this,\''.$vrow['lv001'].'\','.$vBangID.')"/></td>':'').'<td style="color:'.$vrow['ColorName'].'">'.$vrow['Name'].'</td><td><input  id="detail_id_'.$vrow['lv001'].'" style="width:30px;text-align:center;" type="textbox" value="'.$vrow['lv004'].'" onblur="changeqty(this,\''.$vrow['lv001'].'\','.$vBangID.')" title="'.$this->obj_conf->lv004.'"/></td><td><input  id="detail_price_'.$vrow['lv001'].'" style="width:60px;text-align:center;" type="textbox" value="'.$vrow['lv006'].'" onblur="changeprice(this,\''.$vrow['lv001'].'\','.$vBangID.')" title="'.$this->obj_conf->lv004.'"/></td><td><input  id="detail_percent_'.$vrow['lv001'].'" style="width:25px;text-align:center;" type="textbox" value="'.$vrow['lv011'].'" onblur="changediscount(this,\''.$vrow['lv001'].'\','.$vBangID.')" title="'.$this->obj_conf->lv004.'"/></td><td><div style="float:left"><input type="textbox" value="'.$vrow['lv022'].'" style="width:150px;text-align:center;background:orange" onblur="changeqtytratruoc(this,\''.$vrow['lv001'].'\','.$vBangID.')" name="tratruoc_'.$vContractID.'_'.$i.'" title="'.$this->FormatView($vrow['lv013'],4).'"/></div><div style="float:right">'.(($vrow['lv010']!="")?'<span title="'.$vrow['lv010'].'"><image src="../images/icon/note_re.png"/></span>':'').(($vrow['lv023']!="")?'<span title="'.$vrow['lv023'].'"><image src="../images/icon/note_ki.png"/></span>':'').'</div></td><td><input  style="width:25px;text-align:center;" type="textbox" value="'.$vrow['lv015'].'" onblur="changeorder(this,\''.$vrow['lv001'].'\','.$vBangID.')"/></td> </tr>';
			$i++;
		}
		$vstr=$vstr.'
