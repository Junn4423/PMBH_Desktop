<?php
/////////////coding sl_lv0103///////////////
class   sl_lv0103 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
////////////
	...
	function LV_GetBangMini($vTangID,$vTang,$lvNumRow,&$vautocrease,&$vorder,$vLangArr)
	{
		...
		while($row= db_fetch_array($lvResult))
		{
			...
			if($row['active']!="" && $row['active']!=NULL)
				...
				<div id="bangtitle_'.$vorder.'" class="'.(($active || $checks)?(($vsate==0)?(($vKI==1)?'bangtitlemini':'bangtitlemini_alarm'):'bangtitlewaitmini'):'bangtitlecurmini').'" @#@01><div style="font:10px arial;overflow:hidden;height:12px;padding-top:0px">'.(($active && $this->obj_conf->lv008==1)?'<span id="tongtienconlai1_'.$vorder.'"><strong>'.$this->FormatView($vsum-$vung,10).' đ</strong></span>':'').'</div><div style="font-size:25px" title="'.$this->FormatView($vCurHD['lv004'],2).' '.substr($vCurHD['lv004'],11,8).'">'.$row['lv002'].'</div><div>'.(($active)?(($checks)?'<span class="bangtime">'.$vHDGop['name'].'</span>':' <span class="bangtime" id="bangtime_'.$vautocrease.'" title="'.$limittime.'">'.$this->sec_to_times($limittime).'</span>'):'<span class="bangtime">'.$vHDGop['name'].'</span>').'</div></div>
				<ul style="clear:both">
					<li>';
					if(($active || $checks))
						$vLeft=$vLeft;
					else
						$this->LV_GetButtonMini($row['lv001'],$vorder,$vLangArr,$vautocrease,$row['active'],$vLeft);
					$vLeft=$vLeft.'
					...
					<div class="liftview_title"><table cellspacing=2 cellpadding=2 width="99%" border="0" style="color:white;">
						...
						<td class="lvhtable" align="center">'.$vLangArr[88].': <span id="tongtienung_'.$vorder.'">'.$this->FormatView($vung,10).'</span> đ</td>
						<td class="lvhtable" style="text-align:right!important;padding-right:10px;font:bold 16px Arial;">'.$vLangArr[89].': <span id="tongtienconlai_'.$vorder.'"><strong>'.$this->FormatView($vsum-$vung,10).'</strong> đ</span>
						</td></tr></table>	</div>
					<div><table cellspacing=2 cellpadding=2 width="99%" border="0">
						<tr>
						<td>'.((trim($vCurHD['CMND'])=='' || $vCurHD['CMND']==NULL)?'Điện thoại/CMND':$vCurHD['CMND']).'
						</td><td>
						'.((trim($vCurHD['CusName'])=='' || $vCurHD['CusName']==NULL)?'Tên khách hàng':$vCurHD['CusName']).'
						</td><td>'.((trim($vCurHD['Address'])=='' || $vCurHD['Address']==NULL)?'Địa chỉ':$vCurHD['Address']).'
						</td><td>'.((trim($vCurHD['Note'])=='' || $vCurHD['Note']==NULL)?'Ghi chú':$vCurHD['Note']).'
						</td></tr></table>	
					</div>
					<!--
					<div style="float:left;width:20%;padding-bottom:15px;">
						<div class="view_title">Đặt phòng</div>
				...
				'.((($active || $checks) && $vsate==0)?$this->LV_GetDetailControlMini($row['active'],$vorder,$vLangArr,$vlistItem,$vCurHD['VAT']):'').'
					'.(($active && $vsate==0)?'<br><div id="lvloaddata_'.$row['active'].'"></div>':'').'
						<p style="position:absolute;right:0px;top:0px" onclick="closepopcalendar(\''.$vorder.'\')"><img width="60" src="images/icon/closefull.png"/></p>
						<div style="position:absolute;left:0px;bottom:0px;height:25px">
							<div class="vitribang" style="width:556px">'.(($this->GetAdd()>0)?'
							<div class="banhangmini" onclick="tratien(\''.$row['active'].'\',\''.$vorder.'\',5);closepopcalendar(\''.$vorder.'\');"  style="float:left;width:120px">'.'ĐÃ XONG'.'</div>':'').'
							<div class="banhangmini" onclick="Report(\''.$row['active'].'\')"  style="float:left;"> '.$vLangArr[79].' </div>
							</div>
						</div>
						</div>
					</div>					
	...
	{
		$lvsql="select lv001,lv026 KM,lv027 KI,lv030 NV,lv006 VAT,IF(lv011=0,lv004,lv005) lv004,TIME_TO_SEC(substr(IF(lv011=0,lv004,lv005),12,8)) timeview,TIME_TO_SEC(TIMEDIFF('24:00:00',substr(IF(lv011=0,lv004,lv005),12,8))) timeagain,TIME_TO_SEC('24:00:00') h24,DATEDIFF(CURRENT_DATE(),substr(IF(lv011=0,lv004,lv005),1,12)) days,TIME_TO_SEC(CURRENT_TIME()) curtime,lv011 state,lv002 CMND,lv003 CusName,lv009 Address,lv013 Note from  sl_lv0013 Where lv001='$vContractID'";
		$vresult=db_query($lvsql);
	...
	{
		$vstr='
			<div id="detailview_'.$vContractID.'" style="padding:10px;background:#fff;height:296px; overflow: auto;">';
	...
	function LV_GetDetailControl($vContractID,$vBangID,$vLangArr,$vlistItem='',$cvat)
	{
		$vstr='<div><h3>'.$vLangArr[16].'<div class="banhang" onclick="tratien(\''.$vContractID.'\',\''.$vBangID.'\',2);"  style="float:left;padding-left:15px;padding-right:15px;">'.$vLangArr[75].'</div><div class="banhang"   style="float:left;padding-left:15px;padding-right:15px;"><input style="padding:0px;margin:0px;height:15px;" type="checkbox" value=1 onclick="CheckVAT(this,\''.$vContractID.'\',\''.$vBangID.'\');"'. (($cvat>0)?'checked':'').'/><font color="white">VAT</font></div></h3></div>
			...
			<div id="detailview_'.$vContractID.'">';
		$vstr=$vstr.$this->LV_GetDetail($vContractID,$vBangID,$vLangArr,$vSum);
		$vstr=$vstr.'</div>';
		return $vstr;
	}
		function LV_GetDetail($vContractID,$vBangID,$vLangArr,&$vSum)
	{
		...
		$vstr='
			<table class="lvtable">
			<tr class="lvhtable"><td class="lvhtable">'.$vLangArr[18].'</td><td style class="lvhtable" style="width:10px">M</td><td style class="lvhtable" style="width:10px">K</td><td  class="lvhtable" style="width:20px">R</td><td class="lvhtable">Giờ</td><td class="lvhtable">Tên</td><td class="lvhtable">Mã</td><td class="lvhtable">SLượng</td><td class="lvhtable">Giá<td class="lvhtable">Ggiá(%)</td><td class="lvhtable"><image src="../images/icon/note_re.png"/></td><td class="lvhtable"><image src="../images/icon/note_all.png"/></td><td class="lvhtable">Ghi chú</td></tr>
		';		
		$lvsql="select A.*,C.lv002 Name,C.lv009 ColorName,A.lv003 ItemID from sl_lv0014 A inner join sl_lv0013 B on A.lv002=B.lv001 inner join sl_lv0007 C on A.lv003=C.lv001 where 1=1 and B.lv001='$vContractID' order by A.lv019 asc,lv014 asc";	
		$vresult=db_query($lvsql);
		$i=1;
		$vSum=0;
		$vSumTT=0;
		...
		while($vrow = db_fetch_array ($vresult))
		{
			...
			$vstr=$vstr.'<tr class="lvlinehtable'.($i%2).(($vrow['lv004']==0)?' textunderline':'').'"  ><td style="color:'.$vrow['ColorName'].'">'.$i.'</td><td style="color:'.$vrow['ColorName'].'">'.$vrow['lv018'].'</td><td style="color:'.$vrow['ColorName'].'">'.(($vrow['lv019']==0 && $this->GetAdd()>0)?'<input type="checkbox" onclick="changestaffreceivefood(this,\''.$vrow['lv001'].'\','.$vBangID.')"/>':$vrow['lv019']).'</td><td style="color:'.$vrow['ColorName'].'">'.$vrow['lv017'].'</td>'.'<td>'.substr($vrow['lv014'],11,8).'</td><td style="color:'.$vrow['ColorName'].'">'.$vrow['Name'].'</td><td style="color:'.$vrow['ColorName'].'">'.$vrow['ItemID'].'</td><td  align=right>'.$vrow['lv004'].'</td><td align=right>'.$this->FormatView($vrow['lv006'],10).'</td><td  align=right>'.$vrow['lv011'].'</td><td>'.$vrow['lv021'].'</td><td>'.$vrow['lv022'].'</td><td><div style="float:left"><input type="textbox" value="'.$vrow['lv023'].'" style="width:150px;text-align:center;background:orange" onblur="changeqtytratruoc(this,\''.$vrow['lv001'].'\','.$vBangID.')" name="tratruoc_'.$vContractID.'_'.$i.'" title="'.$this->FormatView($vrow['lv013'],4).'"/></div></td></td></tr>';
			$i++;
		}
		$vstr=$vstr.'
		<tr class="lvhtable"><td class="lvhtable">&nbsp;'.$vobjtext.'</td><td class="lvhtable">&nbsp;</td><td class="lvhtable">&nbsp;</td><td class="lvhtable">&nbsp;</td><td class="lvhtable">&nbsp;</td><td class="lvhtable">&nbsp;</td><td class="lvhtable">&nbsp;</td><td class="lvhtable">&nbsp;</td><td class="lvhtable" align="right"><span id="tongtienct_'.$vBangID.'">'.$this->FormatView($vSum,10).'</span></td><td  colspan="4"></td></tr>
		</table>
		';
		return $vstr;
	}
	function LV_GetGroupItem($vList)
	{
		$vArrList=explode(",",$vList);
		$vReturn="";
		foreach($vArrList as $cate)
		{
			if($cate!="")
			{
				$vChildGr=$this->LV_GetChildGroupItem($cate);
				if($vChildGr!="") 
				{
					$vReturn=$vReturn.$vChildGr;
				}
			}
		}
		return $vReturn;
	}
	function LV_GetChildGroupItem($vCateID)
	{
		$vReturn="";
		if(trim($vCateID)=="") return '';
		$vReturn="'".str_replace(",","','",$vCateID)."'";
		$vsql="select lv001 from  sl_lv0006 where lv003 ='$vCateID' ";
		$bResult=db_query($vsql);
		while ($vrow = db_fetch_array ($bResult)){
			$vReturn=$vReturn.",".$this->LV_GetChildGroupItem($vrow['lv001']);
		}
		return $vReturn;
	}
	function LV_GetDetailGroup($vLangArr,&$vSum,$vOpt=0)
	{
		$vobjtext="";
		$vstr='
		
			<table class="lvtable">
			<tr class="lvhtable"><td class="lvhtable">'.$vLangArr[18].'</td><td style class="lvhtable" style="width:10px">M</td><td style class="lvhtable" style="width:10px">K</td><td  class="lvhtable" style="width:20px">R</td><td class="lvhtable">Bàn</td><td class="lvhtable">Giờ</td><td class="lvhtable">Tên</td><td class="lvhtable">Mã SP</td><td class="lvhtable">SLượng</td><td class="lvhtable">Giá<td class="lvhtable">Ggiá(%)</td><td class="lvhtable"><image src="../images/icon/note_re.png"/></td><td class="lvhtable"><image src="../images/icon/note_all.png"/></td><td class="lvhtable">Ghi chú</td></tr>
		';
		$vCondition="";
		$vListCate=$this->LV_GetGroupItem($this->nhommon);
		if($vListCate!="") $vCondition=" AND C.lv003 in (".$vListCate.")";
		if($this->obj_conf->lv026==1) $vCondition=$vCondition." and A.lv018=1 ";
		if($vOpt==0)
		{
		switch($this->viewshow)
		{
			case 1:
				$lvsql="select A.*,B.lv007 bangid,A.lv003 ItemID,C.lv002 Name,C.lv009 ColorName from sl_lv0014 A inner join sl_lv0013 B on A.lv002=B.lv001 inner join sl_lv0007 C on A.lv003=C.lv001 where A.lv019<>1 $vCondition and B.lv011=0 and A.lv004>0 order by A.lv019,A.lv003,A.lv014";	
				break;
			default:
				$lvsql="
				SELECT MP.* from 
				(
					select 0 DaXoa,A.lv103,TIME_TO_SEC(TIMEDIFF(NOW(),A.lv014)) TimeExpire,A.lv001,A.lv002,A.lv003,A.lv004,A.lv005,A.lv006,A.lv007,A.lv008,A.lv009,A.lv010,A.lv011,A.lv012,A.lv014,A.lv020,A.lv021,A.lv022,A.lv023,B.lv007 bangid,A.lv003 ItemID,C.lv002 Name,C.lv009 ColorName from sl_lv0014 A inner join sl_lv0013 B on A.lv002=B.lv001 inner join sl_lv0007 C on A.lv003=C.lv001 where A.lv019<>1  $vCondition and B.lv011=0 and A.lv004>0 
				UNION
					select 1 DaXoa,A.lv103,TIME_TO_SEC(TIMEDIFF(NOW(),A.lv014)) TimeExpire,A.lv001,A.lv002,A.lv003,A.lv004,A.lv005,A.lv006,A.lv007,A.lv008,A.lv009,A.lv010,A.lv011,A.lv012,A.lv014,A.lv020,A.lv021,A.lv022,A.lv023,B.lv007 bangid,A.lv003 ItemID,C.lv002 Name,C.lv009 ColorName from sl_lv0014_1 A inner join sl_lv0013 B on A.lv002=B.lv001 inner join sl_lv0007 C on A.lv003=C.lv001 where A.lv019<>1  $vCondition and B.lv011=0 and A.lv004>0
				) MP
				order by MP.lv014,MP.lv003
				";	
				break;
		}
		$vresult=db_query($lvsql);
		...
		$vCodeID="";
		$vArrItemSort=Array();
		if($this->obj_conf->lv019==0) $this->obj_conf->lv019=5;
		if($this->obj_conf->lv020==0) $this->obj_conf->lv020=15;
		while($vrow = db_fetch_array ($vresult))
		{
			$vDaXoa=$vrow['DaXoa'];
			$vTimeExpire=$vrow['TimeExpire']/60;
			
			$vCheckMore=(int)($vTimeExpire/$this->obj_conf->lv019);
			$vAlarmStyle='';
			$vTextMsg='';
			if($vTimeExpire>=$this->obj_conf->lv020)
			{
				$vTextMsg="Đã quá ".((int)$vTimeExpire)." phút";
			...
			if($vrow['lv103']>0)
			{
				if($vrow['lv103']!=$vrow['lv004'])
				{
					$vTextMsg=$vTextMsg."Đã thay đổi số lượng từ [".$vrow['lv103']."] thành [".$vrow['lv004']."]";
					db_query("update sl_lv0014 set lv104=lv104+1 where lv001='".$vrow['lv001']."'");
					db_query("update sl_lv0014 set lv103=lv004 where lv001='".$vrow['lv001']."' and lv104>=2");
					$vAlarmStyle="-webkit-animation: glowing 1500ms infinite;-moz-animation: glowing 1500ms infinite;-o-animation: glowing 1500ms infinite;animation: glowing 1500ms infinite;";
				}
			...
			switch($this->viewshow)
			{
				...
				if($vItem!=$vrow['lv003'])
				{
					...
					{
						$vstr=$vstr.'
							<tr class="lvhtable"><td class="lvhtable" colspan="4">&nbsp;<!--<input type="button" value="Xong tất cả" onclick="changestaffreceivefoodall(\''.$vCodeID.'\')"/>--></td><td class="lvhtable">&nbsp;</td><td class="lvhtable">&nbsp;</td><td class="lvhtable">&nbsp;</td><td class="lvhtable">&nbsp;</td><td class="lvhtable">'.$this->FormatView($vSumQty,10).'</td><td class="lvhtable" align="right"><span >'.$this->FormatView($vSumG,10).'</span></td><td  colspan="4"></td></tr>
							';
					}
					$vSumQty=0;
					$vSumG=0;
				...
				$vSumG=$vSumG+$vrow['lv004']*$vrow['lv006']-$vrow['lv004']*$vrow['lv006']*$vrow['lv011']/100;
				$vSum=$vSum+$vrow['lv004']*$vrow['lv006']-$vrow['lv004']*$vrow['lv006']*$vrow['lv011']/100;
				//$vSumTT=$vSumTT+$vrow['lv017']*$vrow['lv006'];
				$vstr=$vstr.'<tr class="lvlinehtable'.($i%2).(($vrow['lv004']==0)?' textunderline':'').'"  ><td style="color:'.$vrow['ColorName'].'">'.$i.'</td><td style="color:'.$vrow['ColorName'].'">'.$vrow['lv018'].'</td><td style="color:'.$vrow['ColorName'].'">'.(($vrow['lv019']==0 && $this->GetAdd()>0)?'<input type="checkbox" onclick="changestaffreceivefood(this,\''.$vrow['lv001'].'\','.$vBangID.')"/>':$vrow['lv019']).'</td><td style="color:'.$vrow['ColorName'].'">'.$vrow['lv017'].'</td>'.'<td>'.$this->getvaluelink('lv007',$vrow['bangid']).'</td>'.'<td>'.substr($vrow['lv014'],11,8).'</td><td style="color:'.$vrow['ColorName'].'">'.$vrow['Name'].'</td><td style="color:'.$vrow['ColorName'].'">'.$vrow['ItemID'].'</td><td  align=right>'.$vrow['lv004'].'</td><td align=right>'.$this->FormatView($vrow['lv006'],10).'</td><td  align=right>'.$vrow['lv011'].'</td><td>'.$vrow['lv021'].'</td><td>'.$vrow['lv022'].'</td><td><div style="float:left"><input type="textbox" value="'.$vrow['lv023'].'" style="width:150px;text-align:center;background:orange" onblur="changeqtytratruoc(this,\''.$vrow['lv001'].'\','.$vBangID.')" name="tratruoc_'.$vContractID.'_'.$i.'" title="'.$this->FormatView($vrow['lv013'],4).'"/></div></td></td></tr>';
				$i++;
				break;
			default:
				...
				{
					$vFunc="DAXOA";
					$vStyle=' style="background:red;color:#fff;" ';
					$vAlarmStyle='';
				}
				else
				{
				...
				$vrow['lv003']=$vrow['lv003'].$vCheckMore;
				if($vArrItemSort[$vrow['lv003']][0]==true)
				{
					$vBangID=$vrow['bangid'];
					$vArrItemSort[$vrow['lv003']][1]=$vArrItemSort[$vrow['lv003']][1].",".$vrow['lv001'];
					$vArrItemSort[$vrow['lv003']][3]=$vArrItemSort[$vrow['lv003']][3]+$vrow['lv004'];
					$vArrItemSort[$vrow['lv003']][4]=$vArrItemSort[$vrow['lv003']][4]+$vrow['lv004']*$vrow['lv006']-$vrow['lv004']*$vrow['lv006']*$vrow['lv011']/100;
					$vSum=$vSum+$vrow['lv004']*$vrow['lv006']-$vrow['lv004']*$vrow['lv006']*$vrow['lv011']/100;
					$vArrItemSort[$vrow['lv003']][2]=$vArrItemSort[$vrow['lv003']][2].'<tr class="lvlinehtable'.($i%2).(($vrow['lv004']==0)?' textunderline':'').'"  style="'.$vAlarmStyle.'" ><td  '.$vStyle.'>'.$i.'</td><td '.$vStyle.'>'.$vrow['lv018'].'</td><td '.$vStyle.'>'.(($vrow['lv019']==0 && $this->GetAdd()>0)?'<input style="width:30px" type="checkbox" onclick="changestaffreceivefood'.$vFunc.'(this,\''.$vrow['lv001'].'\','.$vBangID.')"/>':$vrow['lv019']).'</td><td  '.$vStyle.'><input type="button" value="Xong" onclick="changestaffreceivefoodall(\''.$vrow['lv001'].'\')"/>'.$vrow['lv017'].'</td>'.'<td '.$vStyle.'>'.$this->getvaluelink('lv007',$vrow['bangid']).'</td>'.'<td  '.$vStyle.'>'.substr($vrow['lv014'],11,8).'</td><td  '.$vStyle.'>'.$vrow['Name'].'</td><td  '.$vStyle.'">'.$vrow['ItemID'].'</td><td  align=center '.$vStyle.'>'.$vrow['lv004'].'</td><td align=right '.$vStyle.'>'.$this->FormatView($vrow['lv006'],10).'</td><td  align=right '.$vStyle.'>'.$vrow['lv011'].'</td><td '.$vStyle.'>'.$vrow['lv021'].$vTextMsg.'</td><td '.$vStyle.'>'.$vrow['lv022'].'</td><td '.$vStyle.'><div style="float:left"><input type="textbox" value="'.$vrow['lv023'].'" style="width:150px;text-align:center;background:orange" onblur="changeqtytratruoc(this,\''.$vrow['lv001'].'\','.$vBangID.')" name="tratruoc_'.$vContractID.'_'.$i.'" title="'.$this->FormatView($vrow['lv013'],4).'"/></div></td></td></tr>';
					
				}
				else
				{
					$vArrItemSort[$vrow['lv003']][0]=true;
					$vBangID=$vrow['bangid'];
					$vArrItemSort[$vrow['lv003']][1]=$vrow['lv001'];
					$vArrItemSort[$vrow['lv003']][3]=$vrow['lv004'];
					$vArrItemSort[$vrow['lv003']][4]=$vrow['lv004']*$vrow['lv006']-$vrow['lv004']*$vrow['lv006']*$vrow['lv011']/100;
					$vSum=$vSum+$vrow['lv004']*$vrow['lv006']-$vrow['lv004']*$vrow['lv006']*$vrow['lv011']/100;
					$vArrItemSort[$vrow['lv003']][2]='<tr class="lvlinehtable'.($i%2).(($vrow['lv004']==0)?' textunderline':'').'"  style="'.$vAlarmStyle.'" ><td  '.$vStyle.'>'.$i.'</td><td '.$vStyle.'>'.$vrow['lv018'].'</td><td '.$vStyle.'>'.(($vrow['lv019']==0 && $this->GetAdd()>0)?'<input type="checkbox"  style="width:30px" onclick="changestaffreceivefood'.$vFunc.'(this,\''.$vrow['lv001'].'\','.$vBangID.')"/>':$vrow['lv019']).'</td><td  '.$vStyle.'><input type="button" value="Xong" onclick="changestaffreceivefoodall(\''.$vrow['lv001'].'\')"/>'.$vrow['lv017'].'</td>'.'<td '.$vStyle.'>'.$this->getvaluelink('lv007',$vrow['bangid']).'</td>'.'<td  '.$vStyle.'>'.substr($vrow['lv014'],11,8).'</td><td  '.$vStyle.'>'.$vrow['Name'].'</td><td  '.$vStyle.'">'.$vrow['ItemID'].'</td><td  align=center '.$vStyle.'>'.$vrow['lv004'].'</td><td align=right '.$vStyle.'>'.$this->FormatView($vrow['lv006'],10).'</td><td  align=right '.$vStyle.'>'.$vrow['lv011'].'</td><td '.$vStyle.'>'.$vrow['lv021'].$vTextMsg.'</td><td '.$vStyle.'>'.$vrow['lv022'].'</td><td '.$vStyle.'><div style="float:left"><input type="textbox" value="'.$vrow['lv023'].'" style="width:150px;text-align:center;background:orange" onblur="changeqtytratruoc(this,\''.$vrow['lv001'].'\','.$vBangID.')" name="tratruoc_'.$vContractID.'_'.$i.'" title="'.$this->FormatView($vrow['lv013'],4).'"/></div></td></td></tr>';
				}
		...
		$vSum=0;
		$vSumQty=0;
		foreach($vArrItemSort as $vItemRun)
		{
			$vSumQty=$vSumQty+$vItemRun[3];
			$vSum=$vSum+$vItemRun[4];
			$vstr=$vstr.$vItemRun[2];
			$vstr=$vstr.'<tr class="lvhtable"><td class="lvhtable" colspan="4">&nbsp;<!--<input type="button" value="Xong tất cả" onclick="changestaffreceivefoodall(\''.$vItemRun[1].'\')"/>--></td><td class="lvhtable">&nbsp;</td><td class="lvhtable">&nbsp;</td><td class="lvhtable">&nbsp;</td><td class="lvhtable">&nbsp;</td><td class="lvhtable">'.$this->FormatView($vItemRun[3],10).'</td><td class="lvhtable" align="right"><span >'.$this->FormatView($vItemRun[4],10).'</span></td><td  colspan="4"></td></tr>							';
		}
		}
		else
		{
		...
		{
			case 1:
				$lvsql="select A.*,B.lv007 bangid,C.lv002 Name,C.lv009 ColorName,A.lv003 ItemID from sl_lv0014 A inner join sl_lv0013 B on A.lv002=B.lv001 inner join sl_lv0007 C on A.lv003=C.lv001 where A.lv019=1 $vCondition and B.lv001 in (select BB.lv001 from sl_lv0013 BB where BB.lv011=0) order by A.lv019,A.lv003,A.lv014";	
				break;
			default:
				$lvsql="
				SELECT MP.* from 
				(
					select 0 DaXoa,A.lv001,A.lv002,A.lv003,A.lv004,A.lv005,A.lv006,A.lv007,A.lv008,A.lv009,A.lv010,A.lv011,A.lv012,A.lv014,A.lv020,A.lv021,A.lv022,A.lv023,B.lv007 bangid,C.lv002 Name,C.lv009 ColorName,A.lv003 ItemID from sl_lv0014 A inner join sl_lv0013 B on A.lv002=B.lv001 inner join sl_lv0007 C on A.lv003=C.lv001 where A.lv019=1 $vCondition and B.lv011=0 and A.lv004>0	 
				UNION
					select 1 DaXoa,A.lv001,A.lv002,A.lv003,A.lv004,A.lv005,A.lv006,A.lv007,A.lv008,A.lv009,A.lv010,A.lv011,A.lv012,A.lv014,A.lv020,A.lv021,A.lv022,A.lv023,B.lv007 bangid,C.lv002 Name,C.lv009 ColorName,A.lv003 ItemID from sl_lv0014_1 A inner join sl_lv0013 B on A.lv002=B.lv001 inner join sl_lv0007 C on A.lv003=C.lv001 where A.lv019=1 $vCondition and B.lv011=0 and A.lv004>0
				) MP
				order by MP.DaXoa,MP.lv014,MP.lv003
				";	
				//$lvsql="select A.*,B.lv007 bangid,C.lv002 Name,C.lv009 ColorName,A.lv003 ItemID from sl_lv0014 A inner join sl_lv0013 B on A.lv002=B.lv001 inner join sl_lv0007 C on A.lv003=C.lv001 where A.lv019=1 and B.lv001 in (select BB.lv001 from sl_lv0013 BB where BB.lv011=0) order by A.lv003,A.lv014";	
				break;
		}
		$vresult=db_query($lvsql);
		$i=1;
			...
			switch($this->viewshow)
			{
				case 1:
					...
					if($vItem!=$vrow['lv003'])
					{
						...
						{
							$vstr=$vstr.'
								<tr class="lvhtable"><td class="lvhtable" colspan="4">&nbsp;</td><td class="lvhtable">&nbsp;</td><td class="lvhtable">&nbsp;</td><td class="lvhtable">&nbsp;</td><td class="lvhtable">'.$this->FormatView($vSumQty,10).'</td><td class="lvhtable" align="right"><span >'.$this->FormatView($vSumG,10).'</span></td><td  colspan="4"></td></tr>
								';
						}
						$vSumQty=0;
						$vSumG=0;
			...
			default:
				...
				else
				{
					...
					if($vItem!=$vrow['lv003'])
					{
						...
						{
							$vstr=$vstr.'
								<tr class="lvhtable"><td class="lvhtable" colspan="5">&nbsp;</td><td class="lvhtable">&nbsp;</td><td class="lvhtable">&nbsp;</td><td class="lvhtable">&nbsp;</td><td class="lvhtable">'.$this->FormatView($vSumQty,10).'</td><td class="lvhtable" align="right"><span >'.$this->FormatView($vSumG,10).'</span></td><td  colspan="4"></td></tr>
								';
						}
						$vSumQty=0;
						$vSumG=0;
					...
					$vSum=$vSum+$vrow['lv004']*$vrow['lv006']-$vrow['lv004']*$vrow['lv006']*$vrow['lv011']/100;
					//$vSumTT=$vSumTT+$vrow['lv017']*$vrow['lv006'];
					$vstr=$vstr.'<tr class="lvlinehtable'.($i%2).(($vrow['lv004']==0)?' textunderline':'').'"  ><td style="color:'.$vrow['ColorName'].'">'.$i.'</td><td style="color:'.$vrow['ColorName'].'">'.$vrow['lv018'].'</td><td style="color:'.$vrow['ColorName'].'">'.(($vrow['lv019']==0 && $this->GetAdd()>0)?'<input type="checkbox" onclick="changestaffreceivefood(this,\''.$vrow['lv001'].'\','.$vBangID.')"/>':$vrow['lv019']).'</td><td style="color:'.$vrow['ColorName'].'">'.$vrow['lv017'].'</td>'.'<td>'.$this->getvaluelink('lv007',$vrow['bangid']).'</td>'.'<td>'.substr($vrow['lv014'],11,8).'</td><td style="color:'.$vrow['ColorName'].'">'.$vrow['Name'].'</td><td style="color:'.$vrow['ColorName'].'">'.$vrow['ItemID'].'</td><td  align=right '.$vStyle.'>'.$vrow['lv004'].'</td><td align=right>'.$this->FormatView($vrow['lv006'],10).'</td><td  align=right>'.$vrow['lv011'].'</td><td>'.$vrow['lv021'].'</td><td>'.$vrow['lv022'].'</td><td><div style="float:left"><input type="textbox" value="'.$vrow['lv023'].'" style="width:150px;text-align:center;background:orange" onblur="changeqtytratruoc(this,\''.$vrow['lv001'].'\','.$vBangID.')" name="tratruoc_'.$vContractID.'_'.$i.'" title="'.$this->FormatView($vrow['lv013'],4).'"/></div></td></td></tr>';
					$i++;
					break;
			}
		...
		$vstr=$vstr.'
		<tr class="lvhtable"><td class="lvhtable" colspan="4">&nbsp;</td><td class="lvhtable">&nbsp;</td><td class="lvhtable">&nbsp;</td><td class="lvhtable">&nbsp;</td><td class="lvhtable">&nbsp;</td><td class="lvhtable">'.$this->FormatView($vSumQty,10).'</td><td class="lvhtable" align="right"><span id="tongtienct_'.$vBangID.'">'.$this->FormatView($vSum,10).'</span></td><td  colspan="4"></td></tr>';
		$vstr=$vstr.'
		</table>
		';
		return $vstr;
	}
	function LV_GetButtonMini($vRoomID,$vorder,$vLangArr,$vautocrease,$vDHID,&$strLeft)
	{
		$vstr='';
		$vlistID="";
		...
		while($vrow = db_fetch_array ($vresult))
		{
			if(strpos($vlistID,",".$vrow['id'].",")===false)
			{
				if($vrow['Nums']==0)
				{
					$vstr=$vstr.'<div class="bt_setdefaultmini" onclick="SetDefData(\''.$vrow['id'].'\',\''.$vRoomID.'\')" >'.$vrow['name'].'</div>';
				}
				else
				{
					
					$vstr=$vstr.'<div class="bt_setdefaultmini defaultred" onclick="SetDefData(\''.$vrow['id'].'\',\''.$vRoomID.'\')">'.$vrow['name'].'</div>';
				}
			}
