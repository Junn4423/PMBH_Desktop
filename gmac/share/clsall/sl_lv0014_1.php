<?php
/////////////coding sl_lv0014_1///////////////
class   sl_lv0014_1 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
////////////////////GetDate
	...
	protected $objhelp='sl_lv0014_11';
////////////
	var $ArrOther=array();
	var $ArrPush=array();
	var $ArrFunc=array();
	...
	{
		$lvsql="select * from  sl_lv0014_1 Where lv001='$vlv001'";
	...
	{
		$lvsql="select A.lv001,A.lv004 from sl_lv0060 A inner join sl_lv0059 B on B.lv001=A.lv002 where A.lv003='$vItem' and B.lv001='$vProgId' and B.lv008=1";
		$vReturn= db_query($lvsql);
		$vrow=db_fetch_array($vReturn);
		if($vrow)
			return $vrow['lv004'];
	...
	{
		$lvsql="select lv001 from  sl_lv0014_1 Where lv002='$vContracID' and lv003='$vItemID'";
		$vresult=db_query($lvsql);
		$vrow=db_fetch_array($vresult);
		if($vrow)
		{
	...
	{
		$lvsql="delete from sl_lv0014_1 where lv002='$vTemID' and lv003 in (select lv004 from sl_lv0072 A  where A.lv002='$vRoomID' )";
		//$lvsql="delete from sl_lv0014_1 where lv002='$vTemID' and lv003 in (select lv004 from sl_lv0072 A  where A.lv003<>'$vlv002' and A.lv002='$vRoomID' )";
		$vresult=db_query($lvsql);
		return ;
	}
	...
	function LV_InsertOther($vlv002,$vreplace)
	{
		...
		if($this->isAdd==0) return false;
		 $lvsql="insert into sl_lv0014_1 (lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv011,lv012) select '$vreplace' ,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv011,lv012 from sl_lv0011 where lv002='$vlv002'";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'sl_lv0014_1.InsertOther',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_Update()
	{
		...
		$lvsql="Update sl_lv0014_1 set lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004',lv005='$this->lv005',lv006='$this->lv006',lv007='$this->lv007',lv008='$this->lv008',lv009='$this->lv009',lv010='$this->lv010',lv011='$this->lv011',lv012='$this->lv012',lv013='$this->lv013',lv014='$this->lv014',lv015='$this->lv015',lv016='$this->lv016' where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'sl_lv0014_1.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_UpdateCalc($vDetailID)
	{
		$vsql="select B.lv006,A.lv002 ContractID,C.lv007 RoomID from sl_lv0014_1 A inner join sl_lv0007 B on A.lv003=B.lv001 left join sl_lv0013 C on C.lv001=A.lv002 where A.lv001='".$vDetailID."' and (select AA.lv011 from sl_lv0013 AA where AA.lv001=A.lv002)<=0";
		$vresult=db_query($vsql);	
		$vrow = db_fetch_array ($vresult);
		if($vrow)
		{
			...
			$lvsql="Update sl_lv0014_1 set lv004='$vValue' where  lv001='$vDetailID';";
			$vReturn= db_query($lvsql);
			return $vValue;
		}
		return 0;
	...
	{
		$vsql="select B.lv006,A.lv002 ContractID from sl_lv0014_1 A inner join sl_lv0007 B on A.lv003=B.lv001 where A.lv001='".$vDetailID."' and (select AA.lv011 from sl_lv0013 AA where AA.lv001=A.lv002)<=0";
		$vresult=db_query($vsql);	
		$vrow = db_fetch_array ($vresult);
		if($vrow)
		{
			...
			if($vrow['lv006']!="" && $vrow['lv006']!=NULL)
				...
				$vValue=$vQty;
			$lvsql="Update sl_lv0014_1 set lv004='$vValue' where  lv001='$vDetailID' and (select A.lv011 from sl_lv0013 A where A.lv001=sl_lv0014_1.lv002)<=0;";
			$vReturn= db_query($lvsql);
			return $vValue;
		}
		return 0;
	...
	function LV_Delete($lvarr)
	{
		...
		$lvsql1 = "insert into sl_lv0014_1_ (lv024,lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv011,lv012,lv013,lv014,lv015,lv016,lv017,lv018,lv019,lv020,lv021)   (select lv001,lv002,lv002,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv011,lv012,lv013,lv014,lv015,lv016,lv017,lv018,lv019,lv020,lv021 from sl_lv0014_1 where  lv001 IN ($lvarr) and (select B.lv011 from sl_lv0013 B where  B.lv001= sl_lv0014_1.lv002)<=0)  ";
		$vReturn= db_query($lvsql1);
		{
			$lvsql = "DELETE FROM sl_lv0014_1  WHERE sl_lv0014_1.lv001 IN ($lvarr) and (select B.lv011 from sl_lv0013 B where  B.lv001= sl_lv0014_1.lv002)<=0  ";
			$vReturn= db_query($lvsql);
			if($vReturn) $this->InsertLogOperation($this->DateCurrent,'sl_lv0014_1.delete',sof_escape_string($lvsql));
		}
		return $vReturn;
	}	
	...
	{
		if($this->isDel==0) return false;
		$lvsql1 = "insert into sl_lv0014_1_ (lv024,lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv011,lv012,lv013,lv014,lv015,lv016,lv017,lv018,lv019,lv020,lv021)   (select lv001,lv002,lv002,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv011,lv012,lv013,lv014,lv015,lv016,lv017,lv018,lv019,lv020,lv021 from sl_lv0014_1 where  lv018=0 And  lv001 IN ($lvarr) and (select B.lv011 from sl_lv0013 B where  B.lv001= sl_lv0014_1.lv002)<=0)  ";
		$vReturn= db_query($lvsql1);
		{
			$lvsql = "DELETE FROM sl_lv0014_1  WHERE sl_lv0014_1.lv001 And lv018=0 IN ($lvarr) and (select B.lv011 from sl_lv0013 B where  B.lv001= sl_lv0014_1.lv002)<=0  ";
			$vReturn= db_query($lvsql);
			if($vReturn) $this->InsertLogOperation($this->DateCurrent,'sl_lv0014_1.delete',sof_escape_string($lvsql));
		}
		return $vReturn;
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM sl_lv0014_1 WHERE 1=1 ".$this->GetCondition();
...
//////////////////////Buil list////////////////////
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT * FROM sl_lv0014_1 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('".$this->Dir."sl_lv0014_1/?lang=".$this->lang."&childdetailfunc='+value+'&ID=".base64_encode($this->lv002)."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$sqlS = "SELECT * FROM sl_lv0014_1 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
