<?php
/////////////coding mn_lv0005///////////////
class   mn_lv0005 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
///////////
	public $DefaultFieldList="lv001,lv002,lv003";	
////////////////////GetDate
	...
	public $paging=null;
	public $lang=null;
	protected $objhelp='mn_lv0005';
////////////
	var $ArrOther=array();
	var $ArrPush=array();
	var $ArrFunc=array();
	var $ArrGet=array("lv001"=>"2","lv002"=>"3","lv003"=>"4");
	var $ArrView=array("lv001"=>"0","lv002"=>"0","lv003"=>"2");	
	...
	{
		$lvsql="select * from  mn_lv0005 Where lv001='$vlv001'";
	...
	{		
		...
		$lvsql="insert into mn_lv0005 (lv001,lv002,lv003) values('$this->lv001','$this->lv002','$this->lv003')";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'mn_lv0005.insert',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_Update()
	{
		...
		$this->lv003 = ($this->lv003!="")?recoverdate(($this->lv003), $this->lang):$this->DateDefault;
		  $lvsql="Update mn_lv0005 set lv002='$this->lv002' where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'mn_lv0005.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Delete($lvarr)
	{
		...
		$this->lv003 = ($this->lv003!="")?recoverdate(($this->lv003), $this->lang):$this->DateDefault;
		$lvsql = "DELETE FROM mn_lv0005  WHERE mn_lv0005.lv001 IN ($lvarr)";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'mn_lv0005.delete',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_DeleteOrther($lvarr)
	{
		if($this->isDel==0) return false;
		$lvsql = "DELETE FROM mn_lv0005  WHERE mn_lv0005.lv002 IN ($lvarr) and (select lv007 from wh_lv0004 B where  B.lv001= mn_lv0005.lv002)<=0  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'mn_lv0005.delete',sof_escape_string($lvsql));
		return $vReturn;
	}
	...
	{
		$lvsql="select A.lv001 CodeIn,B.* from  mn_lv0005 A inner join sl_lv0007 B on A.lv002=B.lv001 Where A.lv001 in ($vLItemID)";
		$vresult=db_query($lvsql);
		$i=0;
		$vOrder=1;
		...
		while($vrow=db_fetch_array($vresult))
		{
			...
			$i++;
			$vcode=$vrow['CodeIn'];
			//echo file_get_contents("http://localhost/levinhsoft/khobenthanh/server/qrcode/index.php?data=".$vcode);
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM mn_lv0005 WHERE 1=1 ".$this->GetCondition();
...
//////////////////////Buil list////////////////////
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT * FROM mn_lv0005 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('".$this->Dir."mn_lv0005/?lang=".$this->lang."&childdetailfunc='+value+'&ID=".base64_encode($this->lv002)."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
