<?php
/////////////coding sl_lv0013///////////////
class   sl_lv0038 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
////////////
	...
	var $ArrView=array("lv001"=>"0","lv002"=>"0","lv003"=>"0","lv004"=>"2","lv005"=>"2","lv006"=>"0","lv007"=>"0","lv008"=>"0","lv009"=>"0","lv010"=>"0","lv011"=>"0","lv012"=>"0","lv013"=>"0","lv014"=>"0","lv015"=>"10","lv016"=>"0","lv017"=>"0","lv018"=>"1");	
	...
	function LV_Aproval($lvarr)
	{
		...
		$lvsql = "Update sl_lv0013 set lv011=1  WHERE sl_lv0013.lv001 IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
	...
	{
		if($this->isApr==0) return false;
		$lvsql = "Update sl_lv0013 set lv011=0  WHERE sl_lv0013.lv001 IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
	...
	{
		$lvsql="select sum(PM.lv003) money,sum(PM.lv004) convertmoney from ((select sum(A.lv004*A.lv006) lv003,sum(A.lv004*A.lv006*A.lv008/100) lv004 from sl_lv0014 A inner join sl_lv0013 B on A.lv002=B.lv001  where 1=1 and B.lv001='$vContractID' AND B.lv011=1 )) PM ";	
		$vresult=db_query($lvsql);
		$vrow = db_fetch_array ($vresult);
	...
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		while ($vrow = db_fetch_array ($bResult)){
			...
			for($i=0;$i<count($lstArr);$i++)
			{
				if($lstArr[$i]=="lv018")
				{
					$vTemp=str_replace("@02",str_replace("@02",$this->getvaluelink($lstArr[$i],$this->FormatView($this->LV_GetContractMoney($vrow['lv001']),(int)$this->ArrView[$lstArr[$i]])),str_replace("@01",$vrow['lv001'] ,$lvHref)),$this->Align($lvTd,(int)$this->ArrView[$lstArr[$i]]));
				}
				else
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('".$this->Dir."sl_lv0038/?lang=".$this->lang."&childfunc='+value+'&ID=".base64_encode($this->lv002)."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
