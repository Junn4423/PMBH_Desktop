<?php
/////////////coding ml_lv0014///////////////
class   ml_lv0014 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
////////////
	...
	{
		$vsql="select (select count(*) from ml_lv0001 where lv002=1 and lv012=0 and lv011=1 and lv017 in (select ml_lv0008.lv003 from ml_lv0008 where ml_lv0008.lv002='$vlv003')) Inbox,(select count(*) from ml_lv0014 where lv002=2 and lv012=0  and lv011=1 and lv003='$vlv003') Outbox,(select count(*) from ml_lv0014 where lv002=1 and lv012=1  and lv011=1 and lv017 in (select ml_lv0008.lv003 from ml_lv0008 where ml_lv0008.lv002='$vlv003')) DeleteItem,(select count(*) from ml_lv0014 where lv002=2 and lv012=1 and lv011=1 and lv003='$vlv003') SendItem ";
	...
	{
		$lvsql="select * from  ml_lv0001 Where lv001='$vlv001'";
	...
	function LV_Aproval($lvarr)
	{
		...
		$lvsql = "select A.lv001,A.lv002,A.lv003,A.lv004,A.lv005,A.lv006,A.lv007,A.lv008,A.lv009,A.lv010,A.lv011,A.lv012,A.lv013,A.lv014,B.lv005 StateSend from ml_lv0001 A left join ml_lv0008 B on A.lv003=B.lv002 and A.lv006=B.lv003 WHERE A.lv001 IN ($lvarr) and A.lv018=1  ";
		$vReturn= db_query($lvsql);
		if(!$vReturn) return;
		...
		while ($vrow = db_fetch_array ($vReturn)){
			$lvml_lv0008->LV_LoadUser($vrow['lv003'],$vrow['lv006']);
			$lvml_lv0007->LV_LoadUser($vrow['lv003'],$vrow['lv006']);
			$this->lv001=$vrow['lv001'];
			$this->lv002=$vrow['lv002'];
			$this->lv003=$vrow['lv003'];
			$this->lv004=$vrow['lv004'];
			...
			$this->lv019='';
			/*----------------------*/
			$vstrTo=SplitTo(str_replace(";",",",str_replace(" ","",$vrow['lv007'])),"<",">",",");
			$vstrToSend=$this->SplitToEsc($vstrTo,",",0);
			$vstrCC=SplitTo(str_replace(";",",",str_replace(" ","",$vrow['lv008'])),"<",">",",");
			$vstrCCSend=$this->SplitToEsc($vstrCC,",",0);
			$vstrBCC=SplitTo(str_replace(";",",",str_replace(" ","",$vrow['lv016'])),"<",">",",");
			$vstrBCCSend=$this->SplitToEsc($vstrBCC,",",0);
			$lvml_lv0100=new ml_lv0100($_SESSION['ERPSOFV2RRight'],$_SESSION['ERPSOFV2RUserID'],'Ml0100');
			...
			if($vrow['StateSend']==1)
			{
				if(trim($vstrToSend)!="" || trim($vstrCCSend)!="" || trim($vstrBCCSend)!="")				{
					...
					$lvml_lv0100->To(explode(",",$vstrToSend));
					$lvml_lv0100->From($vrow['lv006']);
					$lvml_lv0100->Subject($vrow['lv005']);
					...
					$lvml_lv0100->Body($vrow['lv009'],'');
					if($lvml_lv0100->Send())
					{
						$this->SendLocal($this->SplitToEsc($vrow['lv007'],",",1),$this->SplitToEsc($vrow['lv008'],",",1),$this->SplitToEsc($this->lv016,",",1),1,1,0);
					$this->LV_UpdateDel();
					}
					else
					echo 'khong gui duoc';
			...
			/*------------end---------------------*/
			$vsql = "Update ml_lv0001 set lv018=0  WHERE ml_lv0001.lv001 IN ('".$vrow['lv001']."')  ";
	...
	protected function GetCondition()
	{
		...
		if($this->lv019!="") $strCondi=$strCondi." and lv019='$this->lv019'";
		return $strCondi;
	}
		////////////////Count///////////////////////////
	function GetCount()
	...
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		";
		$lvHref="<a href=\"javascript:FunctRunning1('@01',@#05)\" style=\"text-decoration:none\" class=@#04>@02</a>";
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td  class=@#04 id=@#05>@02</td>";
		$sqlS = "SELECT lv001,lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv011,lv012,lv013,lv014,lv016 FROM ml_lv0001 WHERE 1=1  ".$this->GetCondition()." $strSort,lv014 DESC LIMIT $curRow, $maxRows";
		$vorder=$curRow;
	...
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('".$this->Dir."ml_lv0014/?lang=".$this->lang."&func='+value+'&ID=".base64_encode($this->lv002)."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$sqlS = "SELECT lv001,lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv011,lv012,lv013,lv014,lv016 FROM ml_lv0001 WHERE 1=1  ".$this->GetCondition()." ORDER BY lv001 DESC,lv014 DESC LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
