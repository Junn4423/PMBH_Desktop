<?php
/////////////coding ac_lv0019///////////////
class   ac_lv0019 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
	...
	public $lv017=null;
///////////
	public $DefaultFieldList="lv001,lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv011,lv012,lv013,lv014,lv015,lv016,lv017,lv018,lv022";	
////////////////////GetDate
	...
	public $paging=null;
	public $lang=null;
	protected $objhelp='ac_lv0004';
////////////
	var $ArrOther=array();
	var $ArrPush=array();
	var $ArrFunc=array();
	var $ArrGet=array("lv001"=>"2","lv002"=>"3","lv003"=>"4","lv004"=>"5","lv005"=>"6","lv006"=>"7","lv007"=>"8","lv008"=>"9","lv009"=>"10","lv010"=>"11","lv011"=>"12","lv012"=>"13","lv013"=>"14","lv014"=>"15","lv015"=>"16","lv016"=>"17","lv017"=>"18","lv018"=>"19","lv022"=>"23");
	var $ArrView=array("lv001"=>"0","lv002"=>"0","lv003"=>"0","lv004"=>"0","lv005"=>"0","lv006"=>"0","lv007"=>"0","lv008"=>"0","lv009"=>"2","lv010"=>"0","lv011"=>"0","lv012"=>"0","lv013"=>"0","lv014"=>"2","lv015"=>"0","lv016"=>"0","lv017"=>"10","lv018"=>"0","lv022"=>"0");	
	...
	{
		$lvsql="select * from  ac_lv0004 Where lv001='$vlv001'";
	...
	function LV_Insert()
	{
		...
		$lvsql="insert into ac_lv0004 (lv001,lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv011,lv012,lv013,lv014,lv015,lv016,lv017,lv018,lv022) values('$this->lv001','$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006','$this->lv007','$this->lv008','$this->lv009','$this->lv010','$this->lv011','$this->lv012','$this->lv013','$this->lv014','$this->lv015','$this->lv016','$this->lv017','$this->lv018','$this->lv022')";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ac_lv0004.insert',sof_escape_string($lvsql));
		return $vReturn;
	}	
	...
	function LV_Update()
	{
		...
		if($this->lv001_=="") $this->lv001_=$this->lv001;
		 $lvsql="Update ac_lv0004 set lv001='$this->lv001_',lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004',lv005='$this->lv005',lv006='$this->lv006',lv007='$this->lv007',lv008='$this->lv008',lv009='$this->lv009',lv010='$this->lv010',lv011='$this->lv011',lv012='$this->lv012',lv013='$this->lv013',lv014='$this->lv014',lv015='$this->lv015',lv016='$this->lv016',lv018='$this->lv018',lv022='$this->lv022' where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ac_lv0004.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Delete($lvarr)
	{
		if($this->isDel==0) return false;
		$lvsql = "DELETE FROM ac_lv0004  WHERE ac_lv0004.lv016<=0 AND ac_lv0004.lv001 IN ($lvarr)";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ac_lv0004.delete',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Aproval($lvarr)
	{
		if($this->isApr==0) return false;
		$lvsql = "Update ac_lv0004 set lv016=1  WHERE ac_lv0004.lv001 IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
		if($vReturn){
		 	$this->InsertLogOperation($this->DateCurrent,'ac_lv0004.approval',sof_escape_string($lvsql));
		 }
		return $vReturn;
	}	
	function LV_UnAproval($lvarr)
	{
		...
		$lvsql = "Update ac_lv0004 set lv016=0  WHERE ac_lv0004.lv001 IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ac_lv0004.unapproval',sof_escape_string($lvsql));
		return $vReturn;
	}	
	//GetMoney
	function LV_GetMN($vVoiceID,$vAccount)
	{
		$lvsql = "select (select if(ISNULL(sum(lv004)),0,sum(lv004)) from ac_lv0005 A  WHERE A.lv002 ='$vVoiceID' and A.lv006='$vAccount' and A.lv005<>A.lv006) AddMN ,(select if(ISNULL(sum(lv004)),0,sum(lv004)) from ac_lv0005 A  WHERE A.lv002 ='$vVoiceID' and A.lv005='$vAccount' and A.lv005<>A.lv006) SubMN,((select if(ISNULL(sum(lv004)),0,sum(lv004)) from ac_lv0005 A  WHERE A.lv002 ='$vVoiceID' and A.lv006='$vAccount' and A.lv005<>A.lv006)-(select if(ISNULL(sum(lv004)),0,sum(lv004)) from ac_lv0005 A  WHERE A.lv002 ='$vVoiceID' and A.lv005='$vAccount' and A.lv005<>A.lv006)) SumSub, B.lv005,B.lv006 from ac_lv0005 B where B.lv002 ='$vVoiceID' and (B.lv005='$vAccount' or B.lv006='$vAccount') and B.lv005<>B.lv006 ";
		$vReturnArr=array();
		...
		while($row= db_fetch_array($lvResult)){
			if($vflag==1)
			{
			$vReturnArr[0]=$row['AddMN'];
			$vReturnArr[1]=$row['SubMN'];
			$vReturnArr[4]=$row['SumSub'];

			$vReturnArr[2]=$vReturnArr[2].$row['lv005'];
			$vReturnArr[3]=$vReturnArr[3].$row['lv006'];
			$vflag=2;
			}
			else
			{
			if(strpos(",".$row['lv005'].",",",".$vReturnArr[2].",")===false)
				$vReturnArr[2]=$vReturnArr[2].','.$row['lv005'];
			if(strpos(",".$row['lv006'].",",",".$vReturnArr[3].",")===false)
				$vReturnArr[3]=$vReturnArr[3].','.$row['lv006'];
	...
	function LV_GetMN_InVoice($vVoiceID,$vAccount,$vopt)
	{
		$vObjArr=$this->LV_GetMN($vVoiceID,$vAccount);
		return $vObjArr[$vopt];
	}
	function LV_GetAccountAmount($vVoiceID,$vAccount,$TK)
	{
		$lvsql = "select if(ISNULL(sum(lv004)),0,sum(lv004)) SumAll from ac_lv0005 A  WHERE A.lv002 ='$vVoiceID' and A.lv006='$vAccount' and A.lv005='$TK' and A.lv005<>A.lv006";
		$vReturnArr=array();
		$lvResult= db_query($lvsql);
		$vflag=1;
		$row= db_fetch_array($lvResult);
		return $this->FormatView($row['SumAll'],1);
	}
	//////////get view///////////////
	function GetView()
	{
	...
	protected function GetCondition()
	{
		...
		if($this->lv017!="")  $strCondi=$strCondi." and lv017='$this->lv017'";
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM ac_lv0004 WHERE 1=1 ".$this->GetCondition();
		$bResultC = db_query($sqlC);
		$arrRowC = db_fetch_array($bResultC);
		return $arrRowC['nums'];
	}
	function LV_ExistEmp($vlv008,$vOpt=0)
	{
		$lvsql="select lv001 from ac_lv0004 BB where BB.lv008='$vlv008' and (BB.lv016=0)";
		$vresult=db_query($lvsql);
		$vrow=db_fetch_array($vresult);
		if($vrow)
		{
	...
	function LV_GetContractMoney($vInvoiceID)
	{
		$lvsql = "select sum(A.lv004) SumMoney from ac_lv0005 A  WHERE A.lv002 = '$vInvoiceID'";
		$vReturnArr=array();
		$lvResult= db_query($lvsql);
		$row= db_fetch_array($lvResult);
		return $row['SumMoney'];
	...
	function LV_GetDetail($vContractID,$vBangID,$vLangArr,&$vSum)
	{
		...
		$vstr='
			<table class="lvtable">
			<tr class="lvhtable"><td class="lvhtable">S</td>'.(($this->obj_conf->lv012==1)?'<td class="lvhtable">Xóa</td>':'').'<td class="lvhtable">Tiền</td><td class="lvhtable">Loại chi</td><td class="lvhtable">Ghi chú</td></tr>
		';		
		$lvsql="select A.* from ac_lv0005 A inner join ac_lv0004 B on A.lv002=B.lv001 where 1=1 and B.lv001='$vContractID'";	
		$vresult=db_query($lvsql);
		$i=1;
		$vSum=0;
		$vSumTT=0;
		...
		{
			$vtongtien=$vtongtien+$vrow['lv004'];
			$vstr=$vstr.'
			<tr class="lvlinehtable'.($i%2).'"  >
				<td style="color:'.$vrow['ColorName'].'">'.$i.'</td>
				'.(($this->obj_conf->lv012==1)?'<td style="color:'.$vrow['ColorName'].';text-align:center"><img style="cursor:pointer" src="../images/icon/delete.png"  onclick="delline(this,\''.$vrow['lv001'].'\')"/></td>':'').'
				<td><input  id="soluong_'.$vrow['lv001'].'" style="min-width:60px;width:98%;text-align:center;" type="textbox" value="'.$vrow['lv003'].'" onblur="changeqty(this,\''.$vrow['lv001'].'\')"/></td>
				<td align="center">
					<select  id="tkno_'.$vrow['lv001'].'" style="min-width:53px;width:98%;text-align:center;" type="textbox" value="'.$vrow['lv005'].'" onblur="changetkno(this,\''.$vrow['lv001'].'\')">
					'.$this->LV_LinkField('lv005',$vrow['lv005']).'
					</select>
				</td>
				<td><input type="textbox" value="'.$vrow['lv007'].'" style="min-width:60px;width:98%;;text-align:center;background:orange" onblur="changenote(this,\''.$vrow['lv001'].'\')" name="tratruoc_'.$vContractID.'_'.$i.'"/></td>
				</tr>';
			$i++;
		}
		$vstr=$vstr.'
		<tr class="lvhtable"><td class="lvhtable">&nbsp;'.$vobjtext.'</td>'.(($this->obj_conf->lv012==1)?'<td>&nbsp;</td>':'').'<td class="lvhtable"><span id="tongtien">'.$this->FormatView($vtongtien,10).'</span></td><td colspan="3">&nbsp;</td></tr>
		</table>
		';
		return $vstr;
	}
	...
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT * FROM ac_lv0004 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		...
		while ($vrow = db_fetch_array ($bResult)){
			$strL="";
			$vTongTien=$this->LV_GetMN_InVoice($vrow['lv001'],$vrow['lv010'],4);
			$vSumTongTien=$vSumTongTien+$vTongTien;
			$vSumTienNo=$vSumTienNo+$vTienNo;
			$vorder++;
			for($i=0;$i<count($lstArr);$i++)
			{
				if($lstArr[$i]=="lv017")
			...
			if((int)$vrow['lv016']==1)		$strTr=str_replace("@#04","lvlineapproval",$strTr);
			else	$strTr=str_replace("@#04","",$strTr);			
		}
		$strF=$strF."</tr>";
		$strF=str_replace("<!--lv017-->",$this->FormatView($vSumTongTien,10),$strF);
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('ac_lv0019/?lang=".$this->lang."&func='+value,'','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
";
	...
	function LV_BuilListReportOther($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$sqlS = "SELECT * FROM ac_lv0004 WHERE 1=1  ".$this->GetConditionView()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
		...
		for($i=0;$i<count($lstArr);$i++)
			{
				if($lstArr[$i]=="lv017")
					$vTemp=str_replace("@02",str_replace("@02",$this->getvaluelink($lstArr[$i],$this->FormatView($this->LV_GetMN_InVoice($vrow['lv001'],$vrow['lv010'],4),(int)$this->ArrView[$lstArr[$i]])),str_replace("@01",$vrow['lv001'] ,$lvHref)),$this->Align($lvTd,(int)$this->ArrView[$lstArr[$i]]));
				else
				{
					$vTemp=str_replace("@01","",$lvTdH);
					$vTemp=str_replace("@02",$this->ArrPush[(int)$this->ArrGet[$lstArr[$i]]],$vTemp);
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$lvTable="<div align=\"center\">".$this->Get_TitleHeader()."</div>
		<div align=\"center\"><h1>".($this->ArrPush[0])."</h2></div>
		<table  align=\"center\" class=\"lvtable\" border=1>
		@#01
		@#02
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td align=@#05>@02</td>";
		$sqlS = "SELECT * FROM ac_lv0004 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
	...
	private function sqlcondition($vFile,$vSelectID)
	{
		...
		{
			case 'lv003':
				$vsql="select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from  ac_lv0030";
				break;
			case 'lv005':
				$vsql="select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from  ac_lv0002";
				break;
			case 'lv006':
				$vsql="select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from  ac_lv0002";
				break;
			case 'lv010':
				...
				break;
			case 'lv011':
				$vsql="select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from  hr_lv0018 order by lv001 DESC";
				break;
			case 'lv022':
