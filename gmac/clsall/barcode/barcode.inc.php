	...
	class BARCODE
	{
		...
		function BARCODE($encoding="EAN-13")
		{
		   ...
		   $this->_font=dirname($_SERVER["PATH_TRANSLATED"])."/"."arialbd.ttf";
			...
			$this->_font="/var/www/arialbd.ttf"; 
		...
		function genBarCode($barnumber,$format="gif",$file="")
		{
			...
			elseif($this->_encode=="CODE128A" )
			{ 
				$this->_c128Barcode($barnumber,$this->_scale,$file,'A');
			}
			elseif($this->_encode=="CODE128B" )
			{ 
				$this->_c128Barcode($barnumber,$this->_scale,$file,'B');
			}
			elseif($this->_encode=="CODE128C" )
			{ 
				$this->_c128Barcode($barnumber,$this->_scale,$file,'C');
			}
		...
		*/
		...
		function _c39Barcode($barnumber,$scale=1,$file="",$checkdigit=false)
		{
			...
			$font_arr=@imagettfbbox ( $scale*10, 5, $this->_font, $barnumber);
			$x= floor($total_x-(int)$font_arr[0]-(int)$font_arr[2]+$scale*10)/2;//Vi tri canh theo chieu ngang cua text hien thi
			@imagettftext($im, $scale*8, 0, $x, $height2, $bar_color,$this->_font , $barnumber);
			//$scale*10: Co cua font chu ben duoi barcode
			//0: goc quay cua dong text
		...
		function _c128Barcode($barnumber,$scale=1,$file="",$useKeys)
		{
			/* Doan tu kiem tra de tra ve 128A, 128B, 128C
			$useKeys="B";
