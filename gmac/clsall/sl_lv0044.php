<?php
/////////////coding sl_lv0044///////////////
class   sl_lv0044 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
	...
	public $lv007=null;
///////////
	public $DefaultFieldList="lv001,lv002,lv003,lv004,lv007";	
////////////////////GetDate
	...
	public $paging=null;
	public $lang=null;
	protected $objhelp='sl_lv0044';
////////////
	var $ArrOther=array();
	var $ArrPush=array();
	var $ArrFunc=array();
	var $ArrGet=array("lv001"=>"2","lv002"=>"3","lv003"=>"4","lv004"=>"5","lv005"=>"6","lv006"=>"7","lv007"=>"8");
	var $ArrView=array("lv001"=>"0","lv002"=>"0","lv003"=>"0","lv004"=>"1","lv005"=>"1","lv006"=>"1","lv007"=>"1");	
	...
	function LV_CheckOneAgainCus($vParentID,$vCustomerID)
	{
		$lvsql="select lv001 from  sl_lv0044 Where lv002='$vParentID' and lv003='$vCustomerID'";
		$vresult=db_query($lvsql);
		$vrow=db_fetch_array($vresult);
		if($vrow)
		{
			$vCodeID=$vrow['lv001'];
			$lvsql1="select lv001 from  sl_lv0013 Where lv098='".$vCodeID."'";
			$vresult1=db_query($lvsql1);
			$vrow1=db_fetch_array($vresult1);
			if($vrow1)
			{	
				if($vrow1['lv001']!="" && $vrow1['lv001']!=NULL) $vCodeID='';
			}
		}
		else
		{
	...
	function LV_CheckOneAgainCode($vCodeID)
	{
	...
	{
		$lvsql="select * from  sl_lv0044 Where lv001='$vlv001'";
		$vresult=db_query($lvsql);
		$vrow=db_fetch_array($vresult);
		if($vrow)
		{
	...
	function LV_Insert()
	{
	    $lvsql="insert into sl_lv0044 (lv002,lv003,lv004,lv005,lv006,lv007) values('$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006','$this->lv007')";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'sl_lv0044.insert',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_InsertTemp()
	{
		...
		$lvsql="insert into sl_lv0044 (lv001,lv002,lv003,lv004,lv005,lv006,lv007) values('$this->lv001','$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006','$this->lv007')";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'sl_lv0044.insert',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_GetTron($vMoney,$vPercent)
	{
		/*$StrMoney=trim($vMoney.'');
		$vFirst=substr($StrMoney,0,1);
		$vSecond=substr($StrMoney,1,1);
		if($vSecond>=5) $vFirst=$vFirst+1;
		$vArrMoney=explode(".",$StrMoney);
		for($i=1;$i<=strlen($vArrMoney[0])-1;$i++)
		{
			$vFirst=$vFirst."0";
		}*/
		$vMoney=round($vMoney,-6);
		if(($vMoney*$vPercent/100)<100000) return 100000;
		return $vMoney*$vPercent/100;
	}
	function LV_Update()
	{
		...
		$lvsql="Update sl_lv0044 set lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004',lv005='$this->lv005',lv006='$this->lv006',lv007='$this->lv007',lv008='$this->lv008',lv009='$this->lv009',lv010='$this->lv010',lv011='$this->lv011' ,lv012='$this->lv012',lv013='$this->lv013',lv014='$this->lv014',lv015='$this->lv015',lv016='$this->lv016',lv017='$this->lv017'  where  lv001='$this->lv001' and lv011<=0;";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'sl_lv0044.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Delete($lvarr)
	{
		if($this->isDel==0) return false;
		$lvsql = "DELETE FROM sl_lv0044  WHERE sl_lv0044.lv001 IN ($lvarr) and (select lv006 from sl_lv0043 B where  B.lv001= sl_lv0044.lv002)<=0 ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'sl_lv0044.delete',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_Aproval($lvarr)
	{
		if($this->isApr==0) return false;
		$lvsql = "Update sl_lv0044 set lv011=1  WHERE sl_lv0044.lv001 IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'sl_lv0044.approval',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_UnAproval($lvarr)
	{
		if($this->isApr==0) return false;
		$lvsql = "Update sl_lv0044 set lv011=0  WHERE sl_lv0044.lv001 IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'sl_lv0044.unapproval',sof_escape_string($lvsql));
		return $vReturn;
	}
	//////////get view///////////////
	function GetView()
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM sl_lv0044 WHERE 1=1 ".$this->GetCondition();
	...
	{
		if($opt==0)
			$lvsql="select sum(lv005) convertmoney from sl_lv0044 A where A.lv002='$vParent' AND A.lv003 in (select B.lv002 from sl_lv0037 B where B.lv003='".$vEmployeeID."')";
		else	
			$lvsql="select sum(lv006) convertmoney from sl_lv0044 A where A.lv002='$vParent' AND A.lv003 in (select B.lv002 from sl_lv0037 B where B.lv004='".$vEmployeeID."')";
		$vresult=db_query($lvsql);
		$vrow = db_fetch_array ($vresult);
...
//////////////////////Buil list////////////////////
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT * FROM sl_lv0044 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		...
		while ($vrow = db_fetch_array ($bResult)){
			...
			for($i=0;$i<count($lstArr);$i++)
			{
				if($lstArr[$i]=="lv005")
				{
					...
					$vTemp=str_replace("@02",str_replace("@02",$this->getvaluelink($lstArr[$i],$this->FormatView($this->LV_GetSumMarkForSale($vrow['lv003'],0,$vrow['lv002']),(int)$this->ArrView[$lstArr[$i]])),str_replace("@01",$vrow['lv002'] ,$lvHref)),$this->Align($lvTd,(int)$this->ArrView[$lstArr[$i]]));
				}
				else if($lstArr[$i]=="lv006")
				{
					
					$vTemp=str_replace("@02",str_replace("@02",$this->getvaluelink($lstArr[$i],$this->FormatView($this->LV_GetSumMarkForSale($vrow['lv003'],1,$vrow['lv002']),(int)$this->ArrView[$lstArr[$i]])),str_replace("@01",$vrow['lv002'] ,$lvHref)),$this->Align($lvTd,(int)$this->ArrView[$lstArr[$i]]));
				}
				else
...
//////////////////////Buil list////////////////////
	function LV_BuilListReportOther($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$vTax)
	{
		...
		while ($vrow = db_fetch_array ($bResult)){
			...
			for($i=0;$i<count($lstArr);$i++)
			{
				if($lstArr[$i]=="lv005")
				{
					$vTemp=str_replace("@02",$this->getvaluelink($lstArr[$i],$this->FormatView($this->LV_GetSumMarkForSale($vrow['lv003'],0,$vrow['lv002']),(int)$this->ArrView[$lstArr[$i]])).(($lstArr[$i]=='lv010'|| $lstArr[$i]=='lv011')?'%':''),$this->Align($lvTd,(int)$this->ArrView[$lstArr[$i]]));
				}
				else if($lstArr[$i]=="lv006")
				{
					$vTemp=str_replace("@02",$this->getvaluelink($lstArr[$i],$this->FormatView($this->LV_GetSumMarkForSale($vrow['lv003'],1,$vrow['lv002']),(int)$this->ArrView[$lstArr[$i]])).(($lstArr[$i]=='lv010'|| $lstArr[$i]=='lv011')?'%':''),$this->Align($lvTd,(int)$this->ArrView[$lstArr[$i]]));
				}
				else
	...
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('".$this->Dir."sl_lv0044/?lang=".$this->lang."&childfunc='+value+'&ID=".base64_encode($this->lv002)."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td  class=\"#04\" align=\"@#05\">@02</td>";
		$sqlS = "SELECT * FROM sl_lv0044 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
