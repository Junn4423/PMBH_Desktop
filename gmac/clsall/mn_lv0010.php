<?php
/////////////coding mn_lv0010///////////////
class   mn_lv0010 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
	...
	public $lv006=null;
///////////
	public $DefaultFieldList="lv001,lv002,lv003,lv004,lv005,lv006";	
////////////////////GetDate
	...
	public $paging=null;
	public $lang=null;
	protected $objhelp='mn_lv0010';
////////////
	var $ArrOther=array();
	var $ArrPush=array();
	var $ArrFunc=array();
	var $ArrGet=array("lv001"=>"2","lv002"=>"3","lv003"=>"4","lv004"=>"5","lv005"=>"6","lv006"=>"7");
	var $ArrView=array("lv001"=>"0","lv002"=>"0","lv003"=>"0","lv004"=>"10","lv005"=>"0","lv006"=>"0");	
	var $ArrViewEnter=array("lv003"=>88);
	...
	{
		$lvsql="select * from  mn_lv0010 Where lv001='$vlv001'";
	...
	{		
		if($this->isAdd==0) return false;
		 $lvsql="insert into mn_lv0010 (lv001,lv002,lv003,lv004,lv005) values('$this->lv001','$this->lv002','$this->lv003','$this->lv004','$this->lv005')";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'mn_lv0010.insert',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_InsertTemp()
	{		
		if($this->isAdd==0) return false;
		$lvsql="insert into mn_lv0010 (lv002,lv003,lv004,lv005) values('$this->lv002','$this->lv003','$this->lv004','$this->lv005')";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'mn_lv0010.insert',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Update()
	{
		if($this->isEdit==0) return false;
		  $lvsql="Update mn_lv0010 set lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004',lv005='$this->lv005'   where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'mn_lv0010.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_UpdateOther()
	{
		if($this->isEdit==0) return false;
	    $lvsql="Update mn_lv0010 set lv008='$this->lv008',lv010='$this->lv010'  where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'mn_lv0010.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	...
	function LV_Delete($lvarr)
	{
		...
		$lvsql = "DELETE FROM mn_lv0010  WHERE mn_lv0010.lv001 IN ($lvarr) and mn_lv0010.lv006<=0  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'mn_lv0010.delete',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_DeleteOrther($lvarr)
	{
		if($this->isDel==0) return false;
		$lvsql = "DELETE FROM mn_lv0010  WHERE mn_lv0010.lv002 IN ($lvarr) and (select lv007 from wh_lv0004 B where  B.lv001= mn_lv0010.lv002)<=0  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'mn_lv0010.delete',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Aproval($lvarr)
	{
		...
		$lvsql = "Update mn_lv0010 set lv006=1  WHERE mn_lv0010.lv001 IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'mn_lv0010.unapproval',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_UnAproval($lvarr)
	{
		if($this->isUnApr==0) return false;
		$lvsql = "Update mn_lv0010 set lv006=0  WHERE mn_lv0010.lv001 IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'mn_lv0010.unapproval',sof_escape_string($lvsql));
		return $vReturn;
	}
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM mn_lv0010 WHERE 1=1 ".$this->GetCondition();
...
//////////////////////Buil list////////////////////
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT * FROM mn_lv0010 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('".$this->Dir."mn_lv0010/?lang=".$this->lang."&childdetailfunc='+value+'&ID=".base64_encode($this->lv002)."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
