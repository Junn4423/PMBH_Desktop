...
////////////
	...
	function LV_GetContractMoneyCustomerDateArr($vDateStart='',$vDateEnd='')
	{
		...
		while($vrow = db_fetch_array ($vresult))
		{
			if($vrow['convertmoney']==0)
			{
				if($vrow['money']==0) $vArrCus[$vrow['lv002']]=0;
			}		
			if($vTax!=0)
				{
					$allsum=$vrow['money'];
		...
		while($row= db_fetch_array($lvResult))
		{
			...
			if($row['active']!="" && $row['active']!=NULL)
				...
				<ul style="clear:both">
					...
					<li><div id="calendarview_'.$vorder.'" class="viewcalandar" style="display:none;clear:both;">
					<div style="float:left;width:100%;padding-bottom:25px">
					<div style="float:left;width:100%;padding-bottom:25px" id="calendar_content_' . $vorder . '">
						<div style="text-align:center;color:gray">Đang tải dữ liệu...</div>
					</div>
	...
	function LV_GetWHItemMini_Rewritten()
	{
		global $lvsl_lv0070; // sử dụng cấu hình ảnh
		$output = '<div style="padding:10px;background:#fff; overflow:auto;">';
		$i = 0;
		$vsqlParent = "SELECT * FROM sl_lv0006 WHERE lv004=0 AND (ISNULL(lv003) OR lv003='' OR lv001=lv003) ORDER BY lv005 ASC";
		$vresultParent = db_query($vsqlParent);
		while ($vrowParent = db_fetch_array($vresultParent)) {
			$i++;
			$output .= '<div id="thanhthus_' . $i . '" onclick="viewthanhthu(' . $i . ')" class="thanhcon ' . ($i == 1 ? 'conactive' : '') . '">' . htmlspecialchars($vrowParent['lv002']) . '</div>';
			$output .= '<div id="thanhthu_' . $i . '" style="clear:both;' . ($i == 1 ? 'display:block' : 'display:none') . '">';
			// Lấy danh sách nhóm con (lv003 = lv001 cha hoặc lv001 = lv001 cha)
			...
			while ($vrow = db_fetch_array($vresult)) {
				...
				$output .= '<div class="groupcafe">' . htmlspecialchars($vrow['lv002']) . '</div>';
				$output .= '<div><ul class="ulfix" style="padding:0px;margin:0px;">';
				$vsql1 = "SELECT * FROM sl_lv0007 WHERE lv003='" . $vrow['lv001'] . "' AND lv015=0 ORDER BY lv001, lv002 ASC";
				$vresult1 = db_query($vsql1);
				$vWidth = isset($lvsl_lv0070->lv006) ? intval($lvsl_lv0070->lv006) : 60;
				while ($vrow1 = db_fetch_array($vresult1)) {
					$output .= '<li style="float:left" title="' . htmlspecialchars($vrow1['lv001']) . '">';
					$output .= '<div style="position:relative;" onclick="AddProd(\'' . $vrow1['lv001'] . '\')" class="buttonClass">';

					if (isset($lvsl_lv0070->lv010) && $lvsl_lv0070->lv010 == 1 && !empty($vrow1['lv014'])) {
						$output .= '<img style="width:' . $vWidth . 'px;position:absolute;top:0px;left:0px" src="' . htmlspecialchars($vrow1['lv014']) . '"/>';
					}

					$output .= '<span style="margin-left:' . $vWidth . 'px;">' . htmlspecialchars($vrow1['lv002']) . '</span>';
					$output .= '</div></li>';
				}
			...
			$output .= '</div>'; // đóng thanhthu_i
		}

		$output .= '</div>'; // đóng khung chính
