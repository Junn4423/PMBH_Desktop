<?php
/////////////coding wb_lv0006///////////////
class   wb_lv0006 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
////////////
	...
	var $ArrView=array("lv001"=>"0","lv002"=>"0","lv003"=>"0","lv004"=>"0","lv005"=>"0","lv006"=>"0","lv007"=>"10","lv008"=>"10","lv009"=>"0","lv010"=>"6","lv011"=>"6","lv012"=>"0","lv013"=>"0","lv014"=>"0","lv015"=>"0","lv016"=>"0","lv017"=>"10","lv018"=>"10");	
	...
	function LV_Load()
	{
		$vsql="select * from  wb_lv0006";
		$vresult=db_query_second($vsql);
		$vrow=db_fetch_array_second($vresult);
		if($vrow)
		{
	...
	{
		$lvsql="select * from  wb_lv0006 Where lv001='$vlv001'";
		$vresult=db_query_second($lvsql);
		$vrow=db_fetch_array_second($vresult);
		if($vrow)
		{
			$this->lv001=$vrow['lv001'];
	...
	function LV_Insert()
	{
	    ...
	    $lvsql="insert into wb_lv0006 (lv001,lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv011,lv012,lv013,lv014,lv015,lv016,lv017,lv018) values('$this->lv001','$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006','$this->lv007','$this->lv008','$this->lv009','$this->lv010','$this->lv011','$this->lv012','$this->lv013','$this->lv014','$this->lv015','$this->lv016','$this->lv017','$this->lv018')";
		$vReturn= db_query_second($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'wb_lv0006.insert',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_Update()
	{
		if($this->isEdit==0) return false;
		  $lvsql="Update wb_lv0006 set lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004',lv005='$this->lv005',lv006='$this->lv006',lv007='$this->lv007',lv008='$this->lv008',lv009='$this->lv009',lv010='$this->lv010',lv011='$this->lv011',lv012='$this->lv012',lv013='$this->lv013',lv015='$this->lv015',lv016='$this->lv016',lv017='$this->lv017',lv018='$this->lv018' where  lv001='$this->lv001' and lv014<=0;";
		$vReturn= db_query_second($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'wb_lv0006.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Delete($lvarr)
	{
		if($this->isDel==0) return false;
		$lvsql = "DELETE FROM wb_lv0006  WHERE wb_lv0006.lv001 IN ($lvarr) and (select count(*) from wb_lv0007 B where  B.lv002= wb_lv0006.lv001)<=0  and wb_lv0006.lv014<=0 ";
		$vReturn= db_query_second($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'wb_lv0006.delete',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_Aproval($lvarr)
	{
		if($this->isApr==0) return false;
		$lvsql = "Update wb_lv0006 set lv014=1  WHERE wb_lv0006.lv001 IN ($lvarr)  ";
		$vReturn= db_query_second($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'wb_lv0006.approval',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_UnAproval($lvarr)
	{
		if($this->isApr==0) return false;
		$lvsql = "Update wb_lv0006 set lv014=0  WHERE wb_lv0006.lv001 IN ($lvarr)  ";
		$vReturn= db_query_second($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'wb_lv0006.unapproval',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Update_Balance($lvarr,$vWhID)
	{
		$arItem=split("@",$lvarr);
		foreach($arItem as $Item)
		{
			if($Item!="")
			{
					$lv_slton=$this->LV_get_slt_lot($Item,$vWhID);
					$lvsql="update wb_lv0006 set lv006='$lv_slton' where lv002='$Item'";
					$vReturn= db_query_second($lvsql);
					if($vReturn) $this->InsertLogOperation($this->DateCurrent,'wb_lv0006.update',sof_escape_string($lvsql));
			}
		}
	...
	function LV_Get_Items($lvarr)
	{
		$strar=str_replace("@","','",$lvarr);
		$strar="'".$strar."'";
		$lv_return="@";
		$sql="select lv002 from wb_lv0006 where lv002!='' and lv002 in($strar)";
		$bResult = db_query_second($sql);
		while ($vrow = db_fetch_array_second ($bResult)){
			$lv_return=$lv_return.$vrow['lv002']."@";
		}
		return $lv_return;
	}
	function LV_get_slt_lot($vItemId,$vWhId)
	{
		$sophieu=$this->LV_get_sophieu($vWhId,0,'lv001');
		if($sophieu=="") $sophieu="''";
		$vsql="select sum(lv004) sumslnhap from wh_lv0009 A where A.lv002 in ($sophieu) and A.lv003='$vItemId' ";
		$vresult=db_query($vsql);
		$vrow=db_fetch_array($vresult);
		$slnhap=$vrow['sumslnhap'];
		$sophieu=$this->LV_get_sophieu($vWhId,1,'lv001');
		$vsql="select sum(lv004) sumslxuat from wh_lv0011 A where A.lv002 in ($sophieu) and A.lv003='$vItemId' ";
		$vresult=db_query($vsql);
		$vrow=db_fetch_array($vresult);
		$slxuat=$vrow['sumslxuat'];
		return $this->FormatView($slnhap-$slxuat,10);
	}
	function LV_get_sophieu($vWhId,$vopt,$vField)
	{
		if($vopt==0)
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM wb_lv0006 WHERE 1=1 ".$this->GetCondition();
		$bResultC = db_query_second($sqlC);
		$arrRowC = db_fetch_array_second($bResultC);
		return $arrRowC['nums'];
	}
	//////////////////////Buil list////////////////////
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT * FROM wb_lv0006 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query_second($sqlS);
		$this->Count=db_num_rows_second($bResult);
		$strTrH="";
		$strTr="";
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('wb_lv0006/?lang=".$this->lang."&func='+value,'','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
";
	...
	public function GetBuilCheckList($vListID,$vID,$vTabIndex)
	{
		...
		$i=0;
		$vresult=db_query_second($vsql);
		$numrows=db_num_rows_second($vresult);
		while($vrow=db_fetch_array_second($vresult))		
		{
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$sqlS = "SELECT * FROM wb_lv0006 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query_second($sqlS);
		$this->Count=db_num_rows_second($bResult);
		$strTrH="";
	...
	public function CreateSelect_Second($lvsql,$lvopt)
	{
		$strReturn="";
	...
	{
		return($this->CreateSelect_Second($this->sqlcondition($vFile,$vSelectID),2));
	}
	public function LV_LinkField_Parent($vFile,$vSelectID)
	{
	...
	private function sqlcondition($vFile,$vSelectID)
	{
		...
		switch($vFile)
		{
			case 'lv002':
				...
				break;
			case 'lv003':
				$vsql="select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from  wb_lv0014";
				break;
			case 'lv004':
				$vsql="select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from  wb_lv0005";
				break;
			case 'lv009':
				$vsql="select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from  wb_lv0011";
				break;
		}
		return $vsql;
	}
	private  function getvaluelink($vFile,$vSelectID)
	{
		...
		{
			case 'lv003':
				$vsql="select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from  wb_lv0014 where lv001='$vSelectID'";
				break;
			case 'lv004':
				$vsql="select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from  wb_lv0005 where lv001='$vSelectID'";
				break;
			case 'lv009':
				$vsql="select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from  wb_lv0011 where lv001='$vSelectID'";
				break;
			default:
				$vsql ="";
				break;
