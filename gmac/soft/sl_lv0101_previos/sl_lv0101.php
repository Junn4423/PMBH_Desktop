...
if(isset($_GET['ajaxitemsend']))
{
	...
	{
		$vPercent=$lvsl_lv0014->LV_CheckPriceItem($vItemID,$vprogid);
		...
		if($vDetailID!='' && $vDetailID!=NULL)
			...
			$vsql="insert into sl_lv0014(lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv011) select '$vContractID','$vItemID',1,A.lv004,A.lv007,A.lv008,0,'$vPercent' from sl_lv0007 A where lv001='$vItemID'";
...
if(isset($_GET['ajaxquantity']))
{
	...
	echo '[CHECKDONHANG]';
		...
		if($_GET['qty']<=0 && $_GET['optdel']==1) $lvsl_lv0014->LV_Delete($_GET['chitietid']);
		echo $_GET['bangid'];
		//echo $vdata[1];
...
if($flagCtrl == 1){
$lvsl_lv0013->lv002=str_replace("Điện thoại/CMND","NONE",$_POST['txtlv802']);
...
$lvsl_lv0013->lv009=str_replace("Địa chỉ thrường trú/tạm trú","",$_POST['txtlv809']);
...
<!-------------------SP------------------------->
				<div class="khungsp">
						...
						<div  style="display:block;position:absolute;right:0px;top:0px;" >
							<div id="morelistid" style="display:none" class="morelistid">
								...
								<div><?php echo $vLangArr[38];?><div id="programid"><select name="txtlv909" id="txtlv909"   tabindex="111"  style="width:80%" onkeypress="return CheckKey(event,1)" onfocus="this.title='1';"/>
																	<?php echo $lvsl_lv0014->LV_LinkFieldExt('lv009',$lvsl_lv0014->lv009);?>
																		</select></div></div>
								<div><div style="float:left"><span class="khungsptitle">Chọn sản phẩm để thêm</span></div><div style="float:right"><img onclick="document.getElementById('morelistid').style.display='none';document.getElementById('morelistidmin').style.display='block'" width="20" src="images/icon/close.png"/></div></div>
											 <table style="width:100%"><tr><td style="width:50%"><select name="txtlv903" id="txtlv903"   tabindex="20"  style="width:100%" onkeypress="return CheckKeys(event,1,this)" onFocus="if(document.frmadd.txtlv913.value=='') document.frmadd.txtlv913.value=document.frmadd.txtlv805.value" onblur="LoadItem();changecategory_change(this.value)"/>
												<?php echo $lvsl_lv0014->LV_LinkField('lv003',$lvsl_lv0014->lv003);?>
												</select></td><td><ul id="pop-nav" lang="pop-nav4" onMouseOver="ChangeName(this,4)" xml:lang="pop-nav4">
															<li class="menupopT">
															...
															<div id="lv_popup" lang="lv_popup4" xml:lang="lv_popup4"> </div>
															</li>
														</ul>
														</td></tr></table>
						...
						<div style="float:right;width:87%" class="thanhprocess" >
						<?php 
							$strthanh="";
							$i=0;
						...
						<?php
							echo '<div class="thanhnhom" style="float:left;width:12%"><input type="hidden" id="txttongthanh" value="'.$i.'"/>'.$strthanh.'</div>';
							?>
...
function viewpopcalendar(vstt,vfocus,vtang)
{
	var o1=document.getElementById("txtContractID");
	o1.value=vfocus;
	var o2=document.getElementById("txtbangid");
	...
	var top=(1-vtang)*85;
	if(top<0) top=0;
	if(scrollwidth>(800+130))
	{
		left=(scrollwidth-930)/2
	}
