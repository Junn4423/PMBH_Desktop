...
if(isset($_GET['ajaxitemsend']))
{
	...
	$vBangID=$_GET['BangID'];
	echo '[CHECKHOPDONG]';
	echo $vContractID=$_GET['ContractID'];
	...
	if($vContractID!="" && $vContractID!=NULL)
	{
		...
		if($vDetailID!='' && $vDetailID!=NULL)
			...
			$vsql="insert into sl_lv0014(lv002,lv003,lv004,lv005,lv006,lv007,lv008) select '$vContractID','$vItemID',1,A.lv004,A.lv007,A.lv008,0 from sl_lv0007 A where lv001='$vItemID'";
...
if(isset($_GET['ajaxgopban']))
{
	...
	echo '[CHECKGOPBAN]';
		if((int)$_GET['delbang']==0)
			...
			echo $vsql="update sl_lv0013 set lv024=REPLACE(lv024,',".$_GET['bangid']."','') where lv001='".$_GET['donhangid']."'";
		echo $_GET['bangid'];
		$vresult=db_query($vsql);	
		echo '[ENDCHECKGOPBAN]';
		echo '[DONHANGNHAN]';
...
if(isset($_GET['ajaxquantity']))
{
	...
	echo '[CHECKDONHANG]';
		...
		//if($_GET['qty']<=0) $lvsl_lv0014->LV_Delete($_GET['chitietid']);
		echo $_GET['bangid'];
		//echo $vdata[1];
...
if($lvsl_lv0013->GetApr()==0) $lvsl_lv0013->lv010=getInfor($_SESSION['ERPSOFV2RUserID'],2);
...
if($flagCtrl == 1){
$lvsl_lv0013->lv002=str_replace("Mã khách hàng/CMND","NONE",$_POST['txtlv802']);
...
$lvsl_lv0013->lv013=str_replace("Ghi chú","",$_POST['txtlv813']);
...
.bangleft
{
	height:<?php echo ($lvsl_lv0070->lv006==0)?120:$lvsl_lv0070->lv006;?>px !important;
}
.bt_setdefault
{
	height:25px;
...
<div style="font-size:16px;text-align:center;"><?php echo $vLangArr[70];?> <span id="curday"><?php echo $lvsl_lv0013->FormatView($vNow,2);?></span> <span style="color:black!important;font-size:35px" title="<?php echo GetServerTimeSec();?>" id="countdown">11</span></div>
<div class="hd_cafe">
	<ul class="qlycafe">
		<li><div id="cafetab_1" class="licafecur" onclick="setviewhere(1);curtab(1);"><?php echo $vLangArr[66];?></div></li>
		...
		<li><div class="licafe" ><a href="http://localhost:81" onclick="runsubmit()" target="_blank">Lấy giờ</a></div></li>
	</ul>
</div>
<!---------------Ban hang---------------------->
...
<!-------------------SP------------------------->
				<div class="khungsp">
						<div  style="display:block;position:absolute;right:0px;top:0px;" >
							...
							<div id="morelistidmin" class="morelistidmin"><div style="float:right"><img onclick="document.getElementById('morelistid').style.display='block';document.getElementById('morelistidmin').style.display='none'" width="20" src="images/icon/more.png"/></div></div>
						</div>
						<div class="">
						<?php 
							$strthanh="";
						...
						<?php
							...
							echo '<div class="thanhnhom"><input type="hidden" id="txttongthanh" value="'.$i.'"/>'.$strthanh.'</div>';
							?>
						</div>
					</div>							
<!---------------End SP------------------------>
...
<?php
echo $lvsl_lv0013->LV_getTang($vLangArr);
?>
</div>
...
function AddItemNew(hdid,bangid,vo)
{
		$xmlhttp93=null;
			if(bangid=="") 
			{
			...
			var ArItem=Array();
			ArItem=vo.split(".",2);
			url=url+"?&ajaxitemsend=ajaxcheck"+"&ContractID="+hdid+"&BangID="+bangid+"&ItemID="+ArItem[1];
			url=url.replace("#","");
...
function statetratien()
{
		if (xmlhttp2.readyState==4)
		{
			...
			{
				o.className="bangleft waiting";
				var o=document.getElementById('bangtitle_'+domainid);
				o.className="bangtitlewait";
				var to=document.getElementById('tratien_'+domainid);
				to.innerHTML='<div class="banhang" onclick="tratien(\''+donhang+'\',\''+domainid+'\',3);" >DỌN XONG</div>';
				Report(donhang);
			}
			else if(o.className.indexOf('waiting')>0)
			{
				o.className="bangleft";
				var o=document.getElementById('bangtitle_'+domainid);
				o.className="bangtitlecur";
			}
			else
			{
				o.className="bangleft";
...
function viewpopcalendar(vstt,vfocus)
{
