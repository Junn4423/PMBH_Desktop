...
////////init object////////////////////
	$lvsl_lv0013=new sl_lv0013($_SESSION['ERPSOFV2RRight'],$_SESSION['ERPSOFV2RUserID'],'Sl0101');
...
if(isset($_GET['ajaxitemsend']))
{
	...
	if($vContractID!="" && $vContractID!=NULL )
	{
		...
		if($vresult)
		{
			$vsql="update sl_lv0013 set lv026='0' where lv001='$vContractID' and lv011=0";
			$vresult=db_query($vsql);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
if(isset($_GET['ajaxCHITIET']))
{
	$stt=$_GET['stt'];
	$maHD=$_GET['mahd'];
	echo $stt;
	echo $maHD;	 
	$vlistItem = $lvsl_lv0013->LV_GetWHItemMini();
	$vCurHD = $lvsl_lv0013->LV_GetTimeInvoice($maHD);


	echo '[LV001_VALUE]';  
	echo $lvsl_lv0013->LV_GetDetailControlMini($maHD, $stt, $vLangArr, $vlistItem, $vCurHD['VAT']);
	echo '[END_LV001_VALUE]';
...
if(isset($_GET['ajax']))
{
	...
	{
		echo '[PROGCHECK]';
		echo $vprogid=$lvsl_lv0059->LV_GetProgCus($lvsl_lv0001->lv022);
		echo '[ENDPROGCHECK]';
		...
		echo '[ENDCHECKSTATE]';
			...
			if($vtrangthai==2)
			{
				...
				if(trim($cusid)=='' || $cusid=='Mã KH')
					$vsql="update sl_lv0013 set lv002='',lv003='',lv011=1,lv005=concat(curdate(),' ',curtime()),lv032='".$lvsl_lv0013->LV_UserID."' where lv001='".$_GET['donhangid']."' and lv011=0";
				else
				{
					//$lvsl_lv0001=new sl_lv0001($_SESSION['ERPSOFV2RRight'],$_SESSION['ERPSOFV2RUserID'],'Sl0001');
...
if(isset($_GET['ajaxquantity']))
{
	...
	echo '[CHECKDONHANG]';
		...
		if($vdata[4]==0)
		{
			...
			if($_GET['qty']<=0 && $_GET['optdel']==1) $lvsl_lv0014->LV_DeleteNoApr($_GET['chitietid']);
			$vdata=$lvsl_lv0013->LV_GetBH_Invoice($_GET['chitietid']);
		}
		echo $_GET['bangid'];
		//echo $vdata[1];
...
<div class="hd_cafe">
	<ul class="qlycafe">
		...
		<li><div class="licafe" ><span id="curday"><?php echo $lvsl_lv0013->FormatView($vNow,2);?></span> <span style="" title="<?php echo GetServerTimeSec();?>" id="countdown">11</span></li>
...
<!---------SP------------------------->
				<div class="khungsp">
						<div  style="display:block;position:absolute;right:0px;top:0px;z-index:99999" >
							...
							<div id="morelistidmin" class="morelistidmin"><div style="float:right;padding-top:35px;"><img onclick="document.getElementById('morelistid').style.display='block';document.getElementById('morelistidmin').style.display='none'" width="20" src="images/icon/mui_ten.png"/></div></div>
						</div>
						<div style="clear:both;width:100%;overflow:hidden;" >
						<div style="float:right;width:82%;overflow: auto;position:relative;" class="thanhprocess" id="thanhprocess">
								...
								<div style="float:left">
										...
										<tr>
											<td width="40%">
												...
												<div class="sanpham_bg" style="float:left;width:100%">
													<ul id="pop-nav" lang="pop-nav" onMouseOver="ChangeName(this,0)" onkeyup="ChangeName(this,0)">
														<li class="menupopT">
															<input title="barcode hoặc mã sản phẩm" name="txtlv903" id="txtlv903" tabindex="200"  style="width:100%;height:30px;text-align:center;" onkeypress="return CheckKeys(event,1,this)" onFocus="if(document.frmadd.txtlv903.value=='Tìm sản phầm') document.frmadd.txtlv903.select()" onblur="//LoadItem();//changecategory_change(this.value)" onKeyUp="LoadPopupParent(this,'txtlv903','sl_lv0007','concat(lv002,@! @!,lv001)')" value="Tìm sản phầm"/>
															<div id="lv_popup" lang="lv_popup"> </div>
														</li>
													</ul>
												</div>
											</td>	
											<td width="40%">
												<div class="sanpham_bg" style="float:left;width:100%;cursor:pointer;" onclick="document.getElementById('sanphamthuongchon').style.display='block'">
												<div style="padding-left:10px">
													<div class="sanpham_bg_bnt" >
													Sẩn phẩm thường chọn
													</div>
												</div>	
												</div>
						...
						?>
									<div style="float:left;">
										...
										<div>
											<ul class="ulfix" style="padding:0px;margin:0px;">
											<?php
											$vsql1="select * from sl_lv0007 where lv003='".$vrow['lv001']."' and lv015=0 order by lv001,lv002 asc ";
											$vresult1=db_query($vsql1);
											$vWidth=$lvsl_lv0070->lv006;
											while($vrow1=db_fetch_array($vresult1))
											{
						...
						<?php
							echo '<div class="thanhnhom" style="float:left;width:15%;overflow: auto;" id="thanhnhom"><input type="hidden" id="txttongthanh" value="'.$i.'"/>'.$strthanh.'</div>';
							?>
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
function viewpopcalendar1232(vstt,vfocus)
{
	//////////////////////
	var o = document.getElementById("calendarview_" + vstt);
	o.style.display = "block";

	var container = document.getElementById("calendar_content_1");
	...
	var url=document.location;
	url=url+"?&ajaxCHITIET=ajaxcheck"+"&stt="+vstt+"&mahd="+vfocus;
	url=url.replace("#","");
	xmlhttp.onreadystatechange=function() {stateChangedCHITIET(vstt);};
	xmlhttp.open("GET",url,true);
	xmlhttp.send(null);
	alert (url);
	var vo = document.getElementById("item_" + vfocus);
...
function stateChangedCHITIET(vstt)
  {
   fcus=false;
   if (xmlhttp.readyState==4)
   {
    var startLv001 = xmlhttp.responseText.indexOf('[LV001_VALUE]') + 13;
    var endLv001 = xmlhttp.responseText.indexOf('[END_LV001_VALUE]');
    if (startLv001 !== -1 && endLv001 !== -1) {
     var lv001Value = xmlhttp.responseText.substring(startLv001, endLv001);
     console.log("Giá trị LV001 nhận được (nếu có):", lv001Value);
    } else {
     console.warn("Không tìm thấy giá trị LV001 hoặc tag tương ứng.");
    }

    console.log("--- Kết thúc Phản hồi AJAX ---");

    document.getElementById('calendar_content_' + vstt).innerHTML = lv001Value;
   }
  }
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
