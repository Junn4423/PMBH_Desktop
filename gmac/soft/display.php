...
<table class="ftable" cellpadding="1" cellspacing="1" style="position:relative;">
	...
	$vStrPost='';
	$vCount=0;
	$slq="select * from (select A.lv001,A.lv002,A.lv003,A.lv017,A.lv100,DATEDIFF(concat(year(CURRENT_DATE()),'-',month(lv017),'-',day(lv017)),CURRENT_DATE()) DAYRE  from sl_lv0001 A  where  DATEDIFF(concat(year(CURRENT_DATE()),'-',month(lv017),'-',day(lv017)),CURRENT_DATE())>=0 and DATEDIFF(concat(year(CURRENT_DATE()),'-',month(lv017),'-',day(lv017)),CURRENT_DATE())<=7  ) MP order by MP.DAYRE asc";
...
{
	$slq="select A.*,B.lv002 ItemName from wh_lv0012 A left join sl_lv0007 B on A.lv002=B.lv001 where A.lv004<0; ";
	$vResult=db_query($slq);
	while($vrow=db_fetch_array($vResult))
	{
		$vCount++;
