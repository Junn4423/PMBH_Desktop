...
if(isset($_GET['ajax']))
{
	...
	echo '[CHECK]';
	echo '<select name="txtlv005" id="txtlv005"   tabindex="9"  style="width:100%" onKeyPress="return CheckKeys(event,7,this)"/> '.$lvmn_lv0010->LV_LinkField('lv005',$lvsl_lv0007->lv004).'</select>';
	echo '[ENDCHECK]';
	exit;
}	
if($plang=="") $plang="EN";
...
{
		$data = array();
function add_person( $lv001,$lv002,$lv003, $lv004, $lv005,$lv006)
  {
  global $data;
  ...
  'lv006' => $lv006
	...
	foreach( $data as $row )
	{
		...
		$vitemid=$row['lv003'];
		//$lvmn_lv0010->LV_LoadStrucID($row['lv002'],$row['lv003']);
		//if($lvmn_lv0010->lv001==NULL)
		$lvsl_lv0007=new sl_lv0007($_SESSION['ERPSOFV2RRight'],$_SESSION['ERPSOFV2RUserID'],'Sl0007');
		$lvsl_lv0007->LV_LoadID($vitemid);
		if($lvsl_lv0007->lv001!=NULL && $lvsl_lv0007->lv001!="")
		{
		$lvmn_lv0010->lv002=$row['lv002'];
		$lvmn_lv0010->lv003=$row['lv003'];
		$lvmn_lv0010->lv004=$row['lv004'];
		$lvmn_lv0010->lv005=$row['lv005'];
		$lvmn_lv0010->lv006=$row['lv006'];
		$vresult=$lvmn_lv0010->LV_InsertTemp();
		}
		/*else
		{
		$lvmn_lv0010->lv002=$row['lv002'];
...
<?php
$lvsl_lv0007->LV_LoadID($_GET['ID']);
if($lvmn_lv0010->GetAdd()>0)
{
                      ...
                      <div id="enterline" style="display:block">
						<table width="100%" border="0" align="center" id="table1">
							...
							<tr>
								...
								<td  height="20">
									<input name="txtlv001" type="text" id="txtlv001"  value="<?php echo ($lvmn_lv0010->lv001=="")?InsertWithCheckExt('mn_lv0010', 'lv001', '',1):$lvmn_lv0010->lv001;?>" tabindex="5" maxlength="10" style="width:80%" onKeyPress="return CheckKey(event,7)" readonly="true"/>			</td>
						    </tr>
							<tr>
