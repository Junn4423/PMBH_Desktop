<?php
/////////////coding sl_lv0009///////////////
class sl_lv0009 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
	...
	public $lv005=null;
///////////
	public $DefaultFieldList="lv001,lv002,lv004,lv005";	
////////////////////GetDate
	public $DateCurrent="1900-01-01";
	public $Count=null;
	public $paging=null;
	public $lang=null;
	protected $objhelp='sl_lv0009';
	public $Dir="";
////////////
	var $ArrOther=array();
	var $ArrPush=array();
	...
	var $ArrView=array("lv001"=>"0","lv002"=>"0","lv003"=>"0","lv004"=>"0","lv005"=>"10");
	var $ArrViewEnter=array("lv004"=>"88");
	...
	function LV_CopyData($vListID,$vCodId)
	{
		if($this->isAdd==0) return false;
		$vArrID=explode(",",$vListID);
		foreach ( $vArrID as $vID)
		{
			if($vID!=$vCodId)
			{
				$lvsql = "DELETE FROM sl_lv0072  WHERE lv002 ='$vID'";
				$vReturn= db_query($lvsql);
				if($vReturn)
				{
				$lvsql1="insert into sl_lv0072(lv002,lv003,lv004,lv005,lv006,lv007) select '$vID',A.lv003,A.lv004,A.lv005,A.lv006,A.lv007 from sl_lv0072 A where A.lv002='$vCodId'";
	...
	{
		$lvsql="select * from  sl_lv0009 Where lv001='$vlv001'";
	...
	function LV_Insert()
	{
		$queryMax = "SELECT MAX(lv001) AS max_id FROM sl_lv0009";
		$result = db_query($queryMax);
		$row = db_fetch_array($result);
		$maxId = isset($row["max_id"]) ? intval($row["max_id"]) : 0;
		$lv001 = $maxId + 1;
		// if($this->isAdd==0) return false;
		$lvsql="insert into sl_lv0009 (lv001,lv002,lv003,lv004,lv005) values('$lv001','$this->lv002','$this->lv003','$this->lv004','$this->lv005')";
		$vReturn= db_query($lvsql);
	...
	function LV_Update()
	{
		if($this->isEdit==0) return false;
		 $lvsql="Update sl_lv0009 set lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004',lv005='$this->lv005' where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'sl_lv0009.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Delete($lvarr)
	{
		if($this->isDel==0) return false;
		$lvsql = "DELETE FROM sl_lv0009  WHERE sl_lv0009.lv001 IN ($lvarr)";// and (select count(*) from sl_lv0009 B where  B.lv002= sl_lv0009.lv001)<=0  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'sl_lv0009.delete',sof_escape_string($lvsql));
		return $vReturn;
	}	
	/////lv admin deletet
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM sl_lv0009 WHERE 1=1 ".$this->GetCondition();
	...
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT * FROM sl_lv0009 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
	...
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('".$this->Dir."sl_lv0009/?lang=".$this->lang."&func='+value+'&ID=".base64_encode($this->lv002)."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td align=@#05>@02</td>";
		$sqlS = "SELECT * FROM sl_lv0009 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
