<?php
/////////////coding sl_lv0063///////////////
class   sl_lv0063 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
////////////
	...
	{
		$lvsql="select * from  sl_lv0063 Where lv001='$vlv001'";
	...
	function LV_Insert()
	{
		$lvsql="insert into sl_lv0063 (lv001,lv002,lv003,lv004,lv005,lv006,lv007) values('$this->lv001','$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006','$this->lv007')";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'sl_lv0063.insert',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_Update()
	{
		$this->lv013 = ($this->lv013!="")?recoverdate(($this->lv013), $this->lang):$this->DateDefault;
		$lvsql="Update sl_lv0063 set lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004',lv005='$this->lv005',lv006='$this->lv006',lv007='$this->lv007' where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'sl_lv0063.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	...
	function LV_Delete($lvarr)
	{
		...
		$lvsql = "DELETE FROM sl_lv0063  WHERE sl_lv0063.lv001 IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'sl_lv0063.delete',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_UpdateQty($vID,$vValue,$col)
	{
		$lvsql="Update sl_lv0063 set ".$col."='$vValue' where  lv001='$vID';";
		 $vReturn= db_query($lvsql);
	}	
	//////////get view///////////////
	function GetView()
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM sl_lv0063 WHERE 1=1 ".$this->GetCondition();
...
//////////////////////Buil list////////////////////
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT * FROM sl_lv0063 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$lvTdTextBox="<td align=@#05><input type=\"textbox\" value=\"@02\" @03 onblur=\"updateprice(this,@01,'@99')\" style=\"width:100%;text-align:right\" tabindex=\"2\"/></td>";
		$vorder=$curRow;
		...
		while ($vrow = db_fetch_array ($bResult)){
			...
			for($i=0;$i<count($lstArr);$i++)
			{
				...
				if($lstArr[$i]=='lv004' || $lstArr[$i]=='lv005' || $lstArr[$i]=='lv006')
				{
					$vqty=$vrow[$lstArr[$i]];
					$vArrQty=explode(".",$vqty,2);
					if($vArrQty[1]==0) $vqty=$vArrQty[0];
					$vTemp=str_replace("@99",$lstArr[$i],str_replace("@02",str_replace("@02",$this->getvaluelink($lstArr[$i],$this->FormatView($vqty,0)),str_replace("@01",$vrow['lv001'] ,$lvHref)),$this->Align(str_replace("@01",$vrow['lv001'],$lvTdTextBox),(int)$this->ArrView[$lstArr[$i]])));	
				}
				else
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('".$this->Dir."sl_lv0063/?lang=".$this->lang."&childdetailfunc='+value+'&ID=".base64_encode($this->lv002)."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td align=@#05>@02</td>";
		$sqlS = "SELECT * FROM sl_lv0063 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
