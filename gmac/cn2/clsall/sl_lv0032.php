<?php
/////////////coding sl_lv0032///////////////
class   sl_lv0032 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
////////////
	...
	var $ArrView=array("lv001"=>"0","lv002"=>"0","lv003"=>"0","lv004"=>"1","lv005"=>"0","lv006"=>"1","lv007"=>"0","lv008"=>"10","lv009"=>"0","lv010"=>"0","lv011"=>"0","lv012"=>"0","lv013"=>"4","lv014"=>"10","lv015"=>"0","lv016"=>"0");	
	...
	{
		$lvsql="select * from  sl_lv0032 Where lv001='$vlv001'";
	...
	function LV_Insert()
	{
		...
		$this->lv014 = ($this->lv014!="")?recoverdate(($this->lv014), $this->lang)." ".gettime($this->lv014):$this->DateDefault;
		$lvsql="insert into sl_lv0032 (lv001,lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv011,lv012,lv013,lv014,lv015,lv016) values('$this->lv001','$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006','$this->lv007','$this->lv008','$this->lv009','$this->lv010','$this->lv011','$this->lv012','$this->lv013','$this->lv014','$this->lv015','$this->lv016')";
		$vReturn= db_query($lvsql);
		if($vReturn) 
		{
			$this->LV_InsertGift($this->lv016,$this->lv004,$this->lv009,$this->lv002);
			$this->InsertLogOperation($this->DateCurrent,'sl_lv0032.insert',sof_escape_string($lvsql));
		}
		return $vReturn;
	}
	function LV_InsertGift($vGiftID,$vQuantity,$vProgramID,$vUserID)
	{
		if((float)$vGiftID==0) return;
		$lvsql="select * from  sl_lv0064 Where lv001='$vGiftID'";
		$vresult=db_query($lvsql);
		if($vresult){
		$vrow=db_fetch_array($vresult);
		if($vrow)
		{
			$this->lv002=$vUserID;
			$this->lv003=$vrow['lv004'];
			$vsl=$vQuantity*$vrow['lv005']/$vrow['lv003'];
			$vArrsl=explode(".",$vsl);
			$this->lv004=$vArrsl[0];
	...
	function LV_CheckLine($vitemid,$vempid)
	{
		$lvsql="select lv001 from sl_lv0032 B where  B.lv002='$vempid' and B.lv003='$vitemid'";
		$vReturn= db_query($lvsql);
		$vrow=db_fetch_array($vReturn);
		if($vrow)
			return $vrow['lv001'];
	...
	function LV_InsertOther($vlv002,$vreplace)
	{
		...
		if($this->isAdd==0) return false;
		 $lvsql="insert into sl_lv0032 (lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv011,lv012) select '$vreplace' ,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv011,lv012 from sl_lv0011 where lv002='$vlv002'";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'sl_lv0032.InsertOther',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_CheckPriceItem($vItem,$vProgId)
	{
		$lvsql="select A.lv001,A.lv007 from sl_lv0060 A inner join sl_lv0059 B on B.lv001=A.lv002 where A.lv003='$vItem' and B.lv001='$vProgId' and B.lv008=1";
		$vReturn= db_query($lvsql);
		$vrow=db_fetch_array($vReturn);
		if($vrow)
			return $vrow['lv007'];
		...
		{
			$lvsql="select lv007 from  all_gmacv3_0.sl_lv0007 where lv001='$vItem'";
			$vReturn= db_query($lvsql);
			$vrow1=db_fetch_array($vReturn);
			if($vrow1)
				return $vrow1['lv007'];
			else
				return 0;
	...
	function LV_UpdateIncrease($id)
	{
		if($this->isEdit==0) return false;
		$lvsql="Update sl_lv0032 set lv004=lv004+1 where  lv001='$id';";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'sl_lv0032.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_UpdatePrice($id,$vPrice)
	{
		if($this->isEdit==0) return false;
		$lvsql="Update sl_lv0032 set lv006='$vPrice' where  lv001='$id';";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'sl_lv0032.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_UpdateNote($id,$vNote)
	{
		if($this->isEdit==0) return false;
		$lvsql="Update sl_lv0032 set lv010='$vNote' where  lv001='$id';";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'sl_lv0032.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_UpdateQty($id,$vQuantity)
	{
		if($this->isEdit==0) return false;
		$lvsql="Update sl_lv0032 set lv004='$vQuantity' where  lv001='$id';";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'sl_lv0032.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	...
	function LV_Delete($lvarr)
	{
		...
		$lvsql = "DELETE FROM sl_lv0032  WHERE sl_lv0032.lv001 IN ($lvarr) ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'sl_lv0032.delete',sof_escape_string($lvsql));
		return $vReturn;
	}	
	//////////get view///////////////
	function GetView()
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM sl_lv0032 WHERE 1=1 ".$this->GetCondition();
...
//////////////////////Buil list////////////////////
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$lvTdTextBox="<td align=@#05><input type=\"textbox\" value=\"@02\" @03 onblur=\"updateprice(this,@01)\" style=\"width:60px\" tabindex=\"2\"/></td>";
		$lvTdTextBoxQty="<td align=@#05><input type=\"textbox\" value=\"@02\" @03 onblur=\"updateqty(this,@01)\" style=\"width:60px\" tabindex=\"2\"/></td>";
		$lvTdTextBoxNote="<td align=@#05><input type=\"textbox\" value=\"@02\" @03 onblur=\"updatenote(this,@01)\" style=\"width:100px\" tabindex=\"2\"/></td>";
		$lvHref="@02";
		...
		$sqlS = "SELECT * FROM sl_lv0032 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		...
		while ($vrow = db_fetch_array ($bResult)){
			$strL="";
			$vorder++;
			$vlineamount=$vrow['lv004']*$vrow['lv006']-$vrow['lv004']*$vrow['lv006']*$vrow['lv011']/100+$vrow['lv004']*$vrow['lv006']*$vrow['lv008']/100;
			...
			for($i=0;$i<count($lstArr);$i++)
			{
				if($lstArr[$i]=='lv004')
				{
					...
					$vTemp=str_replace("@02",str_replace("@02",$this->getvaluelink($lstArr[$i],$this->FormatView($vqty,0)),str_replace("@01",$vrow['lv001'] ,$lvHref)),$this->Align(str_replace("@01",$vrow['lv001'],$lvTdTextBoxQty),(int)$this->ArrView[$lstArr[$i]]));	
				}
				elseif($lstArr[$i]=='lv006')
				{
					$vqty=$vrow[$lstArr[$i]];
					$vArrQty=explode(".",$vqty,2);
					if($vArrQty[1]==0) $vqty=$vArrQty[0];
				...
				elseif($lstArr[$i]=='lv010')
				{
					$vnote=$vrow[$lstArr[$i]];
					$vTemp=str_replace("@02",str_replace("@02",$this->getvaluelink($lstArr[$i],$this->FormatView($vnote,0)),str_replace("@01",$vrow['lv001'] ,$lvHref)),$this->Align(str_replace("@01",$vrow['lv001'],$lvTdTextBoxNote),(int)$this->ArrView[$lstArr[$i]]));	
				}
				elseif( $lstArr[$i]=='lv014')
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('".$this->Dir."sl_lv0032/?lang=".$this->lang."&childdetailfunc='+value+'&ID=".base64_encode($this->lv002)."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td align=@#05>@02</td>";
		$sqlS = "SELECT * FROM sl_lv0032 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
