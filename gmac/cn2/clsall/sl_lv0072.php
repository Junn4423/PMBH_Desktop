<?php
/////////////coding sl_lv0072///////////////
class   sl_lv0072 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
////////////////////GetDate
	...
	public $paging=null;
	public $lang=null;
	protected $objhelp='sl_lv0072';
////////////
	var $ArrOther=array();
	var $ArrPush=array();
	var $ArrFunc=array();
	var $ArrGet=array("lv001"=>"2","lv002"=>"3","lv003"=>"4","lv004"=>"5","lv005"=>"6","lv006"=>"7","lv007"=>"8");
	var $ArrView=array("lv001"=>"0","lv002"=>"0","lv003"=>"0","lv004"=>"0","lv005"=>"10","lv006"=>"10","lv007"=>"0");	
	...
	{
		$lvsql="select * from  sl_lv0072 Where lv001='$vlv001'";
	...
	function LV_Insert()
	{
	    ...
	    $lvsql="insert into sl_lv0072 (lv002,lv003,lv004,lv005,lv006,lv007) values('$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006','$this->lv007')";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'sl_lv0072.insert',sof_escape_string($lvsql));
		return $vReturn;
	}	
	...
	function LV_Update()
	{
		...
		$lvsql="Update sl_lv0072 set lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004',lv005='$this->lv005',lv006='$this->lv006',lv007='$this->lv007' where  lv001='$this->lv001'";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'sl_lv0072.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Delete($lvarr)
	{
		if($this->isDel==0) return false;
		$lvsql = "DELETE FROM sl_lv0072  WHERE sl_lv0072.lv001 IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'sl_lv0072.delete',sof_escape_string($lvsql));
		return $vReturn;
	}	
	//////////get view///////////////
	function GetView()
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM sl_lv0072 WHERE 1=1 ".$this->GetCondition();
	...
	{
		if($opt==0)
			$lvsql="select sum(lv005) convertmoney from sl_lv0072 A where A.lv002='$vParent' AND A.lv003 in (select B.lv002 from sl_lv0037 B where B.lv003='".$vEmployeeID."')";
		else	
			$lvsql="select sum(lv006) convertmoney from sl_lv0072 A where A.lv002='$vParent' AND A.lv003 in (select B.lv002 from sl_lv0037 B where B.lv004='".$vEmployeeID."')";
		$vresult=db_query($lvsql);
		$vrow = db_fetch_array ($vresult);
...
//////////////////////Buil list////////////////////
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT * FROM sl_lv0072 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
...
//////////////////////Buil list////////////////////
	...
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('".$this->Dir."sl_lv0072/?lang=".$this->lang."&childfunc='+value+'&ID=".base64_encode($this->lv002)."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$sqlS = "SELECT * FROM sl_lv0072 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
	...
	private function sqlcondition($vFile,$vSelectID)
	{
		...
		switch($vFile)
		{
			case 'lv002':
				...
				break;
			case 'lv003':
				$vsql="select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from  sl_lv0071";
				break;
			case 'lv004':
	...
	private  function getvaluelink($vFile,$vSelectID)
	{
		switch($vFile)
		{
			case 'lv002':
				...
				break;
			case 'lv003':
				$vsql="select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from  sl_lv0071 where lv001='$vSelectID'";
				break;
			case 'lv004':
