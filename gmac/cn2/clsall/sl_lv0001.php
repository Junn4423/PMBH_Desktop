...
////////////
	...
	function LV_Load()
	{
		$vsql="select * from all_gmacv3_0.sl_lv0001";
	...
	{
		$lvsql="select * from all_gmacv3_0.sl_lv0001 Where lv001='$vlv001'";
	...
	{
		$lvsql="update all_gmacv3_0.sl_lv0001 set lv023='$vState' where  lv001='$vCustomerID';";
		$vReturn= db_query($lvsql);
	...
	function LV_Delete($lvarr)
	{
		...
		$lvsql = "DELETE from all_gmacv3_0.sl_lv0001  WHERE sl_lv0001.lv001 IN ($lvarr) and (select count(*) from sl_lv0010 B where  B.lv002= sl_lv0001.lv001)<=0  and (select count(*) from sl_lv0013 B where  B.lv002= sl_lv0001.lv001)<=0 ";
		$vReturn= db_query($lvsql);
	...
	protected function GetCondition()
	{
		...
		{
			$vsql="select lv001 from all_gmacv3_0.hr_lv0020 where lv029 in ('".str_replace(",","','",$this->lv129)."')";
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums from all_gmacv3_0.sl_lv0001 WHERE 1=1 ".$this->GetCondition();
	...
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT * from all_gmacv3_0.sl_lv0001 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		...
		while ($vrow = db_fetch_array ($bResult)){
			...
			for($i=0;$i<count($lstArr);$i++)
			{
				switch($lstArr[$i])
				{
					...
					case "lv100":
						db_query("update all_gmacv3_0.sl_lv0001 set lv100='$vTotalPoint' where lv001='".$vrow['lv001']."'");
					...
					case 'lv101':
						...
						db_query("update all_gmacv3_0.sl_lv0001 set lv101='$vPointVoucher',lv102=(lv100-$vPointVoucher) where lv001='".$vrow['lv001']."'");
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td align=@#05>@02</td>";
		$sqlS = "SELECT * from all_gmacv3_0.sl_lv0001 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
