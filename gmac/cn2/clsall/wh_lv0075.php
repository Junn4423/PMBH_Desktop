<?php
class wh_lv0075 extends lv_controler{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	public function GetBuilCheckListDept($vListID,$vID,$vTabIndex,$vTbl,$vFieldView='lv002',$vDepID="",&$vCount=0)
	{
		$vListID=",".$vListID.",";
		$strTbl="<table  align=\"center\" class=\"lvtable\">
		<input type=\"hidden\" id=$vID name=$vID value=\"@#02\">
		...
		if($i<=1)
		{
			return '<input name="txtlv001" type="hidden" id="txtlv001" value="'.$vGiaTri.'" tabindex="9" maxlength="50" style="width:80%" onKeyPress="return CheckKey(event,7)"> ';
		}
		else
		{
			
			$strReturn=str_replace("@#01",$strGetScript,str_replace("@#02",$numrows,$strTbl));
			$strReturn=$strReturn.'<input name="txtlv001" type="hidden" id="txtlv001" value="" tabindex="9" maxlength="50" style="width:80%" onKeyPress="return CheckKey(event,7)"> ';
		}
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function PrintInOutCompareMonth($plang, $vArrLang,$vlv001,$vDateStart,$vDateEnd,$stropt,$strlv003,$strlv014,$strTyles,$strQuantiative,$strColor,$strNote,$strContractlv001,$strSource,$strCategory,$vVS_Color,$vExistItem,$vExistItemCur,$vshowimage)
	{
		...
		$lvsql = "select A.*  from  all_gmacv3_0.sl_lv0007 A left join sl_lv0005 B on A.lv004=B.lv001 where 1=1  and A.lv015>=0 $vConditionStock  ";	
		//$lvsql="select * from all_gmacv3_0.hr_lv0020 where lv028 in ('KYTHUATCHINH','KYTHUATPHU') order by lv028,lv008";
...
function LV_GetGiaCaoNhat($vItemID,$vWHID,$vGiaBQ,$vStartDate,$vEndDate)
{
	if(trim($vWHID)=='')
		$vsql="select A.lv008 GiaLonNhat from wh_lv0009 A inner join wh_lv0008 B on A.lv002=B.lv001 where A.lv003='$vItemID' and B.lv009>='$vStartDate 00:00:00' and B.lv009<='$vEndDate 23:59:59' order by A.lv008 DESC limit 0,1";
	else
		$vsql="select A.lv008 GiaLonNhat from wh_lv0009 A inner join wh_lv0008 B on A.lv002=B.lv001 where A.lv003='$vItemID' and  B.lv002='$vWHID' and B.lv009>='$vStartDate 00:00:00' and B.lv009<='$vEndDate 23:59:59' order by A.lv008 DESC limit 0,1";
	$bResultC = db_query($vsql);
	$arrRowC = db_fetch_array($bResultC);
	if($arrRowC['GiaLonNhat']>0)
		return $arrRowC['GiaLonNhat'];
	else
		return $vGiaBQ;
}
	...
	$vHeaderReportInventory="
		<table width=\"100%\" align=\"center\" cellpadding=\"0\" cellspacing=\"0\" border=\"1\" class=\"tblprint\">
			<tr height=\"30px\" class=\"tblcaption\">
				...
				<td class=\"htable\" width=\"10%\" >@20</td>
		...
		$vRowLast="
			<tr>
				<td class=\"center_style\"  valign=\"top\" colspan=\"3\" >".(($this->lang=='VN')?'Tổng':'Total')."</td>
				...
				<td class=\"right_style\" ><strong>&nbsp;</strong></td>				
...
//		if($vAddCond!="") $vCondition = $vCondition." AND lv003 IN ( select B.lv003 from wh_lv0020 B where 1=1 $vAddCond) AND Lot IN ( select B.lv014 from wh_lv0020 B where 1=1 $vAddCond)";
		...
		if($totalRows>0){
			while($arrS=db_fetch_array($bResultS)){
				...
				$vGiaCaoNhat=$this->LV_GetGiaCaoNhat($arrS['lv001'],$vWareHouseID,$vGiaVon,$vDateStart,$vDateEnd);
				$vLineRun = str_replace("@20", $this->FormatView(round($vGiaCaoNhat,0),20), $vLineRun);
				...
				{
					case 2:
						$vLineRun = str_replace("@29", '', $vLineRun);	
						break;
					case 3:
						...
						$vLineRun = str_replace("@29","<tr><td colspan=\"".(($vshowimage==1)?11:10)."\">".$vdetail."</td></tr>", $vLineRun);	
						if($vExistItemCur==1)
						{
					...
					case 4:
						...
						break;
					/*case 4:
						$vLineRun = str_replace("@29", $this->GetBuildTableInputOutputDetailStyle($plang, $vArrLang,$vDateStart,$vDateEnd,$arrS['lv001'],$strlv014,$strTyles,$strQuantiative,$strColor,$strNote,$strContractlv001,$strSource,$vlv001), $vLineRun);	
						break;*/
					default:
						...
						if($vdetail=="")
							...
							$vLineRun = str_replace("@29","<tr><td colspan=\"".(($vshowimage==1)?12:11)."\">".$vdetail."</td></tr>", $vLineRun);		
						if($vExistItemCur==1)
						{
							if($vdetail=="") {$vOrder--; $vLineRun="";}
		...
		if($vExistItemCur!=1)
		{
			...
			$strExpportAll=$strExpportAll.$this->SumSQLRun($sqlS,(($vshowimage==1)?12:11),$vArrLang[14],$plang);
		}	
		...
		$vHeader = str_replace("@20",'Giá cao nhất', $vHeader);		
		//Images
