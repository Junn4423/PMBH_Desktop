<?php
/////////////coding sl_lv0057///////////////
class   sl_lv0057 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
////////////
	...
	var $ArrView=array("lv001"=>"0","lv002"=>"0","lv003"=>"0","lv004"=>"0","lv005"=>"0","lv006"=>"0","lv007"=>"0","lv008"=>"0","lv009"=>"4","lv010"=>"0","lv011"=>"0","lv012"=>"0","lv013"=>"0","lv014"=>"0","lv015"=>"0","lv016"=>"0","lv017"=>"10","lv018"=>"0");	
	...
	function LV_LoadID($vlv001)
	{
		...
		if($vrow)
		{
			...
			$this->lv019=$this->LV_GetMN_InVoice($this->lv001);
			$this->lv018=$vrow['lv018'];				
		}
	}
	function CheckExistNK($vMaNhapKho)
	{
		$lvsql="select count(*) Nums from  ac_lv0004 Where lv003='NHAPKHO' and lv013='$vMaNhapKho'";
		$vresult=db_query($lvsql);
		$vrow=db_fetch_array($vresult);
		if($vrow['Nums']>0)
		{
	...
	function LV_Insert()
	{
		...
		$this->lv009 = ($this->lv009!="")?recoverdate(($this->lv009), $this->lang)." ".gettime($this->lv009):$this->DateDefault;
		...
		if($vReturn) 
		{
			$lvsql="insert into ac_lv0005 (lv002,lv003,lv004,lv005,lv006,lv007) values('$this->lv001','$this->lv019','$this->lv019','331','1111','PO(Mua h�ng)')";
			$vReturn= db_query($lvsql);
			$this->InsertLogOperation($this->DateCurrent,'ac_lv0004.insert',sof_escape_string($lvsql));
		}
		return $vReturn;
	}	
	function LV_Update()
	...
	function LV_Delete($lvarr)
	{
		...
		$lvsql = "DELETE FROM ac_lv0005  WHERE  ac_lv0005.lv002 IN ($lvarr) and (select count(*) from ac_lv0004 BB where  BB.lv001= ac_lv0005.lv002 and BB.lv016<=0)>0";
		$vReturn= db_query($lvsql);
		if($vReturn)
		{	
			$lvsql = "DELETE FROM ac_lv0004  WHERE ac_lv0004.lv016<=0 AND ac_lv0004.lv001 IN ($lvarr) and (select count(*) from ac_lv0005 B where  B.lv002= ac_lv0004.lv001)<=0";
			$vReturn= db_query($lvsql);
			$this->InsertLogOperation($this->DateCurrent,'ac_lv0004.delete',sof_escape_string($lvsql));
		}
		return $vReturn;
	}
	...
	//GetMoney
	function LV_GetMN_InVoice($vVoiceID)
	{
		$lvsql = "select if(ISNULL(sum(lv004)),0,sum(lv004)) SumMoney from ac_lv0005 A  WHERE A.lv002 ='$vVoiceID' ";
		$vReturnArr=array();
	...
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		while ($vrow = db_fetch_array ($bResult)){
			...
			{
				if($lstArr[$i]=="lv017")
					$vTemp=str_replace("@02",str_replace("@02",$this->getvaluelink($lstArr[$i],$this->FormatView($this->LV_GetMN_InVoice($vrow['lv001']),(int)$this->ArrView[$lstArr[$i]])),str_replace("@01",$vrow['lv001'] ,$lvHref)),$this->Align($lvTd,(int)$this->ArrView[$lstArr[$i]]));
				else
			...
			if($vrow['lv016']==1)		$strTr=str_replace("@#04","lvlineapproval",$strTr);
	...
	function LV_BuilListMini($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		if($lvList=="") $lvList=$this->DefaultFieldList;
		if($this->isView==0) return false;
		$lstArr=explode(",",$lvList);
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('sl_lv0057/?lang=".$this->lang."&func='+value,'','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
