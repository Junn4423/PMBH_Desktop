<?php
/////////////coding ac_lv0034///////////////
class   ac_lv0233 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
	...
	function PrintInOutPutInStockDetail($plang,$vArrLang,$vDateStart,$vDateEnd,$vOpt=0,$vStaffID='')
	{
		...
		<table width=\"100%\" align=\"center\" cellpadding=\"0\" cellspacing=\"0\" border=\"1\" class=\"tblprint\">
			<tr height=\"30px\" class=\"tblcaption\">
				...
				<td class=\"htable\" width=\"*%\" style=\"cursor:pointer\" colspan=\"3\"><center>Nội dung chi</center></td>
			</tr>
			<tr>
				<td class=\"htable\" width=\"20%\" ondblclick=\"RemoveCol('col_6',this,@!79)\" style=\"cursor:pointer\"><center>Ghi chú chi</center></td>
				<td class=\"htable\" width=\"10%\" ondblclick=\"RemoveCol('col_7',this,@!79)\" style=\"cursor:pointer\"><center>Loại chi</center></td>
		...
		{
			$vsqlstaff="
			select distinct A.lv008 lv001 from ac_lv0004 A where A.lv009>='".$this->datefrom."' and A.lv009<='".$this->dateto."' and  A.lv002=1 
			";
		...
		if(count($this->vArrDay)>0){
			foreach($this->vArrDay as $vDateCal)
			{
				...
				$strExpportAll1='';
				/////Chi khác
				$vsql="select A.*,C.lv002 TKNo from ac_lv0005 A left join ac_lv0002 C on A.lv005=C.lv001  inner join ac_lv0004 B on A.lv002=B.lv001 and B.lv009='".$vDateCal."' and B.lv002=1  and B.lv008 in ($vListStaff)";
				...
				while($vrow =db_fetch_array($bResultS))
				{
					...
					if($vNumline==0)
					{
						...
						$vLineRun = str_replace("@07", $vrow['TKNo'], $vLineRun);
					...
					else
					{
						...
						$vLineRun1 = str_replace("@07",$vrow['TKNo'], $vLineRun1);
				...
				$vLineRun = str_replace("@03", $this->FormatView($vChi,20), $vLineRun);
				if($vNumline==0)
				{
		...
		$vincrease++;
		$vOrder++;
		$vRowLast=str_replace("@03",$this->FormatView($vSumChi,10),$vRowLast);
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		function Export(vFrom,value)
		{
			...
			window.open('".$this->Dir."ac_lv0233/?lang=".$this->lang."&childdetailfunc='+value+'&datefrom='+datefrom+'&dateto='+dateto+'&tkno='+tkno,'','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
		</script>
";
...
//////////////////////Buil list////////////////////
	...
	public function sqlcondition($vFile,$vSelectID)
	{
		...
		switch($vFile)
		{
			...
			case 'lv199';
				$vsql="
			  	 select distinct A.lv008 lv001,B.lv002 lv002,IF(A.lv008='$vSelectID',1,0) lv003 from ac_lv0004 A  inner join all_gmacv3_0.hr_lv0020 B on A.lv008=B.lv001 where A.lv009>='".$this->datefrom."' and A.lv009<='".$this->dateto."' and  A.lv002=1";
			   break;	
		}
		return $vsql;
	}
