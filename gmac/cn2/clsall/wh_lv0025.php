<?php
/////////////coding wh_lv0025///////////////
class   wh_lv0025 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
////////////
	...
	var $ArrView=array("lv001"=>"0","lv002"=>"0","lv003"=>"0","lv004"=>"0","lv005"=>"0","lv006"=>"0","lv007"=>"0","lv008"=>"0","lv009"=>"10","lv010"=>"0","lv011"=>"22","lv012"=>"10","lv013"=>"10","lv014"=>"0","lv015"=>"10");	
	...
	{
		$lvsql="select * from  wh_lv0025 Where lv001='$vlv001'";
	...
	function LV_Insert()
	{
		$lvsql="insert into wh_lv0025 (lv001,lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv011,lv012,lv013,lv014) values('$this->lv001','$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006','$this->lv007','$this->lv008','$this->lv009','$this->lv010','$this->lv011','$this->lv012','$this->lv013','$this->lv014')";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'wh_lv0025.insert',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_Update()
	{
		if($this->isEdit==0) return false;
		 $lvsql="Update wh_lv0025 set lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004',lv005='$this->lv005',lv006='$this->lv006',lv007='$this->lv007',lv008='$this->lv008',lv009='$this->lv009',lv010='$this->lv010',lv011='$this->lv011',lv012='$this->lv012',lv013='$this->lv013',lv014='$this->lv014' where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'wh_lv0025.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Delete($lvarr)
	{
		if($this->isDel==0) return false;
		$lvsql = "DELETE FROM wh_lv0025  WHERE wh_lv0025.lv001 IN ($lvarr)";// and (select count(*) from wh_lv0025 B where  B.lv002= wh_lv0025.lv001)<=0  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'wh_lv0025.delete',sof_escape_string($lvsql));
		return $vReturn;
	}	
	/////lv admin deletet
	...
	protected function GetCondition()
	{
		...
		if($this->lv014!="") $strCondi=$strCondi." and lv010 like '%$this->lv014%'";
		return $strCondi;
	}
		////////////////Count///////////////////////////
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM wh_lv0025 WHERE 1=1 ".$this->GetCondition();
	...
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT * FROM wh_lv0025 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		...
		while ($vrow = db_fetch_array ($bResult)){
			...
			for($i=0;$i<count($lstArr);$i++)
			{
				if($lstArr[$i]=='lv015')
				{
					$vlineamount=$vrow['lv009']-$vrow['lv009']*$vrow['lv013']/100+$vrow['lv009']*$vrow['lv012']/100;
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('$this->Dir'+'wh_lv0025/?lang=".$this->lang."&childfunc='+value+'&ID=".base64_encode($this->lv002)."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td align=@#05>@02</td>";
		$sqlS = "SELECT * FROM wh_lv0025 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
	...
	function LV_GetHistoryPriceProduct($vProductPriceID)
	{
		$sqlC="select lv003,lv004,lv005,lv006,lv007 from wh_lv0049 where lv002='$vProductPriceID'  order by lv007 desc limit 0,10";
		$bResultC = db_query($sqlC);
		$lvTrH="
		<tr class=\"lvhtable\">
			<td width=1% class=\"lvhtable\">STT</td>
			<td width=1% class=\"lvhtable\">Giá</td>
			<td width=1% class=\"lvhtable\">Thuế</td>
			<td width=1% class=\"lvhtable\">Chiết khấu</td>
			<td width=1% class=\"lvhtable\">Ngày</td>
		</tr>
		";
		$i=0;
		$lvTr="";
		while($arrRowC = db_fetch_array($bResultC))
		{
			...
			<td>".$arrRowC['lv005']."</td>
			<td>".$arrRowC['lv006']."</td>
			<td>".$this->FormatView($arrRowC['lv007'],22)."</td>
			</tr>
			";
		}
		$lvTable="
		<table  align=\"center\" class=\"lvtable\">".$lvTrH.$lvTr."</table>	";
		return $lvTable;
	}
//////////////////////Buil list////////////////////
	function LV_BuilListReportMini($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvItemID)
	{
		...
		$lvTrH="<tr class=\"lvhtable\">
			<td width=1% class=\"lvhtable\">".$this->ArrPush[1]."</td>
			<td width=1% class=\"lvhtable\" title=\"Chọn\">*</td>
			@#01
			<td width=1% class=\"lvhtable\">HISTORY</td>
		</tr>
		";
		$lvTr="<tr class=\"lvlinehtable@01\">
			<td width=1% onclick=\"Select_Check('$lvChk@03',$lvFrom, '$lvChk', '$lvChkAll')\">@03</td>
			<td width=1%><input type=\"radio\" name=\"txtlv9141\" value=\"@02\" onclick=\"set_pricenccs('@09','@12','@13','@14','@02')\" /></td>
			@#01
			<td><div style=\"position:relative;\" onmouseover=\"document.getElementById('viewhistory_@03').style.display='block';\" onmouseout=\"document.getElementById('viewhistory_@03').style.display='none';\">View<div id=\"viewhistory_@03\" style=\"position:absolute;display:none;overflow:hidden;right:0px;background:#eee;z-index:9999\">@#02</div></div></td>
		</tr>
		";		
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td  class=\"#04\" align=\"@#05\">@02</td>";
		$sqlS = "SELECT * FROM wh_lv0025 WHERE 1=1 and lv007 = '$lvItemID'   order by lv009,lv013,lv012 asc";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
		...
		while ($vrow = db_fetch_array ($bResult)){
			...
			$vhistorylist=$this->LV_GetHistoryPriceProduct($vrow['lv001']);
			$strTr=$strTr.str_replace("@#01",$strL,str_replace("@14",str_replace("'","\'",$vrow['lv003']."(".$vrow['lv004'].")"),str_replace("@02",$vrow['lv002'],str_replace("@03",$vorder,str_replace("@01",$vorder%2,str_replace("@09",$vrow['lv009'],str_replace("@12",$vrow['lv012'],str_replace("@13",$vrow['lv013'],$lvTr))))))));
		...
		$strTrH=str_replace("@#01",$strH,$lvTrH);
		 ...
		 $vselectbox="
		 <br><strong>Chọn nhà cung cấp khác</strong>
		 <table border=0 cellpadding=0 cellspacing=0 width=\"80%\"><tr>
		 						<td width=\"50%\"><select name=\"txtlv914\" id=\"txtlv914\"   tabindex=\"21\"  style=\"width:100%\" onkeypress=\"return CheckKeys(event,1,this)\" onchange=\"setemptyradio()\"><option value=\"\">...</option>".$this->LV_LinkField('lv914',$this->LV_GetItemSup($lvItemID))."</select></td>";
		 $vselectbox=$vselectbox."<td width=\"50%\"><ul id=\"pop-nav3\" lang=\"pop-nav3\" onkeyup=\"ChangeName(this,3)\" onMouseOver=\"ChangeName(this,3)\" xml:lang=\"pop-nav3\">
                                                            <li class=\"menupopT\">
                                                              <input type=\"text\" autocomplete=\"off\" class=\"search_img_btn\" name=\"txtlv914_search\" id=\"txtlv914_search\" style=\"width:100%\" onKeyUp=\"LoadPopupParent(this,'txtlv914','wh_lv0003','concat(lv002,@! - @!,lv001)')\" onFocus=\"LoadPopupParent(this,'txtlv914','wh_lv0003','concat(lv002,@! - @!,lv001)')\" tabindex=\"200\">
		...
		return str_replace("@#01",$strTrH.$strTr,$lvTable.$vselectbox);
	}
	function LV_GetItemSup($vItemId)
	{
		$vsql="select lv013 from   all_gmacv3_0.sl_lv0007 where lv001='$vItemId'";
		$vresult=db_query($vsql);
		$vrow=db_fetch_array($vresult);
		if($vrow)
		{
