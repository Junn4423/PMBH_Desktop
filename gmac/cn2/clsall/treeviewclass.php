<?php /*Protected by PHP Cryptation*/
...
var $special_sign_names = array("�","�","�","�","�","�","�","�","�","�","�","�");
var $special_sign_values = array("å" ,"ä", "ö", "Å", "Ä", "Ö", "´", "ü", "Ü", "ß", "§", "€");
...
{
$this->layer++;
$this->Nodes[$key]->SetWidthFromLeft($this->layer * $this->widthbetweennodelayers);

$foundchild = true;

if ($this->LoadBaseSettingsToAllNodes($this->Nodes[$key]) == false)
$this->Nodes[$key]->SetLastChild(true);
}
}
...
$this->treeview_string .= "<table ".$cssclass." width=\"".$this->width."\" style=\"height: ".$this->nodeheight."px;\" cellpadding=\"0\" cellspacing=\"0\" border=\"0\">";
...
{
/*Ajax update.*/
$ahrefurl = "<div style=\"cursor: pointer;\" onclick=\"xajax_AjaxUpdateSelectedNode('".$this->treeviewid."','".$this->printnodes[$key]->GetId()."');\">";
$ahrefurlend = "</div>";
}
else
{
...
$this->treeview_string .= "<tr onmouseover=\"this.style.background='".$this->onmouseovernodecolor."';\" onmouseout=\"this.style.background='".$backgroundcolor."';\" bgcolor=\"".$backgroundcolor."\">";
...
$this->treeview_string .= "<img src=\"".TREEVIEW_SOURCE."media/empty.gif\" width=\"".$widthfromleft."\" height=\"1\" alt=\"\"/>";
$this->treeview_string .= "</td>";
...
$this->treeview_string .= "<td>";
$this->treeview_string .= "<input onclick=\"xajax_AjaxUpdateNodesCheckBoxValues('".$this->treeviewid."','".$this->printnodes[$key]->GetId()."');\" type=\"checkbox\" name=\"cbnode".$this->printnodes[$key]->GetId()."\" ".$checked." value=\"\"/>";
...
$this->treeview_string .= "<td>";
$this->treeview_string .= "<input onclick=\"xajax_AjaxUpdateNodesRadioButtonValues('".$this->treeviewid."','".$this->printnodes[$key]->GetId()."');\" type=\"radio\" name=\"rbnode".$this->printnodes[$key]->GetParentId()."\" ".$checked." value=\"".$this->printnodes[$key]->GetId()."\"/>";
...
$main_start_span = "<span ".$underline_javascript." style=\"".$font_color." text-decoration: ".$underline_decoration.";\">";
$main_end_span = "</span>";
...
if ($this->printnodes[$key]->GetExternUrl() != "" and $this->printnodes[$key]->extern_url_new_browser_window == false)
{
$externUrlSameBrowser = "<a href=\"".$this->printnodes[$key]->GetExternUrl()."?treeviewid=".$this->treeviewid."&nodeid=".$this->printnodes[$key]->GetId()."\">";
}
else if ($this->printnodes[$key]->frame_url != "")
{
$frameurl = "<a href=\"".$this->printnodes[$key]->frame_url."\" target=\"".$this->printnodes[$key]->frame_target."\">";
}

if ($externUrlSameBrowser != "")
{
/*Do only open the link when a user clicks on the node text. 
Or else the expand functionality will not work.*/
$ahrefurl = $externUrlSameBrowser;
}
else if ($frameurl != "")
{
/*Do only open the frame-link when a user clicks on the node text. 
Or else the expand functionality will not work.*/
$ahrefurl = $frameurl;  
}

$this->treeview_string .= "<td width=\"100%\">";
$this->treeview_string .= $ahrefurl."".$main_start_span."".$this->printnodes[$key]->GetName()."".$main_end_span."".$ahrefurlend;
$this->treeview_string .= "</td>";
