<?php
/////////////coding wh_lv0016///////////////
class   wh_lv0016 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
////////////
	...
	var $ArrView=array("lv001"=>"0","lv002"=>"0","lv003"=>"0","lv004"=>"2","lv005"=>"0","lv006"=>"0","lv007"=>"22","lv008"=>"0","lv009"=>"10");	
	...
	{
		$lvsql="select * from  wh_lv0020 Where lv001='$vlv001'";
	...
	function LV_Update()
	{
		...
		$this->lv004 = ($this->lv004!="")?recoverdate(($this->lv004), $this->lang):$this->DateDefault;
		//$this->lv007 = ($this->lv007!="")?recoverdate(($this->lv007), $this->lang):$this->DateDefault;
		 $lvsql="Update wh_lv0020 set lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004',lv005='$this->lv005',lv006='$this->lv006',lv008='$this->lv008' where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
	...
	function LV_Delete($lvarr)
	{
		...
		$lvsql = "DELETE FROM wh_lv0020  WHERE wh_lv0020.lv001 IN ($lvarr)";// and (select count(*) from wh_lv0020 B where  B.lv002= wh_lv0020.lv001)<=0  ";
		$vReturn= db_query($lvsql);
	...
	protected function GetCondition()
	{
		...
		if($this->lv018!="") $strCondi=$strCondi." and lv002 in ( select B.lv001 from  all_gmacv3_0.sl_lv0007 B where B.lv003 in ('".str_replace(",","','",$this->lv018)."'))";
		$strwh=$this->Get_WHControler();
		$strCondi=$strCondi." and lv003 in ($strwh)";
		return $strCondi;
	}
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('$this->Dir'+'wh_lv0016/?lang=".$this->lang."&childfunc='+value+'&ID=".base64_encode($this->lv002)."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
";
	...
	public function GetBuilCheckList($vListID,$vID,$vTabIndex)
	{
		...
		<script language=\"javascript\">
		function get_Checked(len,nameobj,namevalue)
		{
			var str='';
			for(i=0;i<len;i++)
			{
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum,$vopt)
	{
		...
		{
			case 1:
				$sqlS = "SELECT * FROM wh_lv0020 WHERE 1=1 and trim(lv001)!='' and  lv004=CURRENT_DATE() ".$this->GetCondition()." $strSort ";
				break;
			case 2:
				$sqlS = "SELECT * FROM wh_lv0020 WHERE 1=1 and trim(lv001)!='' and  lv004=ADDDATE(CURRENT_DATE(),1) ".$this->GetCondition()." $strSort ";
				break;
			case 3:
				$sqlS = "SELECT * FROM wh_lv0020 WHERE 1=1 and trim(lv001)!='' and  lv004<=ADDDATE(CURRENT_DATE(),$this->numdays) ".$this->GetCondition()." $strSort ";
				break;
			default:
				$sqlS = "SELECT * FROM wh_lv0020 WHERE 1=1 and trim(lv001)!='' and  lv004<CURRENT_DATE() ".$this->GetCondition()." $strSort ";
				break;
		...
		while ($vrow = db_fetch_array ($bResult)){
			$strL="";
			$vorder++;
			$vSlton=$this->LV_Get_slt_lot($vrow['lv001'],$vrow['lv002'],$vrow['lv003']);
			if($vSlton>0)
			{
				for($i=0;$i<count($lstArr);$i++)
				{
					if($lstArr[$i]=="lv009")
						$vTemp=str_replace("@02",$this->getvaluelink($lstArr[$i],$this->FormatView($vSlton,(int)$this->ArrView[$lstArr[$i]])),$this->Align($lvTd,(int)$this->ArrView[$lstArr[$i]]));
					else
						$vTemp=str_replace("@02",$this->getvaluelink($lstArr[$i],$this->FormatView($vrow[$lstArr[$i]],(int)$this->ArrView[$lstArr[$i]])),$this->Align($lvTd,(int)$this->ArrView[$lstArr[$i]]));
					$strL=$strL.$vTemp;
				}
...
private function sqlcondition($vFile,$vSelectID)
	{
		...
		switch($vFile)
		{
			...
			case 'lv013':
				...
				break;
			case 'lv014':
				$vsql="select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from  wh_lv0013";
				break;
			case 'lv016':
				$vsql="select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from   all_gmacv3_0.sl_lv0007";
				break;
