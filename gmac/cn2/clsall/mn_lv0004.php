<?php
/////////////coding mn_lv0004///////////////
class   mn_lv0004 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
////////////
	...
	var $ArrView=array("lv001"=>"0","lv002"=>"0","lv003"=>"0","lv004"=>"10","lv005"=>"0","lv006"=>"1","lv007"=>"0","lv008"=>"1","lv009"=>"0","lv010"=>"1","lv011"=>"0","lv012"=>"0","lv013"=>"1");	
	var $ArrViewEnter=array("lv005"=>"88","lv003"=>"88","lv010"=>"101","lv011"=>"101","lv012"=>"101","lv013"=>"101");
	...
	function LV_Load()
	{
		$vsql="select * from all_gmacv3_0.mn_lv0004";
	...
	{
		$lvsql="select * from all_gmacv3_0.mn_lv0004 Where lv001='$vlv001'";
	...
	{		
		if($this->isAdd==0) return false;
		 $lvsql="insert into mn_lv0004 (lv001,lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010) values('$this->lv001','$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006','$this->lv007','$this->lv008','$this->lv009','$this->lv010')";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'mn_lv0004.insert',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_InsertTemp()
	{		
		if($this->isAdd==0) return false;
		$lvsql="insert into mn_lv0004 (lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010) values('$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006','$this->lv007','$this->lv008','$this->lv009','$this->lv010')";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'mn_lv0004.insert',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Update()
	{
		if($this->isEdit==0) return false;
		  $lvsql="Update mn_lv0004 set lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004',lv005='$this->lv005',lv006='$this->lv006',lv007='$this->lv007',lv008='$this->lv008',lv009='$this->lv009',lv010='$this->lv010'  where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'mn_lv0004.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_UpdateOther()
	{
		if($this->isEdit==0) return false;
	    $lvsql="Update mn_lv0004 set lv008='$this->lv008',lv010='$this->lv010'  where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'mn_lv0004.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	...
	function LV_Delete($lvarr)
	{
		...
		$lvsql = "DELETE FROM mn_lv0004  WHERE mn_lv0004.lv001 IN ($lvarr) and mn_lv0004.lv010<=0  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'mn_lv0004.delete',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_DeleteOrther($lvarr)
	{
		if($this->isDel==0) return false;
		$lvsql = "DELETE FROM mn_lv0004  WHERE mn_lv0004.lv002 IN ($lvarr) and (select lv007 from wh_lv0004 B where  B.lv001= mn_lv0004.lv002)<=0  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'mn_lv0004.delete',sof_escape_string($lvsql));
		return $vReturn;
	}
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM mn_lv0004 WHERE 1=1 ".$this->GetCondition();
...
//////////////////////Buil list////////////////////
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$lvTable="
		<div id=\"func_id\" style='background:#f2f2f2'><div style=\"float:left\">".$this->TabFunction($lvFrom,$lvList,$maxRows)."</div><div style=\"float:right\">".$this->ListFieldSave($lvFrom,$lvList,$maxRows,$lvOrderList,$lvSortNum)."</div><div style='float:right'>&nbsp;&nbsp;&nbsp;</div><div style='float:right'>".$this->ListFieldExport($lvFrom,$lvList,$maxRows)."</div></div><div style='height:35px'></div><table  align=\"center\" class=\"lvtable\"><!--<tr ><td colspan=\"".(2+count($lstArr))."\" class=\"lvTTable\">".$this->ArrPush[0]."</td></tr>-->
		@#01
		...
		$sqlS = "SELECT * FROM mn_lv0004 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		...
		if($this->isAdd==1) 
			$strTrEnter="<tr class='entermobil'><td colspan='2'>".'<img tabindex="2" border="0" title="Tiền" class="imgButton" onclick="Save()" onmouseout="this.src=\''.$this->Dir.'../images/iconcontrol/btn_addvn.jpg\';" onmouseover="this.src=\''.$this->Dir.'../images/iconcontrol/btn_add_02vn.jpg\';" src="'.$this->Dir.'../images/iconcontrol/btn_addvn.jpg" onkeypress="return CheckKey(event,11)">'."</td>".$strTrEnter."</tr>";
		else
...
//////////////////////Buil list////////////////////
	function LV_BuilListInput($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		<tr ><td colspan=\"".(count($lstArr))."\">"."<a class=\"lvtoolbar\" href=\"javascript:Save();\" tabindex=\"47\"><img src=\"../images/controlright/save_f2.png\"  alt=\"Save\" title=\"<?php echo $vLangArr[1];?>\" name=\"save\" border=\"0\" align=\"middle\" id=\"save\" /> <?php echo $vLangArr[2];?></a>"."</td><td colspan=\"2\" align=right>".$this->ListFieldSave($lvFrom,$lvList,$maxRows,$lvOrderList,$lvSortNum)."</td></tr>
		@#01
		<tr ><td colspan=\"".(count($lstArr)+2)."\">$paging</td></tr>
		<tr ><td colspan=\"".(count($lstArr))."\">"."<a class=\"lvtoolbar\" href=\"javascript:Save();\" tabindex=\"47\"><img src=\"../images/controlright/save_f2.png\"  alt=\"Save\" title=\"<?php echo $vLangArr[1];?>\" name=\"save\" border=\"0\" align=\"middle\" id=\"save\" /> <?php echo $vLangArr[2];?></a>"."</td><td colspan=\"2\" align=right>".$this->ListFieldExport($lvFrom,$lvList,$maxRows)."</td></tr>
		</table>
		";
		$lvTrH="<tr class=\"lvhtable\">
			<td width=1% class=\"lvhtable\">".$this->ArrPush[1]."</td>
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('".$this->Dir."mn_lv0004/?lang=".$this->lang."&childdetailfunc='+value+'&ID=".base64_encode($this->lv002)."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
	...
	private function sqlcondition($vFile,$vSelectID)
	{
		...
		switch($vFile)
		{
			...
			case 'lv005':
				...
				break;
		/*	case 'lv007':
				$vsql="select lv003 lv001,lv003 lv002,IF(lv003='$vSelectID',1,0) lv003 from  sl_lv0030";
				break;
			case 'lv008':
				$vsql="select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from  sl_lv0027";
				break;*/
			case 'lv011':
				$vsql="select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from all_gmacv3_0.sl_lv0005";
				break;
	...
	private  function getvaluelink($vFile,$vSelectID)
	{
		switch($vFile)
		{
			...
			case 'lv005':
				...
				break;
			/*case 'lv007':
				$vsql="select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from  sl_lv0030 where lv001='$vSelectID'";
				break;
			case 'lv008':
				$vsql="select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from  sl_lv0027 where lv001='$vSelectID'";
				break;*/
			case 'lv011':
				$vsql="select lv001,lv002,IF(lv001='$vSelectID',1,0) lv003 from all_gmacv3_0.sl_lv0005 where lv001='$vSelectID'";
				break;
