<?php
class sl_lv0262 extends lv_controler{
	var $vRowHeader="
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function SumDonHangTheoNgay($vDateStart,$vDateEnd,$vListStaff)
	{
		$vCondition="";
		if($vDateStart!="")
		{
		...
		if($vListStaff!='')
		{
			$vCondition = $vCondition." AND B.lv032 in ($vListStaff) ";
		}
	...
	{
		$str_return1="";
		$vsql="select lv002 from all_gmacv3_0.hr_lv0020 where lv001='$vMaNV'";
		$vresult=db_query($vsql);
		while($vrow=db_fetch_array($vresult))
		{
			if($str_return1=="")
	...
	function PrintInOutPutInStockDetail($plang,$vArrLang,$vDateStart,$vDateEnd,$vStaffID='')
	{
	$vHeaderReportInventory="
		<table width=\"100%\" align=\"center\" cellpadding=\"0\" cellspacing=\"0\" border=\"1\" class=\"tblprint\">
			<tr height=\"30px\" class=\"tblcaption\">
				...
				<td rowspan=\"2\" class=\"htable\" width=\"5%\" >Kế toán giao ca</td>
				<td rowspan=\"2\" class=\"htable\" width=\"10%\" style=\"cursor:pointer\">Tiền còn lại của ca</td>
		...
		if(count($this->vArrDay)>0){
			foreach($this->vArrDay as $vDateCal)
			{
				...
				$vChi=0;
				$vContract=0;
				$vContract=$this->SumDonHangTheoNgay($vDateCal,$vDateCal,$vListStaff);
				$vSumContract=$vSumContract+$vContract;
				$strExpportAll1='';
				$vLineRun = str_replace("@33",$vStaffID.'<br/>'.$this->LV_GET_NameNV($vStaffID), $vLineRun);
				/////Chi khác
				if($vStaffID!='')
					$vsql="select SUM(A.lv003) lv003,A.lv007,B.lv008 NVKT from ac_lv0005 A inner join ac_lv0004 B on A.lv002=B.lv001 and B.lv009='".$vDateCal."' and B.lv008 in ($vListStaff) and B.lv002=1 group by B.lv008";
				else
					$vsql="select SUM(A.lv003) lv003,A.lv007,B.lv008 NVKT from ac_lv0005 A inner join ac_lv0004 B on A.lv002=B.lv001 and B.lv009='".$vDateCal."'  and B.lv002=1 group by B.lv008";
				...
				if($vStaffID!='')
					$vsql="select A.*,C.lv002 Names from wh_lv0009 A inner join all_gmacv3_0.sl_lv0007 C on A.lv003=C.lv001 inner join wh_lv0008 B on A.lv002=B.lv001 and substr(B.lv009,1,10)='".$vDateCal."'  and B.lv003 in ($vListStaff)";
				else
					$vsql="select A.*,C.lv002 Names from wh_lv0009 A inner join all_gmacv3_0.sl_lv0007 C on A.lv003=C.lv001 inner join wh_lv0008 B on A.lv002=B.lv001 and substr(B.lv009,1,10)='".$vDateCal."' ";
				$bResultS = db_query($vsql);
				while($vrow =db_fetch_array($bResultS))
				{
	...
	$vHeaderReportInventory="
		...
		if($totalRows>0){
			while($arrS=db_fetch_array($bResultS)){
				...
				//$TT28=$this->moall_gmacv3_0.sl_lv0007->LV_GetGiaVon($arrS['lv003'],"KHOTONG");
				$TT29=$arrS['TienVon'];
				$TT30=$TT18-$TT29;
				$vLineRun = str_replace("@28", $this->FormatView($TT28,10), $vLineRun);	
				$vLineRun = str_replace("@29", $this->FormatView($TT29,10), $vLineRun);	
	...
	private function sqlcondition($vFile,$vSelectID)
	{
		...
		switch($vFile)
		{
			...
			case 'lv016':
				...
				break;
			case 'lv199';
				$vsql="select distinct A.lv032 lv001,B.lv002 lv002,IF(A.lv032='$vSelectID',1,0) lv003 from  sl_lv0013 A inner join all_gmacv3_0.hr_lv0020 B on A.lv032=B.lv001 where A.lv004>='$this->datefrom 00:00:00' and A.lv004<='$this->dateto 23:59:59'";
				break;	
		}
		return $vsql;
	}
