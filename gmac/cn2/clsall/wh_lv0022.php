<?php
/////////////coding wh_lv0022///////////////
class   wh_lv0022 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
///////////
	public $DefaultFieldList="lv001,lv002,lv088,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv011,lv012,lv013,lv014,lv015,lv016";	
////////////////////GetDate
	...
	public $paging=null;
	public $lang=null;
	protected $objhelp='wh_lv0022';
////////////
	var $ArrOther=array();
	var $ArrPush=array();
	var $ArrFunc=array();
	var $ArrGet=array("lv001"=>"2","lv002"=>"3","lv003"=>"4","lv004"=>"5","lv005"=>"6","lv006"=>"7","lv007"=>"8","lv008"=>"9","lv009"=>"10","lv010"=>"11","lv011"=>"12","lv012"=>"13","lv013"=>"14","lv014"=>"15","lv015"=>"16","lv016"=>"17","lv017"=>"18","lv088"=>'89');
	var $ArrView=array("lv001"=>"0","lv002"=>"0","lv003"=>"0","lv004"=>"10","lv005"=>"0","lv006"=>"10","lv007"=>"0","lv008"=>"10","lv009"=>"10","lv010"=>"0","lv011"=>"0","lv012"=>"0","lv013"=>"22","lv014"=>"10","lv015"=>"10","lv016"=>"10","lv017"=>"0","lv088"=>"0");	
	...
	{
		$lvsql="select lv011 from wh_lv0021 B where  B.lv001='$this->lv002'";
	...
	{
		$lvsql="select * from  wh_lv0022 Where lv001='$vlv001'";
	...
	function LV_Insert()
	{
		...
		$this->lv013 = ($this->lv013!="")?recoverdate(($this->lv013), $this->lang)." ".gettime($this->lv013):$this->DateDefault;
		$lvsql="insert into wh_lv0022 (lv001,lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv011,lv012,lv013) values('$this->lv001','$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006','$this->lv007','$this->lv008','$this->lv009','$this->lv010','$this->lv011','$this->lv012','$this->lv013')";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'wh_lv0022.insert',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_InsertNoID()
	{
		
		if($this->isAdd==0) return false;
		if(!$this->LV_CheckLocked($this->lv002)) return false;
		$this->lv013 = ($this->lv013!="")?recoverdate(($this->lv013), $this->lang)." ".gettime($this->lv013):$this->DateDefault;
		$lvsql="insert into wh_lv0022 (lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv011,lv012,lv013,lv098,lv099) values('$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006','$this->lv007','$this->lv008','$this->lv009','$this->lv010','$this->lv011','$this->lv012','$this->lv013','$this->lv098','$this->lv099')";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'wh_lv0022.insert',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_UpdateNewPriceSup($vSupplierID,$vItemID,$vPrice,$vTax,$vDiscount,$vUnitPriceID)
	{
		$sqlC="select lv001,lv009,lv012,lv013 from wh_lv0025 where lv002='$vSupplierID' and lv007='$vItemID'";
		$bResultC = db_query($sqlC);
		$arrRowC = db_fetch_array($bResultC);
		if($arrRowC['lv001']!=NULL && $arrRowC['lv001']!="")
		{
			$lvsql="update wh_lv0025 set lv009='$vPrice',lv012='$vTax',lv013='$vDiscount' where lv002='$vSupplierID' and lv007='$vItemID'";
			$vReturn= db_query($lvsql);
			if($vReturn) 
			{
				if($arrRowC['lv009']!=$vPrice || $arrRowC['lv012']!=$vTax || $arrRowC['lv013']!=$vDiscount)
				{
					$lvsql="insert into wh_lv0049 (lv002,lv003,lv004,lv005,lv006,lv007) value('".$arrRowC['lv001']."','$vPrice','$vUnitPriceID','$vTax','$vDiscount',concat(CURRENT_DATE(),' ',CURRENT_TIME()))";
					$vReturn= db_query($lvsql);
				}
				$this->InsertLogOperation($this->DateCurrent,'wh_lv0025.update',sof_escape_string($lvsql));
			}
			return $vReturn;
		}
		else
		{
			$vlv001=InsertWithCheckExt('wh_lv0025', 'lv001', 'SP-NCC-',1);
			$lvsql="insert into wh_lv0025 (lv001,lv002,lv003,lv007,lv009,lv010,lv012,lv013) value('$vlv001','$vSupplierID','$vItemID','$vItemID','$vPrice','$vUnitPriceID','$vTax','$vDiscount')";
			$vReturn= db_query($lvsql);
			if($vReturn) 
				{
					$lvsql="insert into wh_lv0049 (lv002,lv003,lv004,lv005,lv006,lv007) value('$vlv001','$vPrice','$vUnitPriceID','$vTax','$vDiscount',concat(CURRENT_DATE(),' ',CURRENT_TIME()))";
					$vReturn= db_query($lvsql);
					$this->InsertLogOperation($this->DateCurrent,'wh_lv0025.insert',sof_escape_string($lvsql));
				}
			return $vReturn;
		}
	}
	function LV_InsertTempPOCP($vTemID,$vlv002,$vlv014)
	{
		$lvsql="select '$vTemID' lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv011,lv012,lv013,lv014,lv098,lv099 from wh_lv0051 where lv002='$vlv002' and lv014='$vlv014'";
		$vReturn= db_query($lvsql);
		while($vrow=db_fetch_array($vReturn))
		{
			...
			$this->lv011=$vrow['lv011'];
			$this->lv012=$vrow['lv012'];
			$this->lv013=$this->FormatView($vrow['lv013'],2);
			$this->lv098=$vrow['lv098'];
			$this->lv099=$vrow['lv099'];
			$vResult=$this->LV_InsertNoID();
			if($vResult) 
			{	
				$this->LV_UpdateNVL($this->lv098,$this->lv099,$vTemID);
				$this->LV_UpdateNewPriceSup($vrow['lv014'],$vrow['lv003'],$vrow['lv006'],$vrow['lv008'],$vrow['lv009'],$vrow['lv007']);
				
			}
		}
		if($vReturn) $this->LV_DeleteTempPOCP($vlv002,$vlv014);
		return $vReturn;
	}
	function LV_InsertTemp($vTemID,$vlv002)
	{
		$lvsql="select '$vTemID' lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv011,lv012,lv013 from wh_lv0032 where lv002='$vlv002'";
		$vReturn= db_query($lvsql);
		while($vrow=db_fetch_array($vReturn))
		{
			$this->lv002=$vrow['lv002'];
	...
	function LV_GetQuantityRecieption($vPOID,$vItemID,$vSize,$vColor)
	{
		$strLotID=$this->LV_GetLot_TP($vItemID,$vColor,$vSize);
		$lvsql="select sum(A.lv004) TotalQty,C.lv006 Quidoi from wh_lv0009 A inner join wh_lv0008 B on A.lv002=B.lv001 left join  all_gmacv3_0.sl_lv0007 C on A.lv003=C.lv001 where  B.lv006='$vPOID' and B.lv005='MUAHANG' and A.lv003='$vItemID' And A.lv014 in ($strLotID)";
		$vReturn= db_query($lvsql);
		if($vReturn)
		{
			$vrow=db_fetch_array($vReturn);
			if($vrow['Quidoi']!=0)
				return $this->FormatView( $vrow['TotalQty']/$vrow['Quidoi'],10);
			else
			return $this->FormatView( $vrow['TotalQty'],10);
		}
	}
	function LV_GetLot_TP($vItemId,$vColor,$vSize)
	{
		$str_return="";
		$vcondtion="";
		if($vColor!="" && $vSize!="")
		{
			$vcondtion = " and lv001='".$vSize."-".$vColor."'";
		}
		elseif($vColor!="") $vcondtion = " and lv001='$vColor'";
		elseif($vSize!="") $vcondtion = " and lv001='$vSize'";
		$vsql="select distinct lv001 from wh_lv0020 where 1=1 and lv002='$vItemId'  $vcondtion";
		$vresult=db_query($vsql);
	...
	{
		$lvsql = "DELETE FROM wh_lv0032  WHERE  lv002='$vlv002'";
		$vReturn= db_query($lvsql);
		return $vReturn;
	}
	function LV_UpdateNVL($vlv098,$vlv099,$vPOID)
	{
		$vsql="update mn_lv0009 set lv099='$vPOID' where lv001='".$vlv098."'";
		$vReturn= db_query($vsql);
		return $vReturn;
	}		
	function LV_DeleteTempPOCP($vlv002,$vlv014)
	{
		$lvsql = "DELETE FROM wh_lv0051  WHERE lv002='$vlv002' and lv014='$vlv014'";
		$vReturn= db_query($lvsql);
		return $vReturn;
	}	
	function LV_Update()
	{
		...
		if(!$this->LV_CheckLocked($this->lv002)) return false;
		$this->lv013 = ($this->lv013!="")?recoverdate(($this->lv013), $this->lang)." ".gettime($this->lv013):$this->DateDefault;
		$lvsql="Update wh_lv0022 set lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004',lv005='$this->lv005',lv006='$this->lv006',lv007='$this->lv007',lv008='$this->lv008',lv009='$this->lv009',lv010='$this->lv010',lv011='$this->lv011',lv012='$this->lv012',lv013='$this->lv013' where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'wh_lv0022.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_CheckLocked($vlv002)
	{
		$lvsql="select lv011 from  wh_lv0021 Where lv001='$vlv002'";
		$vresult=db_query($lvsql);
		if($vresult){
		$vrow=db_fetch_array($vresult);
			if($vrow)
	...
	function LV_Delete($lvarr)
	{
		...
		$lvsql = "DELETE FROM wh_lv0022  WHERE wh_lv0022.lv001 IN ($lvarr) and (select lv011 from wh_lv0021 B where  B.lv001= wh_lv0022.lv002)<=0  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'wh_lv0022.delete',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_Aproval($lvarr)
	{
		if($this->isApr==0) return false;
		$lvsql = "Update wh_lv0022 set lv014=1  WHERE wh_lv0022.lv001 IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'wh_lv0022.approval',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_UnAproval($lvarr)
	{
		if($this->isApr==0) return false;
		$lvsql = "Update wh_lv0022 set lv014=0  WHERE wh_lv0022.lv001 IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'wh_lv0022.unapproval',sof_escape_string($lvsql));
		return $vReturn;
	}
	//////////get view///////////////
	function GetView()
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM wh_lv0022 WHERE 1=1 ".$this->GetCondition();
		$bResultC = db_query($sqlC);
		$arrRowC = db_fetch_array($bResultC);
		return $arrRowC['nums'];
	}
...
//////////////////////Buil list////////////////////
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT *,lv003 lv088 FROM wh_lv0022 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		...
		while ($vrow = db_fetch_array ($bResult)){
			$strL="";
			$vorder++;
			$vlineamount=$vrow['lv004']*$vrow['lv006']-$vrow['lv004']*$vrow['lv006']*$vrow['lv009']/100+$vrow['lv004']*$vrow['lv006']*$vrow['lv008']/100;
			$vsumamount=$vsumamount+$vlineamount;
			$vtotalnum=$vtotalnum+$vrow['lv004'];
			$SLNK=$this->LV_GetQuantityRecieption($vrow['lv002'],$vrow['lv003'],$vrow['lv011'],$vrow['lv010']);
			$vsumNK=$vsumNK+$SLNK;
			for($i=0;$i<count($lstArr);$i++)
			{
				if($lstArr[$i]=='lv015')
				...
				else if ($lstArr[$i]=='lv016')
				{
					$vTemp=str_replace("@02",str_replace("@02",$this->getvaluelink($lstArr[$i],$this->FormatView($SLNK,(int)$this->ArrView[$lstArr[$i]])),str_replace("@01",$vrow['lv001'] ,$lvHref)),$this->Align($lvTd,(int)$this->ArrView[$lstArr[$i]]));
				}
				else
			...
			if($vrow['lv014']==1)		$strTr=str_replace("@#04","lvlineapproval",$strTr);
			else $strTr=str_replace("@#04","",$strTr);
				
		}
		$strF=$strF."</tr>";
		$strF=str_replace("<!--lv004-->",$this->FormatView($vtotalnum,10),$strF);
		$strF=str_replace("<!--lv015-->",$this->FormatView($vsumamount,10),$strF);
		$strF=str_replace("<!--lv016-->",$this->FormatView($vsumNK,10),$strF);
		...
		return str_replace("@#01",$strTrH.$strTr,str_replace("@#02",$lvSumTd,str_replace("@#03",$lvTotalNumTd,$lvTable)));
	}
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('".$this->Dir."wh_lv0022/?lang=".$this->lang."&childdetailfunc='+value+'&ID=".base64_encode($this->lv002)."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$sqlS = "SELECT *,lv003 lv088 FROM wh_lv0022 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
		...
		while ($vrow = db_fetch_array ($bResult)){
			...
			for($i=0;$i<count($lstArr);$i++)
			{
				...
				elseif($lstArr[$i]=='lv016')
				{
					$vTemp=str_replace("@02",$this->getvaluelink($lstArr[$i],$this->FormatView($SLNK,(int)$this->ArrView[$lstArr[$i]])),$this->Align($lvTd,(int)$this->ArrView[$lstArr[$i]]));
				}	
				else
...
//////////////////////Buil list////////////////////
	...
	function LV_BuilListReportOther($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$vTax)
	{
		...
		$lvTrTotal="<tr >
			...
			<td class=\"lvlineboldtable\" ><div style='text-align:right'>@01</div></td>
		</tr>
		";
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td align=@#05>@02</td>";
		...
		while ($vrow = db_fetch_array ($bResult)){
			...
			for($i=0;$i<count($lstArr);$i++)
			{
				if($lstArr[$i]=="lv015")
				{
					if($vTax>0 || $vTax==-1)
					{
					$vTemp=str_replace("@02",$this->getvaluelink($lstArr[$i],$this->FormatView($vrow['lv004']*$vrow['lv006']-$vrow['lv004']*$vrow['lv006']*$vrow['lv009']/100,(int)$this->ArrView[$lstArr[$i]])),$this->Align($lvTd,(int)$this->ArrView[$lstArr[$i]]));
					}
					else
					{
					$vTemp=str_replace("@02",$this->getvaluelink($lstArr[$i],$this->FormatView($vrow['lv004']*$vrow['lv006']+$vrow['lv004']*$vrow['lv006']*$vrow['lv008']/100-$vrow['lv004']*$vrow['lv006']*$vrow['lv009']/100,(int)$this->ArrView[$lstArr[$i]])),$this->Align($lvTd,(int)$this->ArrView[$lstArr[$i]]));
					}
				}
				else
					$vTemp=str_replace("@02",$this->getvaluelink($lstArr[$i],$this->FormatView($vrow[$lstArr[$i]],(int)$this->ArrView[$lstArr[$i]])).(($lstArr[$i]=='lv008' || $lstArr[$i]=='lv009')?'%':''),$lvTd);
				$strL=$strL.$vTemp;
			}
			...
			if($vTax>0 || $vTax==-1)
			{
				$strSubTotal=$strSubTotal+$vrow['lv004']*$vrow['lv006']-$vrow['lv004']*$vrow['lv006']*$vrow['lv009']/100;
			}
			else
			{
				$strSubTotal=$strSubTotal+$vrow['lv004']*$vrow['lv006']+$vrow['lv004']*$vrow['lv006']*$vrow['lv008']/100 -$vrow['lv004']*$vrow['lv006']*$vrow['lv009']/100;
			}
			
		}
		$strLine=str_replace("@01",$this->FormatView($strSubTotal,10),$lvTrTotal);
			$strLine=str_replace("@03",str_replace("@01"," (".$vUnitPrice.")",$this->ArrPush[16]),$strLine);
			...
			$strLine=str_replace("@03",str_replace("@02",$this->FormatView($vTax,10),$this->ArrPush[18]),$strLine);
			$strLine=str_replace("@04",count($lstArr),$strLine);
			$strTr=$strTr.$strLine;
			}
			...
			$strLine=str_replace("@03",str_replace("@01"," (".$vUnitPrice.")",$this->ArrPush[17]),$strLine);
			$strLine=str_replace("@04",count($lstArr),$strLine);
			$strTr=$strTr.$strLine;
...
//////////////////////Buil list////////////////////
	function LV_BuilListReportMini($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvItemID)
	{
		...
		$lvTrH="<tr class=\"lvhtable\">
			<td width=1% class=\"lvhtable\">".$this->ArrPush[1]."</td>
			<td width=1% class=\"lvhtable\" title=\"Chá»�n\">*</td>
			@#01
		</tr>
		";
		$lvTr="<tr class=\"lvlinehtable@01\">
			<td width=1% onclick=\"Select_Check('$lvChk@03',$lvFrom, '$lvChk', '$lvChkAll')\">@03</td>
			<td width=1%><input type=\"radio\" name=\"txtlv9142\" value=\"@02\" onclick=\"set_pricenccs('@09','@12','@13','@14','@17')\" /></td>
			@#01
		</tr>
		";		
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td  class=\"#04\" align=\"@#05\">@02</td>";
		$sqlS = "SELECT A.*,B.lv002 lv017,B.lv004 lv016 FROM wh_lv0022 A inner join wh_lv0021 B on A.lv002=B.lv001 WHERE 1=1 and A.lv003 = '$lvItemID'   order by B.lv004 desc";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
		...
		while ($vrow = db_fetch_array ($bResult)){
			...
			for($i=0;$i<count($lstArr);$i++)
			{
				if($lstArr[$i]=='lv015')
				{
					$vlineamount=$vrow['lv006']-$vrow['lv006']*$vrow['lv009']/100+$vrow['lv006']*$vrow['lv008']/100;
					$vTemp=str_replace("@02",$this->getvaluelink($lstArr[$i],$this->FormatView($vlineamount,(int)$this->ArrView[$lstArr[$i]])),$this->Align($lvTd,(int)$this->ArrView[$lstArr[$i]]));
				}
				else
				$vTemp=str_replace("@02",$this->getvaluelink($lstArr[$i],$this->FormatView($vrow[$lstArr[$i]],(int)$this->ArrView[$lstArr[$i]])),$this->Align($lvTd,(int)$this->ArrView[$lstArr[$i]]));
			...
			$strTr=$strTr.str_replace("@#01",$strL,str_replace("@14",str_replace("'","\'",$vrow['lv012']),str_replace("@02",$vrow['lv002'],str_replace("@03",$vorder,str_replace("@01",$vorder%2,str_replace("@09",$vrow['lv006'],str_replace("@12",$vrow['lv008'],str_replace("@13",$vrow['lv009'],str_replace("@17",$vrow['lv017'],$lvTr)))))))));
			if($vrow['lv007']==1)		$strTr=str_replace("@#04","lvlineapproval",$strTr);
			else	$strTr=str_replace("@#04","",$strTr);
			$strTr=str_replace("@#02",$vhistorylist,$strTr);
	...
	private function sqlcondition($vFile,$vSelectID)
	{
		...
		switch($vFile)
		{
			...
			case 'lv011':
				...
				break;
			case 'lv020':
				$vsql="select lv001,lv003 lv002,IF(lv001='$vSelectID',1,0) lv003 from  sl_lv0027";
				break;
			case 'lv021':
				$vsql="select lv003 lv001,lv003 lv002,0 lv003 from  sl_lv0030  order by lv004 asc";
				break;
		}
		return $vsql;
	}
	private  function getvaluelink($vFile,$vSelectID)
	{
		switch($vFile)
		{
			...
			case 'lv007':
				...
				break;
			case 'lv010':
				$vsql="select lv001,lv003 lv002,IF(lv001='$vSelectID',1,0) lv003 from  sl_lv0027 where lv001='$vSelectID'";
				break;			
			case 'lv017':
