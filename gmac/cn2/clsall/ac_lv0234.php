	...
	function PrintInOutPutInStockDetail($plang,$vArrLang,$vDateStart,$vDateEnd,$vOpt=0,$vStaffID='')
	{
		...
		if($vStaffID!='')
				...
				else
				{
					...
					";*/
					$vsqlstaff="
					select distinct * from (select A.lv008 lv001 from ac_lv0004 A where A.lv009>='".$this->datefrom."' and A.lv009<='".$this->dateto."' and  A.lv002=1) MP
					";
		...
		if(count($this->vArrDay)>0){
			foreach($this->vArrDay as $vDateCal)
			{
				foreach($vArrStaff as $vStaff)
				{
					...
					$strExpportAll1='';
					/////Chi khác
					if($vOpt==0 || $vOpt==1 || $vOpt==2)
					{
						if($vOpt==2)
						{
							if($vStaffID!='')
								$vsql="select A.*,B.lv013 from ac_lv0005 A inner join ac_lv0004 B on A.lv002=B.lv001 and B.lv009='".$vDateCal."' and B.lv002=1 and B.lv008 in ($vListStaff) and A.lv003='NHAPKHO'";
							else
								$vsql="select A.*,B.lv013 from ac_lv0005 A inner join ac_lv0004 B on A.lv002=B.lv001 and B.lv009='".$vDateCal."' and B.lv002=1  and A.lv003='NHAPKHO'";

						}
						elseif($vOpt==1)
						{
							if($vStaffID!='')
								$vsql="select A.* from ac_lv0005 A inner join ac_lv0004 B on A.lv002=B.lv001 and B.lv009='".$vDateCal."' and B.lv002=1 and B.lv008 in ($vListStaff) and A.lv003<>'NHAPKHO'";
							else
								$vsql="select A.* from ac_lv0005 A inner join ac_lv0004 B on A.lv002=B.lv001 and B.lv009='".$vDateCal."' and B.lv002=1  and A.lv003<>'NHAPKHO'";

						}
						else
						{
							if($vStaffID!='')
								$vsql="select A.*,B.lv013 from ac_lv0005 A inner join ac_lv0004 B on A.lv002=B.lv001 and B.lv009='".$vDateCal."' and B.lv002=1 and B.lv008 in ($vListStaff)";
							else
								$vsql="select A.*,B.lv013 from ac_lv0005 A inner join ac_lv0004 B on A.lv002=B.lv001 and B.lv009='".$vDateCal."' and B.lv002=1";
						}
						...
						while($vrow =db_fetch_array($bResultS))
						{
							if($vOpt==2 || $vOpt==0)
							{
								$vArrPhieuNhapKho[$vrow['lv013']]=$vArrPhieuNhapKho[$vrow['lv013']]+round($vrow['lv003'],0);
							}
				...
				{
					if($vStaffID!='')
						$vsql="select A.*,C.lv002 Names from wh_lv0009 A inner join all_gmacv3_0.sl_lv0007 C on A.lv003=C.lv001 inner join wh_lv0008 B on A.lv002=B.lv001 and substr(B.lv009,1,10)='".$vDateCal."'  and B.lv005<>'KIEMKHO'  and B.lv003 in ($vListStaff)";
					else
						$vsql="select A.*,C.lv002 Names from wh_lv0009 A inner join all_gmacv3_0.sl_lv0007 C on A.lv003=C.lv001 inner join wh_lv0008 B on A.lv002=B.lv001 and substr(B.lv009,1,10)='".$vDateCal."'  and B.lv005<>'KIEMKHO'";
						$bResultS = db_query($vsql);
...
//////////////////////Buil list////////////////////
	...
	public function sqlcondition($vFile,$vSelectID)
	{
		...
		switch($vFile)
		{
			...
			case 'lv199';
	 			$vsql="
				select distinct * from (select A.lv008 lv001,B.lv002 lv002,IF(A.lv008='$vSelectID',1,0) lv003 from ac_lv0004 A  inner join all_gmacv3_0.hr_lv0020 B on A.lv008=B.lv001 where A.lv009>='".$this->datefrom."' and A.lv009<='".$this->dateto."' and  A.lv002=1
				union
				select A.lv003 lv001,B.lv002 lv002,IF(A.lv003='$vSelectID',1,0) lv003 from  wh_lv0008 A  inner join all_gmacv3_0.hr_lv0020 B on A.lv003=B.lv001 where A.lv009>='".$this->datefrom." 00:00:00' and A.lv009<='".$this->dateto." 23:59:59' and A.lv005<>'KIEMKHO') MP
				";
				break;	
		}
		return $vsql;
