<?php
/////////////coding ac_lv0034///////////////
class   ac_lv0076 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
////////////
	...
	{
		$lvsql="select lv007 from ac_lv0076 B where  B.lv001='$this->lv002'";
	...
	{
		$lvsql="select * from  ac_lv0076 Where lv001='$vlv001'";
	...
	function LV_Insert()
	{
		$lvsql="insert into ac_lv0076 (lv002,lv003,lv004,lv005,lv006,lv007) values('$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006','$this->lv007')";
		$vReturn= db_query($lvsql);
		if($vReturn){	
		 	$this->InsertLogOperation($this->DateCurrent,'ac_lv0076.insert',sof_escape_string($lvsql));
		 }
		return $vReturn;
	}	
	function LV_InsertTemp($vTemID,$vlv002,$vlv008)
	{
		$lvsql="insert into ac_lv0005(lv002,lv003,lv004,lv005,lv006,lv007) select '$vTemID' ,lv003,lv004,lv005,lv006,lv007 from ac_lv0076 where lv002='$vlv002' and lv008='$vlv008'";
		$vReturn= db_query($lvsql);
	...
	function LV_InsertCAL($vCalID,$vUserID,$vlv005,$vlv006,$vlv011,$vlv012)
	{
		...
		$lvsql="insert into ac_lv0076(lv002,lv003,lv004,lv005,lv006,lv007,lv008) select '$vUserID' ,sum(A.lv004*A.lv006+A.lv016),sum(A.lv009+A.lv017),'$vlv005','$vlv006','SO(Bán hàng)','$vCalID' from ac_lv0006 A inner join ac_lv0004 B on A.lv002=B.lv001  where 1=1 and A.lv002='$vCalID'";		
		$vresult=db_query($lvsql);
		return $vresult;
	...
	{
		$lvsql = "DELETE FROM ac_lv0076  WHERE ac_lv0076.lv002='$vlv002'";
		$vReturn= db_query($lvsql);
		return $vReturn;
	}	
	function LV_Update()
	{
		if($this->isEdit==0) return false;
		  $lvsql="Update ac_lv0076 set lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004',lv005='$this->lv005',lv006='$this->lv006',lv007='$this->lv007' where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ac_lv0076.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_UpdateEdit($vOldlv004,$vOldlv006)
	{
		...
		$vReturn= db_query($lvsql);
		if($vReturn){
		 $this->InsertLogOperation($this->DateCurrent,'ac_lv0076.update',sof_escape_string($lvsql));
	...
	function LV_Delete($lvarr)
	{
		...
		$lvsql = "DELETE FROM ac_lv0076  WHERE ac_lv0076.lv001 IN ($lvarr)";
		$vReturn= db_query($lvsql);
		if($vReturn) {
			$this->InsertLogOperation($this->DateCurrent,'ac_lv0076.delete',sof_escape_string($lvsql));
		}
		return $vReturn;
	}	
	//////////get view///////////////
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM ac_lv0076 WHERE 1=1 ".$this->GetCondition();
	...
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM ac_lv0076 WHERE lv002='$vlv002'";
		//$sqlC = "SELECT COUNT(*) AS nums FROM ac_lv0076 WHERE lv002='$vlv002' and lv008='$vlv008'";
		$bResultC = db_query($sqlC);
		$arrRowC = db_fetch_array($bResultC);
		return $arrRowC['nums'];
	}
...
//////////////////////Buil list////////////////////
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT * FROM ac_lv0076 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('".$this->Dir."ac_lv0076/?lang=".$this->lang."&childdetailfunc='+value+'&ID=".base64_encode($this->lv002)."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td align=@#05>@02</td>";
		$sqlS = "SELECT * FROM ac_lv0076 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
