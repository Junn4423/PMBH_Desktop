<?php
/////////////coding sl_lv0037///////////////
class   sl_lv0037 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
////////////////////GetDate	
	...
	public $paging=null;
	public $lang=null;
	protected $objhelp='sl_lv0037';
////////////
	var $ArrOther=array();
	var $ArrPush=array();
	var $ArrFunc=array();
	var $ArrGet=array("lv001"=>"2","lv002"=>"3","lv003"=>"4","lv004"=>"5","lv005"=>"6","lv006"=>"7","lv007"=>"8");
	var $ArrView=array("lv001"=>"0","lv002"=>"0","lv003"=>"0","lv004"=>"0","lv005"=>"22","lv006"=>"0","lv007"=>"1");	
	...
	{
		$lvsql="select * from  sl_lv0037 Where lv001='$vlv001'";
	...
	function LV_Insert()
	{
		...
		if($this->isAdd==0) return false;
		 $lvsql="insert into sl_lv0037 (lv001,lv002,lv003,lv004,lv005,lv006) values('$this->lv001','$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006')";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'sl_lv0037.insert',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_Update()
	{
		if($this->isEdit==0) return false;
		  $lvsql="Update sl_lv0037 set lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004',lv006='$this->lv006' where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'sl_lv0037.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Delete($lvarr)
	{
		if($this->isDel==0) return false;
		$lvsql = "DELETE FROM sl_lv0037  WHERE sl_lv0037.lv001 IN ($lvarr) and (select count(*) from sl_lv0010 B where  B.lv002= sl_lv0037.lv001)<=0  and (select count(*) from sl_lv0013 B where  B.lv002= sl_lv0037.lv001)<=0 ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'sl_lv0037.delete',sof_escape_string($lvsql));
		return $vReturn;
	}	
	//////////get view///////////////
	function GetView()
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM sl_lv0037 WHERE 1=1 ".$this->GetCondition();
	...
	{
		$lvsql="select sum(PM.lv003) money,sum(PM.lv004) convertmoney,sum(PM.lv005) converdiscount from ((select sum(A.lv004*A.lv006) lv003,sum(A.lv004*A.lv006*A.lv008/100) lv004,sum(A.lv004*A.lv006*A.lv011/100) lv005 from sl_lv0014 A inner join sl_lv0013 B on A.lv002=B.lv001  where 1=1 and B.lv010='$vContractID' AND B.lv011=1 )) PM ";	
		$vresult=db_query($lvsql);
		$vrow = db_fetch_array ($vresult);
		$lvsql="select sum(PM.lv003) money,sum(PM.lv004) convertmoney,sum(PM.lv005) converdiscount from ((select sum(A.lv004*A.lv008) lv003,sum(A.lv004*A.lv008*A.lv010/100) lv004,sum(A.lv004*A.lv008*A.lv011/100) lv005 from wh_lv0011 A inner join wh_lv0010 B on A.lv002=B.lv001  where 1=1 and B.lv003='$vContractID' AND B.lv007=1 )) PM ";	
		$vresult1=db_query($lvsql);
		$vrow1 = db_fetch_array ($vresult1);
		return $vrow['convertmoney']+$vrow['money']+$vrow1['convertmoney']+$vrow1['money']-$vrow['converdiscount']-$vrow1['converdiscount'];;
	}
	function LV_GetContractMoneyDate($vContractID,$StartDate,$EndDate)
	{
		...
		$lvsql="select sum(PM.lv003) money,sum(PM.lv004) convertmoney,sum(PM.lv005) converdiscount from ((select sum(A.lv004*A.lv006) lv003,sum(A.lv004*A.lv006*A.lv008/100) lv004,sum(A.lv004*A.lv006*A.lv011/100) lv005 from sl_lv0014 A inner join sl_lv0013 B on A.lv002=B.lv001  where 1=1 and B.lv010='$vContractID' AND B.lv011=1 $condition1 )) PM ";	
		$vresult=db_query($lvsql);
		$vrow = db_fetch_array ($vresult);
		$lvsql="select sum(PM.lv003) money,sum(PM.lv004) convertmoney,sum(PM.lv005) converdiscount from ((select sum(A.lv004*A.lv008) lv003,sum(A.lv004*A.lv008*A.lv010/100) lv004,sum(A.lv004*A.lv008*A.lv011/100) lv005 from wh_lv0011 A inner join wh_lv0010 B on A.lv002=B.lv001  where 1=1 and B.lv003='$vContractID' AND B.lv007=1 $condition2)) PM ";	
		$vresult1=db_query($lvsql);
		$vrow1 = db_fetch_array ($vresult1);
		return $vrow['convertmoney']+$vrow['money']+$vrow1['convertmoney']+$vrow1['money'] -$vrow['converdiscount']-$vrow1['converdiscount'];
	}
	...
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT * FROM sl_lv0037 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		...
		while ($vrow = db_fetch_array ($bResult)){
			...
			for($i=0;$i<count($lstArr);$i++)
			{
				if($lstArr[$i]=="lv007")
				{
					...
					$vTemp=str_replace("@02",str_replace("@02",$this->getvaluelink($lstArr[$i],$this->FormatView($this->LV_GetContractMoney($vrow['lv002']),(int)$this->ArrView[$lstArr[$i]])),str_replace("@01",$vrow['lv002'] ,$lvHref)),$this->Align($lvTd,(int)$this->ArrView[$lstArr[$i]]));
				}
				else
				$vTemp=str_replace("@02",str_replace("@02",$this->getvaluelink($lstArr[$i],$this->FormatView($vrow[$lstArr[$i]],(int)$this->ArrView[$lstArr[$i]])),str_replace("@01",$vrow['lv002'] ,$lvHref)),$this->Align($lvTd,(int)$this->ArrView[$lstArr[$i]]));
				$strL=$strL.$vTemp;
			}
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('sl_lv0037/?lang=".$this->lang."&func='+value,'','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td align=@#05>@02</td>";
		$sqlS = "SELECT * FROM sl_lv0037 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
...
//////////////////////Buil list////////////////////
	function LV_BuilListReportStartEndDate($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvEmployeeID,$lvStartDate,$lvEndDate)
	{
		...
		$sqlS = "SELECT * FROM sl_lv0037 WHERE 1=1 $condition ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
	...
	private function sqlcondition($vFile,$vSelectID)
	{
		...
		{
			case 'lv002':
				$vsql="select lv001,concat(lv004,' ',lv003,' ',lv002,'(',lv001,')') lv002,IF(lv001='$vSelectID',1,0) lv003 from  all_gmacv3_0.hr_lv0020";
				break;
			case 'lv003':
				$vsql="select lv001,concat(lv004,' ',lv003,' ',lv002,'(',lv001,')') lv002,IF(lv001='$vSelectID',1,0) lv003 from  all_gmacv3_0.hr_lv0020";
				break;
	...
	private  function getvaluelink($vFile,$vSelectID)
	{
		...
		{
			case 'lv002':
				$vsql="select lv001,concat(lv004,' ',lv003,' ',lv002,'(',lv001,')') lv002,IF(lv001='$vSelectID',1,0) lv003 from  all_gmacv3_0.hr_lv0020 where lv001='$vSelectID'";
				break;
			case 'lv003':
				$vsql="select lv001,concat(lv004,' ',lv003,' ',lv002,'(',lv001,')') lv002,IF(lv001='$vSelectID',1,0) lv003 from  all_gmacv3_0.hr_lv0020 where lv001='$vSelectID'";
				break;
