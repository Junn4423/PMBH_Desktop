<?php
/////////////coding wh_lv0051///////////////
class   wh_lv0051 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
////////////
	...
	var $ArrView=array("lv001"=>"0","lv002"=>"0","lv003"=>"0","lv004"=>"10","lv005"=>"0","lv006"=>"10","lv007"=>"0","lv008"=>"10","lv009"=>"10","lv010"=>"0","lv011"=>"0","lv012"=>"0","lv013"=>"22","lv014"=>"0","lv015"=>"10");	
	...
	function LV_LoadSup($vUserID)
	{
		$vsql="select distinct lv014 SupplierID,(select B.lv013 from wh_lv0051 B where B.lv014=A.lv014 Order by lv013 ASC limit 0,1) DeliveryDate from  wh_lv0051 A where A.lv002='$vUserID' ";
		return $vresult=db_query($vsql);
		
	}
	function LV_Load()
	{
		$vsql="select * from  wh_lv0051";
	...
	{
		$lvsql="select * from  wh_lv0051 Where lv001='$vlv001'";
	...
	function LV_Insert()
	{
		...
		$lvsql="insert into wh_lv0051 (lv001,lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv011,lv012,lv013,lv014) values('$this->lv001','$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006','$this->lv007','$this->lv008','$this->lv009','$this->lv010','$this->lv011','$this->lv012','$this->lv013','$this->lv014')";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'wh_lv0051.insert',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_Insert_Temp()
	{
		$lvsql="insert into wh_lv0051 (lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv011,lv012,lv013,lv014,lv098,lv099) values('$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006','$this->lv007','$this->lv008','$this->lv009','$this->lv010','$this->lv011','$this->lv012','$this->lv013','$this->lv014','$this->lv098','$this->lv099')";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'wh_lv0051.insert',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_CalMeterial()
	{
		$sqlS = "SELECT A.*,C.lv004 UnitID,C.lv005 OtherUnitID,C.lv006 Converts,C.lv007 Price,C.lv008 UnitPrice,C.lv011 Tax,C.lv013 SupID FROM mn_lv0009 A inner join sl_lv0013 B on A.lv002=B.lv001 left join  all_gmacv3_0.sl_lv0007 C on A.lv003=C.lv001 WHERE B.lv011=1 and A.lv010=1 and ( A.lv001 not in (select BBB.lv098 from wh_lv0022 BBB)) and ( A.lv001 not in (select BBB.lv098 from wh_lv0051 BBB))";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->lv013 = ($this->lv013!="")?recoverdate(($this->lv013), $this->lang):$this->DateDefault;
		while ($vrow = db_fetch_array ($bResult)){
			$this->lv003=$vrow['lv003'];
			if($vrow['lv005']==$vrow['UnitID'] || $vrow['lv005']==$vrow['OtherUnitID'])
			{
				if($vrow['lv005']==$vrow['UnitID'])
				{
					$this->lv006=$vrow['Price'];
					if($vrow['Converts']!=0)
					{
						$this->lv004=($vrow['lv012']+$vrow['lv013'])/$vrow['Converts'];
						$this->lv006=$vrow['Price']*$vrow['Converts'];
					}
					else
						$this->lv004=($vrow['lv012']+$vrow['lv013']);
					$this->lv005=$vrow['OtherUnitID'];
				}
				else
				{
					$this->lv004=$vrow['lv012']+$vrow['lv013'];
			...
			$this->lv007=$vrow['UnitPrice'];
			$this->lv008=$vrow['Tax'];
			$this->lv010=$vrow['lv008'];
			$this->lv009=$vrow['lv007'];
			$this->lv011='';
			$this->lv012='';
			$this->lv014=$vrow['SupID'];			
			$this->lv098=$vrow['lv001'];	
			$this->lv099=$vrow['lv002'];	
			$this->LV_Insert_Temp();	
		}
	}
	function LV_Update()
	{
		$this->lv013 = ($this->lv013!="")?recoverdate(($this->lv013), $this->lang)." ".gettime($this->lv013):$this->DateDefault;
		$lvsql="Update wh_lv0051 set lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004',lv005='$this->lv005',lv006='$this->lv006',lv007='$this->lv007',lv008='$this->lv008',lv009='$this->lv009',lv010='$this->lv010',lv011='$this->lv011',lv012='$this->lv012',lv013='$this->lv013',lv014='$this->lv014' where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'wh_lv0051.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_UpdateEdit()
	{
		$this->lv013 = ($this->lv013!="")?recoverdate(($this->lv013), $this->lang)." ".gettime($this->lv013):$this->DateDefault;
		  $lvsql="Update wh_lv0051 set lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004',lv005='$this->lv005',lv006='$this->lv006',lv007='$this->lv007',lv008='$this->lv008',lv009='$this->lv009',lv010='$this->lv010',lv011='$this->lv011',lv012='$this->lv012',lv013='$this->lv013',lv014='$this->lv014' where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
	...
	function LV_Delete($lvarr)
	{
		...
		$lvsql = "DELETE FROM wh_lv0051  WHERE wh_lv0051.lv001 IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'wh_lv0051.delete',sof_escape_string($lvsql));
		return $vReturn;
	}	
	//////////get view///////////////
	function GetView()
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM wh_lv0051 WHERE 1=1 ".$this->GetCondition();
...
//////////////////////Buil list////////////////////
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT * FROM wh_lv0051 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('".$this->Dir."wh_lv0051/?lang=".$this->lang."&childdetailfunc='+value+'&ID=".base64_encode($this->lv002)."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$lvTdF="<td align=\"right\"><strong>@01</strong></td>";
		$sqlS = "SELECT * FROM wh_lv0051 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
