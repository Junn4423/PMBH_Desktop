	...
	$vHeaderReportInventory="
		<table width=\"100%\" align=\"center\" cellpadding=\"0\" cellspacing=\"0\" border=\"1\" class=\"tblprint\">
			<tr height=\"5px\" class=\"tblRunChild\" >
				...
				<td class=\"htablerun\" width=\"15%\" >LotID</td>
				<td class=\"htablerun\" width=\"15%\" >VS-Color</td>
				<td class=\"htablerun\" width=\"*%\" >SLT</td>
				<td class=\"htablerun\" width=\"15%\" >DateExpire</td>
			</tr>
			@01
		</table>";			
		$vRowFirst="
	    ...
	    $lvsql="select * from wh_lv0020 where lv002='$vlv003' and lv003='$vlv001' $vAddCond order by lv004 desc";
		$strAllTr='';
		$lvResult = db_query($lvsql);
		while($row=db_fetch_array($lvResult)){
				$lvTemp=str_replace("@02",$row['lv001'],$vRowFirst);
				$lvTemp=str_replace("@08",$row['lv008'],$lvTemp);
...
function PrintInOutPutInStockDetail($plang, $vArrLang,$vlv001,$vDateStart,$vDateEnd,$stropt,$strlv003,$strlv014,$strTyles,$strQuantiative,$strColor,$strNote,$strContractlv001,$strSource,$strCategory,$vVS_Color,$vExistItem,$vExistItemCur)
	{
	$vHeaderReportInventory="
		<table width=\"100%\" align=\"center\" cellpadding=\"0\" cellspacing=\"0\" border=\"1\" class=\"tblprint\">
			<tr height=\"30px\" class=\"tblcaption\">
...
//		if($vAddCond!="") $vCondition = $vCondition." AND lv003 IN ( select B.lv003 from wh_lv0020 B where 1=1 $vAddCond) AND Lot IN ( select B.lv014 from wh_lv0020 B where 1=1 $vAddCond)";
			...
			while($arrS=db_fetch_array($bResultS)){
				...
				switch($stropt)
				{
					...
					case 3:
						...
						$vLineRun = str_replace("@20", $vdetail, $vLineRun);
						if($vExistItemCur==1)
						{
	...
	$vHeaderReportInventory="
		...
		{
			$strwh=$this->Get_WHControler();
			$vAddContractR=$vAddContractR." AND (B.lv002 in in ($strwh)) ";
			$vAddContractO=$vAddContractO." AND (B.lv002 in in ($strwh))  ";
		}
		//if($vAddCond!="") $vCondition = $vCondition." AND lv003 IN ( select B.lv003 from wh_lv0020 B where 1=1 $vAddCond) AND Lot IN ( select B.lv014 from wh_lv0020 B where 1=1 $vAddCond)";
