<?php
/////////////coding ac_lv0114///////////////
class   ac_lv0114 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
///////////
	public $DefaultFieldList="lv001,lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009,lv010,lv011,lv012,lv013,lv014,lv015,lv016,lv017,lv018,lv019";	
////////////////////GetDate
	...
	public $paging=null;
	public $lang=null;
	protected $objhelp='ac_lv0114';
////////////
	var $ArrOther=array();
	var $ArrPush=array();
	var $ArrFunc=array();
	var $ArrGet=array("lv001"=>"2","lv002"=>"3","lv003"=>"4","lv004"=>"5","lv005"=>"6","lv006"=>"7","lv007"=>"8","lv008"=>"9","lv009"=>"10","lv010"=>"11","lv011"=>"12","lv012"=>"13","lv013"=>"14","lv014"=>"15","lv015"=>"16","lv016"=>"17","lv017"=>"18","lv018"=>"19","lv019"=>"20");
	var $ArrView=array("lv001"=>"0","lv002"=>"0","lv003"=>"0","lv004"=>"2","lv005"=>"2","lv006"=>"10","lv007"=>"0","lv008"=>"0","lv009"=>"0","lv010"=>"0","lv011"=>"10","lv012"=>"0","lv013"=>"0","lv014"=>"0","lv015"=>"0","lv016"=>"10","lv017"=>"10","lv018"=>"10","lv019"=>"0");	
	...
	{
		$lvsql="select * from  ac_lv0114 Where lv001='$vlv001'";
	...
	function LV_LoadYearShow($lvYear)
	{
		$lvsql="select * from  ac_lv0114 Where lv006='$lvYear'";
		$vresult=db_query($lvsql);
		$vrow=db_fetch_array($vresult);
		if($vrow)
		{
	...
	function LV_LoadActiveID()
	{
		$lvsql="select * from  ac_lv0114 Where lv011=1";
		$vresult=db_query($lvsql);
		$vrow=db_fetch_array($vresult);
		if($vrow)
		{
	...
	function LV_InsertOther($lv002,$vlv0022)
	{
		...
		$lvsql="insert into tc_lv0015 (lv002,lv003,lv004,lv005) select '$lv002',lv003,lv004,lv005 from tc_lv0015 where lv002='$vlv0022'";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'tc_lv0015.insert',sof_escape_string($lvsql));
		$lvsql="insert into tc_lv0025 (lv002,lv003,lv004,lv005) select '$lv002',lv003,lv004,lv005 from tc_lv0025 where lv002='$vlv0022'";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'tc_lv0025.insert',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Update()
	{
		...
		$this->lv004 = ($this->lv004!="")?recoverdate(($this->lv004), $this->lang):$this->DateDefault;
		$this->lv005 = ($this->lv005!="")?recoverdate(($this->lv005), $this->lang):$this->DateDefault;
	  	$lvsql="Update ac_lv0114 set lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004',lv005='$this->lv005',lv006='$this->lv006',lv007='$this->lv007',lv008='$this->lv008',lv009='$this->lv009',lv010='$this->lv010',lv012='$this->lv012',lv013='$this->lv013',lv014='$this->lv014',lv015='$this->lv015',lv016='$this->lv016',lv017='$this->lv017',lv018='$this->lv018',lv019='$this->lv019' where  lv001='$this->lv001' AND lv011<=0;";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ac_lv0114.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Delete($lvarr)
	{
		if($this->isDel==0) return false;
		$lvsql = "DELETE FROM ac_lv0114  WHERE ac_lv0114.lv011<=0 AND ac_lv0114.lv001 IN ($lvarr) and (select count(*) from tc_lv0014 where ac_lv0114.lv001=tc_lv0014.lv002)<=0 and (select count(*) from tc_lv0015 where ac_lv0114.lv001=tc_lv0015.lv002)<=0  and (select count(*) from tc_lv0025 where ac_lv0114.lv001=tc_lv0025.lv002)<=0";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ac_lv0114.delete',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_Aproval($lvarr)
	{
		if($this->isApr==0) return false;
		$lvsql = "Update ac_lv0114 set lv011=lv011+1  WHERE ac_lv0114.lv001 IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ac_lv0114.approval',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_UnAproval($lvarr)
	{
		...
		$lvsql = "Update ac_lv0114 set lv011=lv011-1  WHERE ac_lv0114.lv001 IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ac_lv0114.unapproval',sof_escape_string($lvsql));
		return $vReturn;
	}
	//////////get view///////////////
	function GetView()
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM ac_lv0114 WHERE 1=1 ".$this->GetCondition();
...
//////////////////////Buil list////////////////////
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT * FROM ac_lv0114 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('".$this->Dir."ac_lv0114/?lang=".$this->lang."&func='+value+'&ID=".base64_encode($this->lv001)."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td  class=\"@#04\" align=\"@#05\">@02</td>";
		$sqlS = "SELECT * FROM ac_lv0114 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
	...
	private function sqlcondition($vFile,$vSelectID)
	{
		...
		switch($vFile)
		{
			...
			case 'lv008':
				...
				break;		
			case 'lv009':
				$vsql="select lv001,concat(lv001,' ',lv002) lv002,IF(lv001='$vSelectID',1,0) lv003 from  ac_lv0115";
				break;		
			case 'lv010':
				$vsql="select lv001,concat(lv001,' ',lv002) lv002,IF(lv001='$vSelectID',1,0) lv003 from  ac_lv0116";
				break;	
			default:
				break;
		}
	...
	public  function getvaluelink($vFile,$vSelectID)
	{
		switch($vFile)
		{
			...
			case 'lv008':
				...
				break;
			case 'lv009':
				$vsql="select lv001, lv002,IF(lv001='$vSelectID',1,0) lv003 from  ac_lv0115 where lv001='$vSelectID'";	
				break;
			case 'lv010':
				$vsql="select lv001, lv002,IF(lv001='$vSelectID',1,0) lv003 from  ac_lv0116 where lv001='$vSelectID'";	
				break;
			default:
				$vsql ="";
				break;
