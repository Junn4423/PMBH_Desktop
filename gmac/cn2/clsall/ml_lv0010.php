<?php
/////////////coding ml_lv0010///////////////
class   ml_lv0010 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
...
////////////
	...
	var $ArrView=array("lv001"=>"0","lv002"=>"0","lv003"=>"1","lv004"=>"3","lv005"=>"0","lv006"=>"2");	
	...
	{
		$lvsql="select lv011 from sl_lv0010 B where  B.lv001='$this->lv002'";
	...
	{
		$lvsql="select * from  ml_lv0010 Where lv001='$vlv001'";
	...
	function LV_LoadUserID($vlv002)
	{
		...
		$lvsql="select lv005,lv003 from  ml_lv0010 Where lv002='$vlv002'";
		$vresult=db_query($lvsql);
		while($vrow = db_fetch_array ($vresult))
		{
			...
			if($strReturn=="") $strReturn="<tr bordercolor=#999999><td align=left>".$this->lv005."</td><td align=right>".$this->LV_GetByte($this->lv003)."</td></tr>";
			else
			$strReturn=$strReturn."<tr bordercolor=#999999><td align=left>".$this->lv005."</td><td align=right>".$this->LV_GetByte($this->lv003)."</td></tr>";
		}
		if($strReturn!="") $strReturn=$strReturn."<tr bordercolor=#999999><td align=left>&nbsp;"."</td><td align=right>".$this->LV_GetByte($strTotal)."</td></tr>";
	...
	function LV_LoadUserArray($vlv002)
	{
		$strReturn="";
		$strTotal=0;
		$lvsql="select * from  ml_lv0010 Where lv002='$vlv002'";
		$vresult=db_query($lvsql);
		return $vresult;
	}
	function LV_GetByte($vByte)
	{
		if($vByte>=1048576) return round($vByte/1048576,3)."MB";
		elseif($vByte>=1024) return round($vByte/1024,2)."KB";
		else return  ($vByte)."Byte";
	}
	function LV_Insert()
	{
	...
	//	$this->lv005 = ($this->lv005!="")?recoverdate(($this->lv005), $this->lang):$this->DateDefault;
		$lvsql="insert into ml_lv0010 (lv001,lv002,lv003,lv004,lv005,lv006) values('$this->lv001','$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006')";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ml_lv0010.insert',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_Update()
	{
		if($this->isEdit==0) return false;
		 $this->lv005 = ($this->lv005!="")?recoverdate(($this->lv005), $this->lang):$this->DateDefault;
		 $lvsql="Update ml_lv0010 set lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004',lv005='$this->lv005',lv006='$this->lv006' where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ml_lv0010.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Delete($lvarr)
	{
		if($this->isDel==0) return false;
		$lvsql = "DELETE FROM ml_lv0010  WHERE ml_lv0010.lv001 IN ($lvarr)";// and (select count(*) from ml_lv0010 B where  B.lv002= ml_lv0010.lv001)<=0  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ml_lv0010.delete',sof_escape_string($lvsql));
		return $vReturn;
	}	
		function LV_DeleteUser($vUserID)
	{
	if($this->isDel==0) return false;
		$lvsql = "DELETE FROM ml_lv0010  WHERE ml_lv0010.lv002='$vUserID'";// and (select count(*) from ml_lv0010 B where  B.lv002= ml_lv0010.lv001)<=0  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'ml_lv0010.delete',sof_escape_string($lvsql));
		return $vReturn;
	}
	...
	function LV_GetForward($AttachFile,$AttachFileHidden)
	{
		...
		{
			$strTemp=$strTr;
			$this->lv001=InsertWithCheckExt('ml_lv0010', 'lv001', '',0);
			...
			$this->lv004="images/human/File/MailTemp/";
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM ml_lv0010 WHERE 1=1 ".$this->GetCondition();
	...
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT * FROM ml_lv0010 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('$this->Dir'+'ml_lv0010/?lang=".$this->lang."&childdetailfunc='+value+'&ID=".base64_encode($this->lv002)."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td align=@#05>@02</td>";
		$sqlS = "SELECT * FROM ml_lv0010 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
	...
	private function sqlcondition($vFile,$vSelectID)
	{
		...
		{
			case 'lv002':
				$vsql="select lv001,lv004 lv002,IF(lv001='$vSelectID',1,0) lv003 from  all_gmacv3_0.lv_lv0007";
				break;
		}
		return $vsql;
	}
	private  function getvaluelink($vFile,$vSelectID)
	{
		...
		{
			case 'lv002':
				$vsql="select lv001,lv004 lv002,IF(lv001='$vSelectID',1,0) lv003 from  all_gmacv3_0.lv_lv0007 where lv001='$vSelectID'";
				break;
			default:
				$vsql ="";
				break;
