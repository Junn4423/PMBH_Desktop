  ...
  // Socket Vars
    ...
    */
    ...
    /*
      ...
      Access: Public
    */
    function get_checkattach($response)
    {
     if(eregi("Content-Transfer-Encoding:",$response)) return true;
     if(eregi("Attachment",$response))  return true;
     if(eregi("filename",$response))  return true;
     if(eregi("Content-Type",$response))  return true;
     if(eregi("name=",$response))  return true; 
     return false;  		
    }
        ...
        for($i=0;$i<count($message);$i++){
            ...
            if($str_len>0)
            {
	            ...
	            if(!fputs($this->file_fp,$line,$str_len)){
	            	echo "<br>vinh<br><textarea>".$str_len.'vinh'.$line."</textarea><br>vinh";
