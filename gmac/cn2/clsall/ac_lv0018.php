<?php
/////////////coding ac_lv0018///////////////
class   ac_lv0018 extends lv_controler
{
	public $lv001=null;
	public $lv001_=null;
	public $lv002=null;
...
////////////
	...
	var $ArrView=array("lv001"=>"0","lv002"=>"0","lv003"=>"0","lv004"=>"0","lv005"=>"0","lv006"=>"0","lv007"=>"0","lv008"=>"0","lv009"=>"0","lv010"=>"0","lv011"=>"0","lv012"=>"0","lv013"=>"0","lv014"=>"0","lv015"=>"0","lv016"=>"0","lv017"=>"10","lv018"=>"0","lv022"=>"0");	
	...
	function LV_LoadList($vContractID)
	{
		$lvsql="select * from  ac_lv0004 Where lv013='$vContractID'";
		$vresult=db_query($lvsql);
		return $vresult;
	}
	function LV_Insert()
	...
	{
		$lvsql = "select (select if(ISNULL(sum(lv004)),0,sum(lv004)) from ac_lv0005 A  WHERE A.lv002 ='$vVoiceID' and A.lv005='$vAccount' and A.lv005<>A.lv006) AddMN ,(select if(ISNULL(sum(lv004)),0,sum(lv004)) from ac_lv0005 A  WHERE A.lv002 ='$vVoiceID' and A.lv006='$vAccount' and A.lv005<>A.lv006) SubMN,((select if(ISNULL(sum(lv004)),0,sum(lv004)) from ac_lv0005 A  WHERE A.lv002 ='$vVoiceID' and A.lv005='$vAccount' and A.lv005<>A.lv006)-(select if(ISNULL(sum(lv004)),0,sum(lv004)) from ac_lv0005 A  WHERE A.lv002 ='$vVoiceID' and A.lv006='$vAccount' and A.lv005<>A.lv006)) SumSub, B.lv005,B.lv006 from ac_lv0005 B where B.lv002 ='$vVoiceID' and (B.lv005='$vAccount' or B.lv006='$vAccount') and B.lv005<>B.lv006 ";
			$vReturnArr=array();
	...
	{
		$lvsql = "select if(ISNULL(sum(lv004)),0,sum(lv004)) SumAll from ac_lv0005 A  WHERE A.lv002 ='$vVoiceID' and A.lv005='$vAccount' and A.lv006='$TK' and A.lv005<>A.lv006";
		$vReturnArr=array();
		$lvResult= db_query($lvsql);
		$vflag=1;
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('ac_lv0018/?lang=".$this->lang."&func='+value,'','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
";
	...
	function LV_BuilListReportOther($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		while ($vrow = db_fetch_array ($bResult)){
			...
			{
				if($lstArr[$i]=="lv017")
					$vTemp=str_replace("@02",$this->getvaluelink($lstArr[$i],$this->FormatView($this->LV_GetMN_InVoice($vrow['lv001'],$vrow['lv010'],4),(int)$this->ArrView[$lstArr[$i]])),$this->Align($lvTd,(int)$this->ArrView[$lstArr[$i]]));
				else
				{
