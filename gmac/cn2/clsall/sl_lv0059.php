...
////////////
	...
	function LV_Load()
	{
		$vsql="select * from  all_gmacv3_0.sl_lv0059";
	...
	function LV_ConfirmProgramCus($vCusID)
	{
		...
		$lvsql="select lv022 from all_gmacv3_0.sl_lv0001 A  where A.lv001='$vCusID'";
		...
		if($vNhomCus!='')
		{
			$lvsql="select A.lv001 from  all_gmacv3_0.sl_lv0059 A  where concat(',',A.lv099,',') like concat('%,','$vNhomCus',',%') and A.lv008=1 and (date(A.lv003)<=CURRENT_DATE() and date(A.lv004)>=CURRENT_DATE()) and (time(A.lv003)<=CURRENT_TIME() and time(A.lv004)>=CURRENT_TIME())";
			$vresult=db_query($lvsql);
			$vrow=db_fetch_array($vresult);
			if($vrow)
			{
	...
	{
		$lvsql="select A.*,count(A.lv001) NumLine from  all_gmacv3_0.sl_lv0059 A  where A.lv099='' and A.lv008=1 and (date(A.lv003)<=CURRENT_DATE() and date(A.lv004)>=CURRENT_DATE()) and (time(A.lv003)<=CURRENT_TIME() and time(A.lv004)>=CURRENT_TIME())";
		$vresult=db_query($lvsql);
		$vrow=db_fetch_array($vresult);
		if($vrow)
		{
	...
	{
		$lvsql="select * from  all_gmacv3_0.sl_lv0059 Where concat(',',lv099,',') like '%,$vlv022,%'";
		$vresult=db_query($lvsql);
		$vrow=db_fetch_array($vresult);
		if($vrow)
		{
	...
	{
		$lvsql="select * from  all_gmacv3_0.sl_lv0059 Where lv001='$vlv001'";
		$vresult=db_query($lvsql);
		$vrow=db_fetch_array($vresult);
		if($vrow)
		{
	...
	function LV_Delete($lvarr)
	{
		...
		$lvsql = "DELETE from all_gmacv3_0.sl_lv0059  WHERE sl_lv0059.lv001 IN ($lvarr) and sl_lv0059.lv008=0   ";
		$vReturn= db_query($lvsql);
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums from all_gmacv3_0.sl_lv0059 WHERE 1=1 ".$this->GetCondition();
	...
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT * from all_gmacv3_0.sl_lv0059 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
	...
	function LV_BuilListReportMini($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvDateSort)
	{
		...
		$sqlS = "SELECT * from all_gmacv3_0.sl_lv0059 WHERE 1=1  ".$this->GetConditionMini()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$sqlS = "SELECT * from all_gmacv3_0.sl_lv0059 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
...
//////////////////////Buil list////////////////////
	function LV_BuilListReportStartEndDate($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvEmployeeID,$lvStartDate,$lvEndDate)
	{
		...
		$sqlS = "SELECT * from all_gmacv3_0.sl_lv0059 WHERE 1=1 $condition ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
