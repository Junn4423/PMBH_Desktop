...
////////////
	...
	{
		$lvsql="select sum(PM.lv003) money,sum(PM.lv004) convertmoney from ((select sum(A.lv004*A.lv006) lv003,sum(A.lv004*A.lv006*A.lv008/100) lv004 from sl_lv0014 A inner join sl_lv0013 B on A.lv002=B.lv001 inner join all_gmacv3_0.sl_lv0007 C on A.lv003=C.lv001  where C.lv099=0 and B.lv002='$vCustomerID' AND B.lv011>=1 and Year(B.lv004)='$vYear' )) PM ";	
		$vresult=db_query($lvsql);
		$vrow = db_fetch_array ($vresult);
	...
	function LV_GetDetail($vContractID,$vBangID,$vLangArr,&$vSum)
	{
		...
		';		
		$lvsql="select A.*,B.lv022 CKTong,C.lv002 Name,C.lv009 ColorName from sl_lv0014 A inner join sl_lv0013 B on A.lv002=B.lv001 inner join all_gmacv3_0.sl_lv0007 C on A.lv003=C.lv001 where 1=1 and B.lv001='$vContractID' AND A.lv004>0 order by A.lv018,A.lv001";	
		$vresult=db_query($lvsql);
		$i=1;
		$vSum=0;
		$vSumTT=0;
		...
		$lvsql="select A.*,B.lv022 CKTong,C.lv002 Name,C.lv009 ColorName from sl_lv0014 A inner join sl_lv0013 B on A.lv002=B.lv001 inner join all_gmacv3_0.sl_lv0007 C on A.lv003=C.lv001 where 1=1 and B.lv001='$vContractID' and A.lv004<0";	
		$vresult=db_query($lvsql);
		$vFirts=true;
		while($vrow = db_fetch_array ($vresult))
		{
	...
	{
		$vResult='';
		$lvsql = "select B.lv015 from sl_lv0014 B inner join all_gmacv3_0.sl_lv0001 A on A.lv001=B.lv015 where B.lv002='$vContractID'";
		$lvResult= db_query($lvsql);
		while($row= db_fetch_array($lvResult))
		{
			if($vResult=="")
