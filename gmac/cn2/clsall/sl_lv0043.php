<?php
/////////////coding sl_lv0043///////////////
class   sl_lv0043 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
	public $lv003=null;
	...
	public $lv006=null;
///////////
	public $DefaultFieldList="lv001,lv002,lv003,lv004,lv005,lv006,lv007,lv008,lv009";	
////////////////////GetDate	
	...
	public $paging=null;
	public $lang=null;
	protected $objhelp='sl_lv0043';
	public $obj_detail=null;
	public $obj_config=null;
	public $obj_contract=null;
////////////
	var $ArrOther=array();
	var $ArrPush=array();
	var $ArrFunc=array();
	var $ArrGet=array("lv001"=>"2","lv002"=>"3","lv003"=>"4","lv004"=>"5","lv005"=>"6","lv006"=>"7","lv007"=>"8","lv008"=>"9","lv009"=>"10");
	var $ArrView=array("lv001"=>"0","lv002"=>"0","lv003"=>"2","lv004"=>"2","lv005"=>"1","lv006"=>"10","lv007"=>"2","lv008"=>"10","lv009"=>"10");	
	...
	{
		$lvsql="select * from  sl_lv0043 Where lv001='$vlv001'";
	...
	{
		$lvsql="select * from  sl_lv0043 Where lv006=1";
		$vresult=db_query($lvsql);
		$vrow=db_fetch_array($vresult);
		if($vrow)
		{
	...
	function LV_Insert()
	{
		...
		$this->lv003 = ($this->lv003!="")?recoverdate(($this->lv003), $this->lang):$this->DateDefault;
		...
		$lvsql="insert into sl_lv0043 (lv001,lv002,lv003,lv004,lv005,lv006,lv007,lv008) values('$this->lv001','$this->lv002','$this->lv003','$this->lv004','$this->lv005','$this->lv006','$this->lv007','$this->lv008')";
		$vReturn= db_query($lvsql);
		if($vReturn)
		{	
			$this->LV_InsertDetail($this->obj_detail,$this->obj_config,$this->lv003,$this->lv004);
			$this->InsertLogOperation($this->DateCurrent,'sl_lv0043.insert',sof_escape_string($lvsql));
		}
		return $vReturn;
	}	
	function LV_Update()
	{
		...
		$this->lv004 = ($this->lv004!="")?recoverdate(($this->lv004), $this->lang):$this->DateDefault;
		$this->lv007 = ($this->lv007!="")?recoverdate(($this->lv007), $this->lang):$this->DateDefault;
		$lvsql="Update sl_lv0043 set lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004',lv006='$this->lv006',lv007='$this->lv007',lv008='$this->lv008' where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'sl_lv0043.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Delete($lvarr)
	{
		if($this->isDel==0) return false;
		$lvsql = "DELETE FROM sl_lv0043  WHERE sl_lv0043.lv001 IN ($lvarr) and sl_lv0043.lv006<=0   ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'sl_lv0043.delete',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_Aproval($lvarr)
	{
		...
		echo $lvsql = "Update sl_lv0043 set lv006=IF(lv006<2,lv006+1,2) WHERE sl_lv0043.lv001 IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'sl_lv0043.approval',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_UnAproval($lvarr)
	{
		...
		$vUserID=getInfor($_SESSION['ERPSOFV2RUserID'], 2);
		$lvsql = "Update sl_lv0043 set lv006=IF(lv006>0,lv006-1,0) WHERE sl_lv0043.lv001 IN ($lvarr)  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'sl_lv0043.unapproval',sof_escape_string($lvsql));
		return $vReturn;
	}
	//////////get view///////////////
	function GetView()
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM sl_lv0043 WHERE 1=1 ".$this->GetCondition();
		$bResultC = db_query($sqlC);
		$arrRowC = db_fetch_array($bResultC);
		return $arrRowC['nums'];
	}
	function LV_GetContractMoney($vContractID)
	{
		$lvsql="select sum(PM.lv003) money,sum(PM.lv004) convertmoney from ((select sum(A.lv004*A.lv006) lv003,sum(A.lv004*A.lv006*A.lv008/100) lv004 from sl_lv0014 A inner join sl_lv0043 B on A.lv002=B.lv001  where 1=1 and B.lv010='$vContractID' AND B.lv011=1 )) PM ";	
		$vresult=db_query($lvsql);
		$vrow = db_fetch_array ($vresult);
		$lvsql="select sum(PM.lv003) money,sum(PM.lv004) convertmoney from ((select sum(A.lv004*A.lv008) lv003,sum(A.lv004*A.lv008*A.lv010/100) lv004 from wh_lv0011 A inner join wh_lv0010 B on A.lv002=B.lv001  where 1=1 and B.lv003='$vContractID' AND B.lv007=1 )) PM ";	
		$vresult1=db_query($lvsql);
		$vrow1 = db_fetch_array ($vresult1);
		return $vrow['convertmoney']+$vrow['money']+$vrow1['convertmoney']+$vrow1['money'];
	}
	function LV_GetAllMoney($vCommissionID)
	{
		$lvsql="select sum(lv007) money from sl_lv0044 where 1=1 and lv002='$vCommissionID' ";	
		$vresult=db_query($lvsql);
		$vrow = db_fetch_array ($vresult);
		return $vrow['money'];
	}
	function LV_GetAllCount($vCommissionID)
	{
		$lvsql="select count(*) nums from sl_lv0044 where 1=1 and lv002='$vCommissionID' ";	
		$vresult=db_query($lvsql);
		$vrow = db_fetch_array ($vresult);
		return $vrow['nums'];
	}
	function GetPercent($vSalaray)//Get percent of tax
	{
		$vReturn=0;
		$i=0;
		$vFrom[0]=0;
		$vTo[0]=0;		
		$vPerCal[0]=0;
		$vsql="select * from  tc_lv0005 A where A.lv002<$vSalaray Order by lv004 Desc ";
		$vresult=db_query($vsql);
	...
	function LV_InsertDetail($obj_detail,$obj_config,$vDateFrom,$vDateTo)
	{
		$vsql="select * from all_gmacv3_0.sl_lv0001";
		$bResult=db_query($vsql);
		while ($vrow = db_fetch_array ($bResult)){
			$obj_detail->lv002=	$this->lv001;
			$obj_detail->lv003=	$vrow['lv001'];
			$obj_detail->lv007=$this->obj_contract->LV_GetContractMoneyCurFrom($vrow['lv001'],0,$vDateFrom,$vDateTo);
			$obj_detail->lv004=	$this->GetPercent($obj_detail->lv007);//diem ban
			$obj_detail->lv005=	$obj_detail->lv004;//diem giới thiệu
			$obj_detail->lv006=	$obj_detail->lv004;//diem người quản lý		
			if($obj_detail->lv007>0)
			{
	...
	function LV_GetContractMoneyDate($vContractID,$StartDate,$EndDate)
	{
		...
		$lvsql="select sum(PM.lv003) money,sum(PM.lv004) convertmoney from ((select sum(A.lv004*A.lv006) lv003,sum(A.lv004*A.lv006*A.lv008/100) lv004 from sl_lv0014 A inner join sl_lv0043 B on A.lv002=B.lv001  where 1=1 and B.lv010='$vContractID' AND B.lv011=1 $condition1 )) PM ";	
		$vresult=db_query($lvsql);
		$vrow = db_fetch_array ($vresult);
		$lvsql="select sum(PM.lv003) money,sum(PM.lv004) convertmoney from ((select sum(A.lv004*A.lv008) lv003,sum(A.lv004*A.lv008*A.lv010/100) lv004 from wh_lv0011 A inner join wh_lv0010 B on A.lv002=B.lv001  where 1=1 and B.lv003='$vContractID' AND B.lv007=1 $condition2)) PM ";	
		$vresult1=db_query($lvsql);
		$vrow1 = db_fetch_array ($vresult1);
		return $vrow['convertmoney']+$vrow['money']+$vrow1['convertmoney']+$vrow1['money'];
	}
	...
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT * FROM sl_lv0043 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		...
		while ($vrow = db_fetch_array ($bResult)){
			...
			for($i=0;$i<count($lstArr);$i++)
			{
				if($lstArr[$i]=="lv009")
				{
					...
					$vTemp=str_replace("@02",str_replace("@02",$this->getvaluelink($lstArr[$i],$this->FormatView($this->LV_GetAllMoney($vrow['lv001']),(int)$this->ArrView[$lstArr[$i]])),str_replace("@01",$vrow['lv001'] ,$lvHref)),$this->Align($lvTd,(int)$this->ArrView[$lstArr[$i]]));
				}
				elseif($lstArr[$i]=="lv005")
				{
					
					$vTemp=str_replace("@02",str_replace("@02",$this->getvaluelink($lstArr[$i],$this->FormatView($this->LV_GetAllCount($vrow['lv001']),(int)$this->ArrView[$lstArr[$i]])),str_replace("@01",$vrow['lv001'] ,$lvHref)),$this->Align($lvTd,(int)$this->ArrView[$lstArr[$i]]));
				}
				else
		...
		while ($vrow = db_fetch_array ($bResult)){
			...
			for($i=0;$i<count($lstArr);$i++)
			{
				if($lstArr[$i]=="lv009")
				{
					...
					$vTemp=str_replace("@02",$this->getvaluelink($lstArr[$i],$this->FormatView($this->LV_GetAllMoney($vrow['lv001']),(int)$this->ArrView[$lstArr[$i]])),$this->Align($lvTd,(int)$this->ArrView[$lstArr[$i]]));
				}
				else
...
/////////////////////ListFieldExport//////////////////////////
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('sl_lv0043/?lang=".$this->lang."&func='+value,'','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td align=@#05>@02</td>";
		$sqlS = "SELECT * FROM sl_lv0043 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
		...
		while ($vrow = db_fetch_array ($bResult)){
			...
			for($i=0;$i<count($lstArr);$i++)
			{
				if($lstArr[$i]=="lv009")
				{
					$vTemp=str_replace("@02",$this->getvaluelink($lstArr[$i],$this->FormatView($this->LV_GetContractMoney($vrow['lv002']),(int)$this->ArrView[$lstArr[$i]])),$this->Align($lvTd,(int)$this->ArrView[$lstArr[$i]]));
				}
				else
				$vTemp=str_replace("@02",$this->getvaluelink($lstArr[$i],$this->FormatView($vrow[$lstArr[$i]],(int)$this->ArrView[$lstArr[$i]])),$this->Align($lvTd,(int)$this->ArrView[$lstArr[$i]]));
				$strL=$strL.$vTemp;
...
//////////////////////Buil list////////////////////
	function LV_BuilListReportStartEndDate($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvEmployeeID,$lvStartDate,$lvEndDate)
	{
		...
		$sqlS = "SELECT * FROM sl_lv0043 WHERE 1=1 $condition ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
		...
		while ($vrow = db_fetch_array ($bResult)){
			...
			for($i=0;$i<count($lstArr);$i++)
			{
				if($lstArr[$i]=="lv009")
				{
					$vTemp=str_replace("@02",$this->getvaluelink($lstArr[$i],$this->FormatView($this->LV_GetContractMoneyDate($vrow['lv002'],$lvStartDate,$lvEndDate),(int)$this->ArrView[$lstArr[$i]])),$this->Align($lvTd,(int)$this->ArrView[$lstArr[$i]]));
				}
				else
				$vTemp=str_replace("@02",$this->getvaluelink($lstArr[$i],$this->FormatView($vrow[$lstArr[$i]],(int)$this->ArrView[$lstArr[$i]])),$this->Align($lvTd,(int)$this->ArrView[$lstArr[$i]]));
				$strL=$strL.$vTemp;
