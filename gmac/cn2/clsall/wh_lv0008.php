...
////////////
	...
	{
		$lvsql="select A.lv004 lv004 from sl_lv0014 A inner join  all_gmacv3_0.sl_lv0007 B on A.lv003=B.lv001  where A.lv002='$vlv002'";
		$vReturn= db_query($lvsql);
	...
	function LV_AprovalCreateLot($lvarr)
	{
		...
		if($vReturn)
		{
			...
			while($vrow=db_fetch_array($vReturn1))
			{
				...
				if($this->objwh_lv0001!=null) $this->objwh_lv0001->LV_ApprovalItem($vrow['WhID'],$vrow['lv003']);
			} 
		}
		return $vReturn;
	}	
	...
	function LV_GetDetail($vContractID,$vBangID,$vLangArr,&$vSum)
	{
		...
		';		
		$lvsql="select A.*,D.lv002 UnitName,C.lv002 Name,C.lv009 ColorName from wh_lv0009 A inner join wh_lv0008 B on A.lv002=B.lv001 inner join  all_gmacv3_0.sl_lv0007 C on A.lv003=C.lv001 left join sl_lv0005 D on C.lv004=D.lv001 where 1=1 and B.lv001='$vContractID'";	
		$vresult=db_query($lvsql);
		$i=1;
		$vSum=0;
		$vSumTT=0;
