<?php
/////////////coding lv_lv0004///////////////
class   lv_lv0004 extends lv_controler
{
	public $lv001=null;
	public $lv002=null;
///////////
...
////////////
	...
	{
		$lvsql="select * from  lv_lv0004 Where lv001='$vlv001'";
	...
	function LV_Insert()
	{
		...
		$lvsql="insert into lv_lv0004 (lv001,lv002) values('$this->lv001','$this->lv002')";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'lv_lv0004.insert',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_Update()
	{
		if($this->isEdit==0) return false;
		 $lvsql="Update lv_lv0004 set lv002='$this->lv002' where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'lv_lv0004.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Delete($lvarr)
	{
		if($this->isDel==0) return false;
		 $lvsql = "DELETE FROM lv_lv0004  WHERE lv_lv0004.lv001 IN ($lvarr) ";//and (select count(*) from lv_lv0004 B where  B.lv002= lv_lv0004.lv001)<=0  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,'lv_lv0004.delete',sof_escape_string($lvsql));
		return $vReturn;
	}	
	//////////get view///////////////
	function GetView()
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM lv_lv0004 WHERE 1=1 ".$this->GetCondition();
	...
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging)
	{
		...
		$sqlS = "SELECT * FROM lv_lv0004  WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
