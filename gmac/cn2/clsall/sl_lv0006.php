...
////////////////////GetDate
	...
	public $paging=null;
	public $lang=null;
	protected $objhelp=' all_gmacv3_0.sl_lv0006';
	public $Dir="";
////////////
	var $ArrOther=array();
	var $ArrPush=array();
	...
	{
		$vsql="select * from   all_gmacv3_0.sl_lv0006 where lv001='$vCateID' and (lv003='' or lv001=lv001)";
	...
	{
		$vsql="select * from   all_gmacv3_0.sl_lv0006 where lv003='$vCateID'";
		$vresult=db_query($vsql);
		while($vrow=db_fetch_array($vresult))
		{
				$str_return=$str_return.",'".$vrow['lv001']."'".$this->LV_CheckChildCate($vrow['lv001']);
	...
	{
		$lvsql="select * from   all_gmacv3_0.sl_lv0006 Where lv001='$vlv001'";
	...
	function LV_Insert()
	{
		...
		$lvsql="insert into  all_gmacv3_0.sl_lv0006 (lv001,lv002,lv003,lv004,lv005) values('$this->lv001','$this->lv002','$this->lv003','$this->lv004','$this->lv005')";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,' all_gmacv3_0.sl_lv0006.insert',sof_escape_string($lvsql));
		return $vReturn;
	}	
	function LV_Update()
	{
		if($this->isEdit==0) return false;
		$lvsql="Update  all_gmacv3_0.sl_lv0006 set lv002='$this->lv002',lv003='$this->lv003',lv004='$this->lv004',lv005='$this->lv005' where  lv001='$this->lv001';";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,' all_gmacv3_0.sl_lv0006.update',sof_escape_string($lvsql));
		return $vReturn;
	}
	function LV_Delete($lvarr)
	{
		if($this->isDel==0) return false;
		$lvsql = "DELETE FROM  all_gmacv3_0.sl_lv0006  WHERE  all_gmacv3_0.sl_lv0006.lv001 IN ($lvarr)";// and (select count(*) from  all_gmacv3_0.sl_lv0006 B where  B.lv002=  all_gmacv3_0.sl_lv0006.lv001)<=0  ";
		$vReturn= db_query($lvsql);
		if($vReturn) $this->InsertLogOperation($this->DateCurrent,' all_gmacv3_0.sl_lv0006.delete',sof_escape_string($lvsql));
		return $vReturn;
	}	
	/////lv admin deletet
	...
	function GetCount()
	{
		$sqlC = "SELECT COUNT(*) AS nums FROM  all_gmacv3_0.sl_lv0006 WHERE 1=1 ".$this->GetCondition();
	...
	function LV_BuilList($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList,$lvSortNum)
	{
		...
		$sqlS = "SELECT * FROM  all_gmacv3_0.sl_lv0006 WHERE 1=1  ".$this->GetCondition()." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
	...
	function ListFieldExport($lvFrom,$lvList,$maxRows)
	{
		...
		{
			window.open('".$this->Dir." all_gmacv3_0.sl_lv0006/?lang=".$this->lang."&func='+value+'&ID=".base64_encode($this->lv002)."','','width=800,height=600,left=200,top=100,screenX=0,screenY=100,resizable=yes,status=no,scrollbars=yes,menubar=yes');
		}
...
//////////////////////Buil list////////////////////
	function LV_BuilListReport($lvList,$lvFrom,$lvChkAll,$lvChk,$curRow, $maxRows,$paging,$lvOrderList)
	{
		...
		$lvTdH="<td width=\"@01\" class=\"lvhtable\">@02</td>";
		$lvTd="<td align=@#05>@02</td>";
		$sqlS = "SELECT * FROM  all_gmacv3_0.sl_lv0006 WHERE 1=1  ".$this->RptCondition." $strSort LIMIT $curRow, $maxRows";
		$vorder=$curRow;
		$bResult = db_query($sqlS);
		$this->Count=db_num_rows($bResult);
		$strTrH="";
