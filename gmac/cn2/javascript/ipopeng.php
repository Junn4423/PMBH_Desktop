...
<html>
<head>
<title>PopCalendarXP 9.4.258 Lite Edition - Copyright 2003-2005 Idemfactor Solutions, Inc.</title>
...
var tlang='<?php echo strtolower($_GET['lang']);?>';
if(tlang=='') tlang='en';
...
function fParseDate(ds,tlang) {
	...
	if (pd.length==3) {
		...
		for (i=0; (i<12)&&(gMonths[tlang][i].substring(0,3).toLowerCase()!=m.substring(0,3).toLowerCase())&&(i+1!=m); i++);
		if (i<12) {
...
function fFormatDate(y,m,d){
	var M=giMonthMode==0?gbPadZero&&m<10?"0"+m:m:giMonthMode==1?gMonths[tlang][m-1]:gMonths[tlang][m-1].substring(0,giMonthMode);
...
function fDrawCal(y,m) {
	...
	for (var wd=0,i=0; i<8; i++)
       ...
       +y+","+m+","+(giFirstDOW+wd)%7+");return false;'>"+gWeekDay[tlang][wd]+"</a>":"<span class='DoWAnchor'>"+gWeekDay[tlang][wd]+"</span>"
...
with (document) {
	...
	if (giDCStyle==2)
		...
		for (var i=0; i<12; i++) mstr.push("<OPTION value='",i+1,"'>",gMonths[tlang][i],"</OPTION>");
