...
var giDatePos=((tlang=='vn')?0:((tlang=='en')?1:2));	// date format sequence  0: D-M-Y ; 1: M-D-Y; 2: Y-M-D
...
var	gMonths = {
		vn : new Array('Tháng một','Tháng hai','Tháng ba','Tháng tư','Tháng năm','Tháng sáu','Tháng bảy','Tháng tám','Tháng chín','Tháng mười','Tháng mười một','Tháng mười hai'),
		en : new Array('Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'),
		lv : new Array('Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec')
	};
var gWeekDay={
			vn : new Array('T2','T3','T4','T5','T6','T7','CN'),
			en : new Array('Mo','Tu','We','Th','Fr','Sa','Su'),
			lv : new Array('Mo','Tu','We','Th','Fr','Sa','Su'),
			};
var gButtonLang=
{
	vn:new Array('Hôm nay','Ngày','Năm'),
	en:new Array('Today','',''),
	lv:new Array('Today','','')
}
...
var gcCalBG="FFB121";	// the background color of the outer calendar panel.
var guCalBG=null;	//  the background image url for the inner table.
var gcCalFrame="#AAAAAA";	// the background color of the inner table, showing as a frame.
...
var gsCalTitle="gMonths['"+tlang+"']["+"gCurMonth[1]-1]+' '+gCurMonth['"+tlang+"']["+"0]";	// dynamic statement to be eval()'ed as the title when giDCStyle>0.
...
var gstrButton='';
//Thao tác gán tiếng việc
if(tlang=='vn')
{
	gstrButton=gButtonLang[tlang][0]+" : "+gToday[2]+"/"+gToday[1]+"/"+gToday[0];
}
else
{
	gstrButton=gButtonLang[tlang][0]+" : "+gToday[1]+"/"+gToday[2]+"/"+gToday[0];
}
var gsBottom="<A href='javascript:void(0)' class='BottomAnchor' onclick='if(this.blur)this.blur();if(!fSetDate(gToday[0],gToday[1],gToday[2]))alert(\"You cannot select today!\");return false;' onmouseover='return true;' >"+gstrButton+"</A>";	// the content of the bottom section TND.
