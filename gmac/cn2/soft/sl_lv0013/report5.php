...
<table style="width:100%;max-with:375px;" border="0" cellspacing="0" cellpadding="0" align="center">
	<?php echo ($mosl_lv0013->lv011>0)?"<tr><td><img style='max-width:375px;width:100%;height:50px;' src='../../clsall/barcode/barcode.php?barnumber=".$mosl_lv0013->lv001."'></td></tr>":"<tr><td height=5></td></tr>";?>
   <tr>
    <td align="center" onDblClick="this.innerHTML=''">
	<div style="clear:both;"> 
		<div style="float:left;width:85%;text-align:left;">
		
			<strong><span style="font:18px arial,tahoma"><?php echo $mosl_lv0014->GetCompany();?></span></strong>
			<br><span style="font:12px arial,tahoma"><?php echo 'ĐC: ';?><?php echo $mosl_lv0014->GetAddress();?></span>
			<br><span style="font:12px arial,tahoma"><?php echo 'ĐT: ';?><?php echo $mosl_lv0014->GetPhone();?>  </span>
  ...
  <tr>
    <td align="center">
	<div id='idphieu'  style="font-size:18px;font-weight:bold">PHIẾU THU</div></td>
  </tr>
   <tr>
    <td align="center">
	<?php echo $mosl_lv0013->getvaluelink('lv077',$mosl_lv0013->lv007);?>
	</td>
	</tr>
	<tr>
	<td>
	<br/>
			<table width="100%" style="border:0px;text-align:left;" cellpadding="0" cellspacing="0">
				<tr><td>Ngày: <?php echo getday($mosl_lv0013->lv004);?>/<?php echo getmonth($mosl_lv0013->lv004);?>/<?php echo getyear($mosl_lv0013->lv004);?></td><td><strong>Mã phiếu: </strong><?php echo $mosl_lv0013->lv001;?></td></tr>
				<tr><td>In lúc: <?php echo substr(GetServerTime(),0,5);?></td><td>NV TT: <?php echo $mosl_lv0013->lv032;?> NV TĐ:	<?php echo $mohr_lv0020->lv001;?></td>
				<tr><td><strong>Giờ vào: </strong> <?php echo substr($mosl_lv0013->lv004,11,5);?> </td><td> Giờ ra: <?php echo substr($mosl_lv0013->lv005,11,5);?></td></tr>
				<?php
				...
				<tr>
					<td>
						<strong>KH: </strong><?php echo $mosl_lv0001->lv002;?>   <strong>
					</td>
					<td>
							Mã KH: </strong><?php echo $mosl_lv0001->lv001;?> <strong>
					</td>
				</tr>
				<?php
				$vSum=$mosl_lv0013->LV_GetContractMoneyCustomerDate($mosl_lv0001->lv001,$lvsl_lv0070->lv015,$lvsl_lv0070->lv016)+$mosl_lv0013->LV_GetContractMoneyCustomerDetailDate($mosl_lv0001->lv001,$lvsl_lv0070->lv015,$lvsl_lv0070->lv016);
				$vSumPoint=round($vSum/$lvsl_lv0070->lv009,0);
				$vSumVoucher=$mosl_lv0013->LV_GetPointVoucher($mosl_lv0001->lv001,$lvsl_lv0070->lv015,$lvsl_lv0070->lv016);
				if($vSumPoint!=0 || $vSumVoucher!=0)
				{
				?>
					<tr>
						<td><div ><strong>Điểm lũy kế:</strong>  <?php echo $mosl_lv0013->FormatView($vSumPoint,10);?></td><td><strong>Đổi voucher:</strong> <?php  echo $mosl_lv0013->FormatView($vSumVoucher,10);?>
  ...
  <tr>
    <td align="center"><table align="center" class="lvtable" border="1" cellspacing="0" cellpadding="0">
		<tbody><tr class="lvhtable">
			...
			<td width="" class="lvhtable"><?php echo 'SL';//$vLangArr[22];?> </td>
			...
			{
			?>
			<td width="" class="lvhtable" style="padding-left:5px;padding-right:5px"><?php echo 'Mã KH';?></td>
			<td width="" class="lvhtable" style="padding-left:5px;padding-right:5px"><?php echo 'Điểm lũy kế';?></td>
			<?php
			}
			?>
		</tr>
<?php
	$vsumCash=0;//$mosl_lv0013->LV_GetPTMoney($mosl_lv0013->lv001);
	$vorder=1;
	$sqlS1 = "SELECT sum(A.lv004*A.lv006) GiatUi FROM sl_lv0014 A inner join all_gmacv3_0.sl_lv0007 B on A.lv003=B.lv001 left join sl_lv0005 C on A.lv005=C.lv001 left join sl_lv0059 D on A.lv009=D.lv001 WHERE B.lv003='Giatui' and A.lv002='".$mosl_lv0013->lv001."' order by A.lv015 asc,A.lv001 asc";
	$bResult1 = db_query($sqlS1);
	$vrow1 = db_fetch_array ($bResult1);
	$vGiatUi=$vrow1['GiatUi'];
	$sqlS = "SELECT A.*,B.lv002 ItemName,C.lv002 UnitName,D.lv002 ProgName FROM sl_lv0014 A inner join all_gmacv3_0.sl_lv0007 B on A.lv003=B.lv001 left join sl_lv0005 C on A.lv005=C.lv001 left join sl_lv0059 D on A.lv009=D.lv001 WHERE B.lv003<>'Giatui' and A.lv002='".$mosl_lv0013->lv001."'  order by A.lv015 asc,A.lv001 asc";
	$bResult = db_query($sqlS);
	$sumMoney=0;
	$sumScore=0;
	$sumDiscount=0;
...
?>		
		<tr class="lvlinehtable1">
			...
			<td align="right" style="padding:2px;padding-right:5px;padding-left:5px;font-size:14px"><?php echo $mosl_lv0014->FormatView($vrow['lv006'],10);?></td>
			<?php 
			if($vCount>0){
			?>
			<td align="right" style="padding:2px;padding-right:5px;padding-left:5px;font-size:14px"><?php echo $mosl_lv0014->FormatView($vrow['lv011'],10);?></td>
			<?php
			}
			?>
			<td align="right" style="padding:2px;padding-right:5px;padding-left:5px;font-size:14px"><?php echo $mosl_lv0014->FormatView($vrow['lv004']*$vrow['lv006']-$vrow['lv004']*$vrow['lv006']*$vrow['lv011']/100,10);?></td>
			<?php if($vNumRows>1)
			{
			$vCusID=$vrow['lv015'];
			if(strlen($vCusID)<4)
			{
				$vCusID=$mosl_lv0001->lv001;
			}
			if($vArray[$vCusID]==NULL)
			{
				$vArray[$vCusID]=true;
			?>
			<td align="left"><?php echo $vCusID;?></td>
			<td align="right" style="padding:2px;padding-right:5px;padding-left:5px;font-size:14px"><?php $vSum=$mosl_lv0013->LV_GetContractMoneyCustomerDate($vCusID,$lvsl_lv0070->lv015,$lvsl_lv0070->lv016)+$mosl_lv0013->LV_GetContractMoneyCustomerDetailDate($vCusID,$lvsl_lv0070->lv015,$lvsl_lv0070->lv016);echo $mosl_lv0013->FormatView($vSum/$lvsl_lv0070->lv009,10);?></td>
			<?php
			}
			else
			{
...
?>		
		<tr class="lvlinehtable1">
			<td class="lvlineboldtable" colspan="<?php echo ($vCount>0)?4:3;?>">TỔNG TIỀN:</td>
			<td align="right" style="padding:2px;padding-right:5px;padding-left:5px;font-size:14px"><strong><?php echo $mosl_lv0014->FormatView($sumMoney-$sumDiscount,10);?></strong></td>
			<?php if($vNumRows>1)
			{
			?>
			<td>&nbsp;</td>
		...
		?>
		<tr class="lvlinehtable1">
			<td class="lvlineboldtable" colspan="<?php echo ($vCount>0)?4:3;?>">TỔNG TIỀN PHẢI THU:</td>
		...
		<tr ondblclick="this.innerHTML=''">
			<td class="lvlineboldtable" colspan="<?php echo ($vCount>0)?9:8;?>">Bằng chữ: <?php echo LNum2Text($sumMoney-$vsumCash+$vVAT-$sumDiscount+$vGiatUi-$vCK,$plang);?> </td>
		</tr>
