...
$lvsl_lv0006=new sl_lv0006($_SESSION['ERPSOFV2RRight'],$_SESSION['ERPSOFV2RUserID'],'Sl0006');
$vlsCate=$lvsl_lv0006->LV_CheckCate('NUOC');
$lvsl_lv0070->LV_Load();
...
<table style="width:100%;max-with:375px;" border="0" cellspacing="0" cellpadding="0" align="center">
	<?php echo ($mosl_lv0013->lv011>0)?"<tr><td><img style='width:140px' src='../../clsall/barcode/barcode.php?barnumber=".$mosl_lv0013->lv001."'></td></tr>":"<tr><td height=5></td></tr>";?>
   <tr>
  ...
  <tr>
    <td align="center">
	<div id='idphieu'  style="font-size:18px;font-weight:bold">PHIẾU IN XUỐNG QUẦY PHA CHẾ</div></td>
  	</tr>
  	<tr>
		<td align="center">
	...
	<tr>
		...
		<br/>
			<table width="100%" style="border:0px;text-align:left;" cellpadding="0" cellspacing="0">
				...
				<tr><td>In lúc: <?php echo substr(GetServerTime(),0,5);?></td><td>NV Tạo Đơn:	<?php echo $mohr_lv0020->lv001;?></td>
...
<?php
	...
	$sqlS = "SELECT A.*,B.lv003 CateID,B.lv002 ItemName,C.lv002 UnitName,D.lv002 ProgName FROM sl_lv0014 A inner join all_gmacv3_0.sl_lv0007 B on A.lv003=B.lv001 left join sl_lv0005 C on A.lv005=C.lv001 left join sl_lv0059 D on A.lv009=D.lv001 WHERE B.lv003<>'Giatui' and A.lv002='".$mosl_lv0013->lv001."' and trim(A.lv015)='".trim($vroworder['vorder'])."' order by A.lv015 asc,A.lv001 asc";
	$bResult = db_query($sqlS);
	$sumMoney=0;
	$sumScore=0;
	$sumDiscount=0;
	...
	if($vrow['lv004']>0)
	{
		$mosl_lv0014->LV_UpdatePrint($vrow['lv001']);
		if(strpos($vlsCate,$vrow['CateID'])>0) $vIsNuoc=true;
...
?>		
		<tr class="lvlinehtable1" ondblclick="this.innerHTML=''">
...
?>		
		<tr class="lvlinehtable1"  ondblclick="this.innerHTML=''">
			<td class="lvlineboldtable" style="padding:5px;" colspan="<?php echo ($vCount>0)?3:2;?>"><strong>TỔNG SL:</strong></td>
...
<script>
//window.setTimeout('PrintNuoc()',200);
function PrintNuoc()
{
		var o1=document.frmprocess;
		o1.target="_blank";
		o1.action="../sl_lv0013?&childfunc=rptnuoc&ID=<?php echo $vlv001;?>&lang=<?php echo $plang;?>";
		o1.submit();
}		
</script>	
...
//window.setTimeout('PrintMonAn()',100);
function PrintMonAn()
{
var o2=document.frmprocess1;
		o2.target="_blank";
		o2.action="../sl_lv0013?&childfunc=rptmonan&ID=<?php echo $vlv001;?>&lang=<?php echo $plang;?>";
		o2.submit();
}
		</script>
</body>
