...
////////init object////////////////////
	...
	$lvwh_lv0005=new wh_lv0005($_SESSION['ERPSOFV2RRight'],$_SESSION['ERPSOFV2RUserID'],'Wh0103');	
	$lvsl_lv0070=new sl_lv0070($_SESSION['ERPSOFV2RRight'],$_SESSION['ERPSOFV2RUserID'],'Wh0103');
	$lvsl_lv0059=new sl_lv0059($_SESSION['ERPSOFV2RRight'],$_SESSION['ERPSOFV2RUserID'],'Wh0103');
	$lvsl_lv0001=new sl_lv0001($_SESSION['ERPSOFV2RRight'],$_SESSION['ERPSOFV2RUserID'],'Wh0103');
	$lvsl_lv0007=new sl_lv0007($_SESSION['ERPSOFV2RRight'],$_SESSION['ERPSOFV2RUserID'],'Wh0103');
	$lvmn_lv0005=new mn_lv0005($_SESSION['ERPSOFV2RRight'],$_SESSION['ERPSOFV2RUserID'],'Wh0103');
	$lvwh_lv0034=new wh_lv0034($_SESSION['ERPSOFV2RRight'],$_SESSION['ERPSOFV2RUserID'],'Wh0103');
	$mowh_lv0009=new wh_lv0009($_SESSION['ERPSOFV2RRight'],$_SESSION['ERPSOFV2RUserID'],'Wh0103');
	$mowh_lv0011=new wh_lv0011($_SESSION['ERPSOFV2RRight'],$_SESSION['ERPSOFV2RUserID'],'Wh0103');	
	$mowh_lv0020=new wh_lv0020($_SESSION['ERPSOFV2RRight'],$_SESSION['ERPSOFV2RUserID'],'Wh0103');
	$lvsl_lv0070->LV_Load();
...
if(isset($_GET['ajaxdel']))
{
	...
	$lvwh_lv0004->LV_Delete("'".$vContractID."'");
}	
...
if(isset($_GET['ajaxitemsend']))
{
	...
	if($vContractID!="" && $vContractID!=NULL)
	{
		...
		if($vresult)
		{
			...
			echo $lvwh_lv0004->LV_GetDetailFull($vContractID,$vWHID,$vLangArr,$vSum);
	...
	if($vdonhangid!='' && $vdonhangid!=NULL)
	{
		...
		{			
			case 9:
				...
				break;
			case 5:
				$vsql="update wh_lv0004 set lv005='".recoverdate($vText,$plang)."' where lv001='$vdonhangid' and lv007=0";
				break;
			case 94:
...
{	
	...
	$vsum4=$lvwh_lv0004->LV_GetContractMoney($vdonhangid,4);
	$vsum10=$lvwh_lv0004->LV_GetContractMoney($vdonhangid,10);
		...
		echo '[ENDSOLUONGTHUC]';
		echo '[SOLUONGKHO]';
		echo $lvwh_lv0004->FormatView($vsum4,10);
		echo '[ENDSOLUONGKHO]';
		echo '[SOLUONGKIEM]';
		echo $lvwh_lv0004->FormatView($vsum4,10);
		echo '[ENDSOLUONKIEM]';
...
if(isset($_GET['ajaxaproval']))
{
		...
		echo '[ENDDONHANGNHAN]';
		echo '[EMPTYCONTRACT]';
		echo $lvwh_lv0004->LV_GetDetailFull('','',$vLangArr,$vSum);
...
if(isset($_GET['ajaxdonvi']))
{
	$soluongkiem=$_GET['soluongkiem'];
	$donvi=$_GET['donvi'];
	$vsql="update wh_lv0005 set lv007='$donvi',lv006='$soluongkiem' where lv001='".$_GET['chitietid']."' and (select A.lv007 from wh_lv0004 A where A.lv001=wh_lv0005.lv002)<=0";		
	$vresult=db_query($vsql);
	exit;
}
if(isset($_GET['ajaxslkk']))
{
	$soluongkiem=$_GET['soluongkiem'];
	$soluongthuc=$_GET['soluongthuc'];
	echo '[CHECKDONHANG]';
	$vsql="update wh_lv0005 set lv008='$soluongthuc',lv006='$soluongkiem' where lv001='".$_GET['chitietid']."' and (select A.lv007 from wh_lv0004 A where A.lv001=wh_lv0005.lv002)<=0";		
	$vresult=db_query($vsql);	
...
<script language="javascript" src="<?php echo $vDir;?>../javascript/engine.js"></script>
	...
	function AddProd(value)
	{
		...
		if(o1.value=='Mã kiểm kho') o1.value='';
	...
	{
		var o=document.frmadd;
		ajax_do ('wh_lv0101/wh_lv0101exce.php?&lang=<?php echo $plang;?>&childfunc=load'+'&lv002='+o.txtlv903.value,1);
	...
	function LoadSource()
	{
		...
		switch(vo)
		{
			...
			case 'TRAHANG':
				break;
			case 'MUAHANG':
				ajax_do ('wh_lv0101/wh_lv0101excesource.php?&lang=<?php echo $plang;?>&childfunc=load'+'&lv002='+o.txtlv802.value	+'&lv005='+o.txtlv805.value+'&lv006='+o.txtlv806.value,1);
	...
	function  baocaoton()
	{
		window.open('?lang=<?php echo $plang;?>&opt=10&item=&link=d2hfbHYwMDc2L3doX2x2MDA3Ni5waHA=','_self');
	}
	function nhapkiem()
	{
	...
	function AlertFullKho(o)
	{
		if(o.checked)
		{
			if(confirm("Bạn muốn khiểm kho toàn bộ hệ thống không (Y/N)?"))
			{
				AddProd('00000000000000');
			}
			else
			{
				o.checked=false;
...
<div class="hd_cafe">
	...
	<?php
		...
		$lvwh_lv0015=new wh_lv0015($_SESSION['ERPSOFV2RRight'],$_SESSION['ERPSOFV2RUserID'],'Wh0015');
		if($_SESSION['ERPSOFV2RRight']=='admin' || $lvwh_lv0015->GetView())
		{
			echo '<li><div class="licafe" onclick="baocaoton()">BC TỒN</div></li>';
		}
		?>
...
<div class="hd_cafe">
	<div style="clear:both;padding:5px;font:bold 18px Arial,Tahoma;line-height:35px;">
		...
		<div style="float:left;width:30px;">
			<input title="Bạn có muốn kiểm kho toàn bộ" type="checkbox" value="1" name="isFullKho" id="isFullKho" onclick="AlertFullKho(this);"/>
		</div>
		<div style="float:left;width:70px;padding-right:5px;">
			<select class="kk_txtkho"  name="txtlv994" id="txtlv994" onblur="UpdateContract(this,94)"   tabindex="50"  style="width:70px;height:30px;" onkeypress="return CheckKey(event,1)" />
		...
		<table border="0">
			<tr>
				<td style="padding-right:5px">
					<input tabindex="19" title="barcode hoặc mã sản phẩm" name="txtlv903" id="txtlv903" tabindex="19"  style="width:75px;height:25px;text-align:center;" onkeypress="return CheckKeys(event,1,this)" value="<?php echo $lvwh_lv0005->lv003;?>" onFocus="if(document.frmadd.txtlv913.value=='') document.frmadd.txtlv913.value=document.frmadd.txtlv805.value" onblur="//LoadItem();//changecategory_change(this.value)"/>
				</td>
				<td style="padding-right:5px">
					<ul id="pop-nav" lang="pop-nav1" onMouseOver="ChangeName(this,1)" xml:lang="pop-nav1">
						<li class="menupopT">
						<input type="text" autocomplete="off" class="search_img_btn" name="txtlv903_search" id="txtlv903_search" style="width:75px;height:25px;" onKeyUp="LoadPopupParent(this,'txtlv903','*@*@*.sl_lv0007','concat(lv002,@! @!,lv001)')" onFocus="LoadPopupParent(this,'txtlv903','*@*@*.sl_lv0007','concat(lv002,@! @!,lv001)')" tabindex="200">
	  ...
	  <div  class="kk_chonsanpham" style="float:left;width:70px;font:bold 12px Arial,Tahoma;padding-right:5px;" onclick="ShowNhomSanPham()">
			<div class="sanpham_bg_bnt" style="height:20px;padding:8px;padding-bottom:0px;"><center>Chọn SP</center></div>
		</div>							
...
<!-------------------SP------------------------->
				<div class="khungsp" id="khungsp">
						<div id="kk_detailview" style="background:#fff;height:400px;overflow: scroll;width100%;">
							...
							<?php 
							$vLangArr=GetLangFile("$vDir../","SL0049.txt",$plang);
							echo $lvwh_lv0004->LV_GetDetailFull($lvwh_lv0004->lv001,$vWHID,$vLangArr,$vSum);
							?>
							</div>
						</div>
						...
						<?php 
							...
							$vthanhcon="";
							$i=0;
							$vsql="select * from all_gmacv3_0.sl_lv0006 where  (ISNULL(lv003) or lv003='' or lv001=lv003) order by lv005 asc";
							...
							while($vrowparent=db_fetch_array($vresultparent))
							{
								...
								$vsql="select distinct * from all_gmacv3_0.sl_lv0006 where (lv003='".$vrowparent['lv001']."') or ( lv001='".$vrowparent['lv001']."' and (ISNULL(lv003) or lv003='')) order by lv005 asc";
								$vresult=db_query($vsql);
								$vthanhcon2='';
								while($vrow=db_fetch_array($vresult))
								{
									...
									$vthanhcon1='
										...
										<div>
											...
											$vsql1="select * from all_gmacv3_0.sl_lv0007 where lv003='".$vrow['lv001']."' and lv015>0 order by lv010,lv002 asc";
...
function AddItemNew(hdid,WHID,vo)
{
		...
		$xmlhttp93=null;
			...
			var ArItem=Array();
			ArItem=vo.split(".",2);
			var FullKho=document.getElementById("isFullKho")
			isFullKho=0;
			if(FullKho.checked) isFullKho=1;
			url=url+"?&ajaxitemsend=ajaxcheck"+"&ContractID="+hdid+"&WHID="+WHID+"&ItemID="+ArItem[1]+"&update="+vUpdate+"&isFullKho="+isFullKho;
			url=url.replace("#","");
...
function statechitietdh()
{
		if (xmlhttp3.readyState==4)
		{
			...
			var startdomain11=xmlhttp3.responseText.indexOf('[SOLUONGKIEM]')+13;
			var enddomain11=xmlhttp3.responseText.indexOf('[ENDSOLUONGKIEM]');
			var domainid11=xmlhttp3.responseText.substr(startdomain11,enddomain11-startdomain11);
			var o=document.getElementById('tongsoluongkiem').innerHTML=domainid11;
	...
	if (xmlhttp655.readyState==4)
		{
			...
			var o=document.getElementById('tongsoluongthuc').innerHTML=domainid1;
			var startdomain11=xmlhttp655.responseText.indexOf('[SOLUONGKIEM]')+13;
			var enddomain11=xmlhttp655.responseText.indexOf('[ENDSOLUONGKIEM]');
			var domainid11=xmlhttp655.responseText.substr(startdomain11,enddomain11-startdomain11);
			var o=document.getElementById('tongsoluongkiem').innerHTML=domainid11;
...
function changedonvifull(o,chitietid,itemid)
		{
			notes = document.getElementById('notes_'+chitietid);
			donvi = document.getElementById('donvi_'+chitietid);
			donvichinh=document.getElementById('txtdonvichinh_'+chitietid);
			//Là chính
			soluongton = parseFloat(document.getElementById('soluongton_'+chitietid).value);
			var quidoi=parseFloat(document.getElementById(itemid+'_'+o.value).value);
			if(quidoi==0) quidoi=1;
			/*if(donvi==donvichinh)
				document.getElementById('soluongkiem_'+chitietid).value=soluongton;
			else*/
			if(donvi!=donvichinh)
			{
				 document.getElementById('soluongkiem_'+chitietid).value='';
				 document.getElementById('soluongthuc_'+chitietid).value=''
			}
			$xmlhttp26=null;
			if(chitietid=="") 
			{
			...
			var url=document.location;
			donhangid = document.getElementById('txtContractID').value;
			url=url+"?&ajaxdonvi=ajaxcheck"+"&donvi="+o.value+"&soluongkiem="+document.getElementById('soluongkiem_'+chitietid).value+"&chitietid="+chitietid+"&optprice=1"+"&donhangid="+donhangid;
			url=url.replace("#","");
			xmlhttp26.onreadystatechange=statepdonvi;
			xmlhttp26.open("GET",url,true);
			xmlhttp26.send(null);
		}
		function changedonvi(o,chitietid)
		{
			notes = document.getElementById('notes_'+chitietid);
			donvi = document.getElementById('donvi_'+chitietid);
			donvichinh=document.getElementById('txtdonvichinh_'+chitietid);
			//Là chính
			soluongton = parseFloat(document.getElementById('soluongton_'+chitietid).value);
			var quidoi=parseFloat(document.getElementById('txtquidoi_'+chitietid).value);
			if(quidoi==0) quidoi=1;
			/*if(donvi==donvichinh)
				document.getElementById('soluongkiem_'+chitietid).value=soluongton;
			else*/
		...
		{	
			...
			var url=document.location;
			donhangid = document.getElementById('txtContractID').value;
			url=url+"?&ajaxslkiem=ajaxcheck"+"&soluongthuc="+o.value+"&soluongkiem="+document.getElementById('soluongkiem_'+chitietid).value+"&chitietid="+chitietid+"&optprice=1"+"&donhangid="+donhangid;
		...
		function changeSLKK(o,chitietid)
		{	
			notes = document.getElementById('notes_'+chitietid);
			donvi = document.getElementById('donvi_'+chitietid);
			donvichinh=document.getElementById('txtdonvichinh_'+chitietid);
			//Là chính
			soluongton = parseFloat(document.getElementById('soluongton_'+chitietid).value);
			if(donvi.value==donvichinh.value)
			{
				var soluongkiem=parseFloat(o.value);
				document.getElementById('soluongthuc_'+chitietid).value=soluongkiem;
				if(document.getElementById('soluongthuc_'+chitietid).value=='')  soluongthuc=parseFloat(document.getElementById('soluongthuc_'+chitietid).value);
				//Giá trị sửa của kiem luon bang chinh
				/*if(notes.value=='' && soluongton!=soluongthuc)
				{
					alert('Xin vui lòng nhập ghi chú trước khi đổi số lượng');
					document.getElementById('soluongthuc_'+chitietid).value=soluongton;
					o.value=soluongton;
					notes.focus();
				}*/
			}
			else
			{
				...
				var quidoi=parseFloat(document.getElementById('txtquidoi_'+chitietid).value);
				if(quidoi==0) quidoi=1;
				var soluongthuc=Math.round(soluongkiem*1000/quidoi)/1000;
				if(document.getElementById('soluongthuc_'+chitietid).value=='') document.getElementById('soluongthuc_'+chitietid).value=soluongthuc;
				/*if(notes.value=='' && soluongton!=soluongthuc)
				{
					document.getElementById('soluongthuc_'+chitietid).value=soluongton;
					alert('Xin vui lòng nhập ghi chú trước khi đổi số lượng');
					o.value=soluongton/quidoi;
					notes.focus();
				}*/
			}
			var soluongkiem=parseFloat(document.getElementById('soluongkiem_'+chitietid).value);
			if(parseInt(o.value)==0)
				o.parentNode.parentNode.className=o.parentNode.parentNode.className+'  textunderline';
			else
				o.parentNode.parentNode.className=o.parentNode.parentNode.className.replace('textunderline','');
			...
			var url=document.location;
			donhangid = document.getElementById('txtContractID').value;
			url=url+"?&ajaxslkk=ajaxcheck"+"&soluongkiem="+o.value+"&soluongthuc="+document.getElementById('soluongthuc_'+chitietid).value+"&chitietid="+chitietid+"&optprice=1"+"&donhangid="+donhangid;
			url=url.replace("#","");
			xmlhttp16.onreadystatechange=statepricedhSLKK;
			xmlhttp16.open("GET",url,true);
			xmlhttp16.send(null);
		}		
		function statepricedhSLKK()
		{
			if (xmlhttp16.readyState==4)
			{
				var startdomain=xmlhttp16.responseText.indexOf('[CHECKDONHANG]')+14;
				var enddomain=xmlhttp16.responseText.indexOf('[ENDCHECKDONHANG]');
				var domainid=xmlhttp16.responseText.substr(startdomain,enddomain-startdomain);
				var startdomain1=xmlhttp16.responseText.indexOf('[SOLUONGTHUC]')+13;
				var enddomain1=xmlhttp16.responseText.indexOf('[ENDSOLUONGTHUC]');
				var domainid1=xmlhttp16.responseText.substr(startdomain1,enddomain1-startdomain1);
				var o=document.getElementById('tongsoluongthuc').innerHTML=domainid1;
				var startdomain11=xmlhttp16.responseText.indexOf('[SOLUONGKIEM]')+13;
				var enddomain11=xmlhttp16.responseText.indexOf('[ENDSOLUONGKIEM]');
				var domainid11=xmlhttp16.responseText.substr(startdomain11,enddomain11-startdomain11);
				var o=document.getElementById('tongsoluongkiem').innerHTML=domainid11;

				var startdomain1=xmlhttp16.responseText.indexOf('[SOLUONGKHO]')+12;
				var enddomain1=xmlhttp16.responseText.indexOf('[ENDSOLUONGKHO]');
				var domainid1=xmlhttp16.responseText.substr(startdomain1,enddomain1-startdomain1);
				var o=document.getElementById('tongsoluongkho').innerHTML=domainid1;
				var startdomain4=xmlhttp16.responseText.indexOf('[SOLUONGMUA]')+12;
				var enddomain4=xmlhttp16.responseText.indexOf('[ENDSOLUONGMUA]');
				var domainid4=xmlhttp16.responseText.substr(startdomain4,enddomain4-startdomain4);
				var o=document.getElementById('tongsoluongmua').innerHTML=domainid4;
			}
		}				
...
function statepricedh()
{
		if (xmlhttp6.readyState==4)
		{
			...
			var o=document.getElementById('tongsoluongthuc').innerHTML=domainid1;
			var startdomain11=xmlhttp6.responseText.indexOf('[SOLUONGKIEM]')+13;
			var enddomain11=xmlhttp6.responseText.indexOf('[ENDSOLUONGKIEM]');
			var domainid11=xmlhttp6.responseText.substr(startdomain11,enddomain11-startdomain11);
			var o=document.getElementById('tongsoluongkiem').innerHTML=domainid11;
...
function viewkiemkho()
{
	...
	kk_detailview.style.height=(vHeight-180)+'px';
	var khungsp=document.getElementById("khungsp");
	khungsp.style.height=(window.innerHeight-180)+'px';
}
/* Timer */
