...
define("DB_USER","demo");
define("DB_PWD","demosofv.2");
define("DB_DATABASE","huyvanv2_0");
...
{	if('admin'==$_SESSION['ERPSOFV2RRight'])
	...
	else
	{
		...
		{
			case "HR":
				$vsql=	"select count(*) as count from all_gmacv3_0.lv_lv0008 B where B.lv002='$vlv002' and B.lv003='Ad0018' and B.lv004=1";
				$tresult=db_query($vsql);
				$trow=db_fetch_array($tresult);
				if($trow['count']>0) return 1;	
				break;
			case "WB":
				$vsql=	"select count(*) as count from all_gmacv3_0.lv_lv0008 B where B.lv002='$vlv002' and B.lv003='Ad0034' and B.lv004=1";
				$tresult=db_query($vsql);
				$trow=db_fetch_array($tresult);
				if($trow['count']>0) return 1;	
				break;
			case "LB":
				$vsql=	"select count(*) as count from all_gmacv3_0.lv_lv0008 B where B.lv002='$vlv002' and B.lv003='Ad0035' and B.lv004=1";
				$tresult=db_query($vsql);
				$trow=db_fetch_array($tresult);
				if($trow['count']>0) return 1;	
				break;				
		...
		if($vrightcontrol=="")
		{
			$vsql=	"select count(*) as count from all_gmacv3_0.lv_lv0008 B where B.lv002='$vlv002' and B.lv003='$vright' and B.lv004=1";
		}
		else
		{
			$vsql="select count(*) as count from all_gmacv3_0.lv_lv0009 A Where A.lv002='$vrightcontrol' and A.lv004=1 and A.lv003 in (select B.lv001 from all_gmacv3_0.lv_lv0008 B where B.lv002='$vlv002' and B.lv003='$vright' and B.lv004=1) ";
		}
		$tresult=db_query($vsql);
		$trow=db_fetch_array($tresult);
...
{
	$vsql="insert into log (ID,LoginDate,LoginTime,State,Ip,Mac) values('$vlv002','$vLoginDate','$vLoginTime',$vState,'$vIp','$vMac')";
	db_query($vsql);	
}
...
function CheckPermission($vlv002,$vright)
{
	...
	{
	$vsql="select count(*) as count from all_gmacv3_0.lv_lv0008 A where A.lv002='$vlv002' and A.lv003='$vright' and A.lv004=1) ";
	$tresult=db_query($vsql);
	$trow=db_fetch_array($tresult);
	return (int)$trow['count'];
	}
