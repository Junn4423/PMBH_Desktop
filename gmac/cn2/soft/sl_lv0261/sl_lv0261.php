...
/////////////init object//////////////
$mosl_lv0261=new  sl_lv0261($_SESSION['ERPSOFV2RRight'],$_SESSION['ERPSOFV2RUserID'],'Sl0261');
...
$mosl_lv0070->LV_Load();
$mosl_lv0261->obj_conf=$mosl_lv0070;
//////////////////////////////////////////////////////
//////Delete message///
$lvMessage=array();
...
$vNow=GetServerDate();
$mosl_lv0261->lv002=getyear($vNow)."-".getmonth($vNow)."-"."01";
$mosl_lv0261->lv003=$vNow;
//$ma=$_GET['ma'];
...
elseif($flagID==2)
{
	$mosl_lv0261->datefrom=$_POST['txtDateFrom'];
	$mosl_lv0261->dateto=$_POST['txtDateTo'];
}
//first is load
if($_POST["txtFlag"]=="")
{
...
if($mosl_lv0261->datefrom=='' && $mosl_lv0261->dateto=='')
{
	$vNow=GetServerDate();
	$mosl_lv0261->datefrom=$vNow;
	$mosl_lv0261->dateto=$vNow;
}
else
{
	if($mosl_lv0261->datefrom!='') $mosl_lv0261->datefrom=recoverdate($mosl_lv0261->datefrom,$plang);
	if($mosl_lv0261->dateto!='') $mosl_lv0261->dateto=recoverdate($mosl_lv0261->dateto,$plang);
}
$maxPages = 10;
if($curPage=="") 
$curPage = 1;
...
function RptWH(vValue)
{
	...
	o.action="sl_lv0261?func="+func+"&lang=<?php echo $plang;?>&txtDateFrom="+o.txtDateFrom.value+"&txtDateTo="+o.txtDateTo.value;
	o.submit();
}
...
<!--////////////////////////////////////Code add here///////////////////////////////////////////-->
	...
	<form onSubmit="return false;" action="?func=<?php echo $_GET['func'];?>&childfunc=<?php echo $_GET['childfunc'];?>&ChildID=<?php echo $_GET['ChildID'];?>&ID=<?php echo $_GET['ID']?>&<?php  echo getsaveget($plang,$popt,$pitem,$plink,$pgroup,0,0,1,0);?>" method="post" name="frmchoose" id="frmchoose">
	  ...
	  <table style="background:#f2f2f2;font:10px arial">
			...
			<td  height="20px">
				<input autocomplete="off" style="width:65px;text-align:center;" name="txtDateFrom" type="text" id="txtDateFrom" value="<?php echo $mosl_lv0261->FormatView($mosl_lv0261->datefrom,2);?>" tabindex="2" style="width:100%" onKeyPress="return CheckKey(event,7)" onClick="if(self.gfPop)gfPop.fPopCalendar(this);return false;" ></td> 
			<td>
			đến
			</td>
			<td  height="20px">
				<input autocomplete="off" style="width:65px;text-align:center;" name="txtDateTo" type="text" id="txtDateTo" value="<?php echo $mosl_lv0261->FormatView($mosl_lv0261->dateto,2);?>" tabindex="2" style="width:100%" onKeyPress="return CheckKey(event,7)" onClick="if(self.gfPop)gfPop.fPopCalendar(this);return false;" >
			</td> 
		...
		<?php
			echo $strDataShow = $mosl_lv0261->PrintInOutPutInStockDetail($plang, $vLangArr,$mosl_lv0261->datefrom,$mosl_lv0261->dateto);
		?>
  </form>
