...
if(isset($_GET['ajaxsate']))
	{	
		$vsql="update all_gmacv3_0.lv_lv0007 set lv098=concat(curdate(),' ',curtime()) where lv001='".$_SESSION['ERPSOFV2RUserID']."' and lv097<>''";
		$vresult=db_query($vsql);
		$vsql="select lv007,lv100 from all_gmacv3_0.lv_lv0007 where lv001='".$_SESSION['ERPSOFV2RUserID']."' ";
		$vresult=db_query($vsql);
		...
		if($vrow)
		{
			...
			else
			{
				if($vrow['lv100']!='')
				{
					if(strpos($vrow['lv100'],'cn002')===false)
					{
						$_SESSION['ERPSOFV2RUserID']=NULL;
						$_SESSION['ERPSOFV2RRight']=NULL;
					}
				...
				<div id="sof_left" style="float:left;">
					...
					<div class="sof_left_themes">
						...
						<div class="sof_left_themes_content" style="clear:both;overflow:hidden">
							...
							<ul style="clear:both">
							<?php
							$vsql="select lv001,".(($plang=='VN')?'lv002':'lv003')." lv002 from all_gmacv3_0.lv_lv0011";
							$vresult=db_query($vsql);
