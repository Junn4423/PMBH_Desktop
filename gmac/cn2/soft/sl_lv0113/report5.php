...
<?php
	...
	$vorder=1;
	$sqlS1 = "SELECT sum(A.lv004*A.lv006) GiatUi FROM sl_lv0114 A inner join all_gmacv3_0.sl_lv0007 B on A.lv003=B.lv001 left join sl_lv0005 C on A.lv005=C.lv001 left join sl_lv0059 D on A.lv009=D.lv001 WHERE B.lv003='Giatui' and A.lv002='".$mosl_lv0113->lv001."' order by A.lv015 asc,A.lv001 asc";
	$bResult1 = db_query($sqlS1);
	$vrow1 = db_fetch_array ($bResult1);
	$vGiatUi=$vrow1['GiatUi'];
	$sqlS = "SELECT A.*,B.lv002 ItemName,C.lv002 UnitName,D.lv002 ProgName FROM sl_lv0114 A inner join all_gmacv3_0.sl_lv0007 B on A.lv003=B.lv001 left join sl_lv0005 C on A.lv005=C.lv001 left join sl_lv0059 D on A.lv009=D.lv001 WHERE B.lv003<>'Giatui' and A.lv002='".$mosl_lv0113->lv001."'  order by A.lv015 asc,A.lv001 asc";
	$bResult = db_query($sqlS);
	$sumMoney=0;
	$sumScore=0;
	$sumDiscount=0;
...
?>		
		<tr class="lvlinehtable1">
			...
			<td align="right" style="padding:2px;padding-right:5px;padding-left:5px;font-size:14px"><?php $vSum=$mosl_lv0113->LV_GetContractMoneyCustomerDate($vCusID,$lvsl_lv0070->lv015,$lvsl_lv0070->lv016)+$mosl_lv0113->LV_GetContractMoneyCustomerDetailDate($vCusID,$lvsl_lv0070->lv015,$lvsl_lv0070->lv016);echo $mosl_lv0113->FormatView($vSum/$lvsl_lv0070->lv009,10);?></td>
			<?php
			}
			else
			{
...
?>		
		...
		?>
		<tr class="lvlinehtable1">
			<td class="lvlineboldtable" colspan="<?php echo ($vCount>0)?4:3;?>">GIẢM GIÁ <?php echo $mosl_lv0114->FormatView($mosl_lv0113->lv022,10);?>%: </td>
		...
		?>
		<tr class="lvlinehtable1">
			<td class="lvlineboldtable" colspan="<?php echo ($vCount>0)?4:3;?>"><?php echo $mosl_lv0114->FormatView($mosl_lv0113->lv006,10);?>% VAT:</td>
		...
		<tr ondblclick="this.innerHTML=''">
			<td class="lvlineboldtable" colspan="<?php echo ($vCount>0)?9:8;?>">Bằng chữ: <?php echo LNum2Text($sumMoney-$sumDiscount-$vsumCash,$plang);?> </td>
		</tr>
