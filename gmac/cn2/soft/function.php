...
{
$tsql="select TIME_TO_SEC(CURRENT_TIME()) as GDate";
$tresult=db_query($tsql);
if($tresult)
	{
		$trow=db_fetch_array($tresult);
...
{
	if(strpos($vTimeEnd," ")>0 || strpos($vTimeStart," ")>0)
	{
		$ArrTimeEnd=explode(" ",$vTimeEnd,2);
		$ArrTimeStart=explode(" ",$vTimeStart,2);
		if($ArrTimeEnd[0]==$ArrTimeStart[0])
		{
			$vTimeStart=$ArrTimeStart[1];
			$vTimeEnd=$ArrTimeEnd[1];
			...
			if(($vSEnd-$vSStart)<0)
			{
				$sreturn=Fillnum(60+($vSEnd-$vSStart),2);
				$vMEnd=$vMEnd-1;
			}
			else
				$sreturn=Fillnum($vSEnd-$vSStart,2);

			if(($vMEnd-$vMStart)<0)
			{
				$sreturn=Fillnum(60+($vMEnd-$vMStart),2).":".$sreturn;
				$vHEnd=$vHEnd-1;
			}
			else
				$sreturn=Fillnum($vMEnd-$vMStart,2).":".$sreturn;

				$sreturn=Fillnum($vHEnd-$vHStart,2).":".$sreturn;
				return $sreturn;
		}
		else
		{
...
function GetDayWorkInMonth($vYear,$vMonth)
{
	$vReturn=4;
...
function LCurrencys($strValue,$vLang)
{
	...
	$strReturn = "";
	$arrNum = explode(".", $strValue);//Tach thanh 2 phan 2 ben dau cham dua vao mang
	if(((int)$arrNum[1])!=0) $decimal=strlen($arrNum[1]);
	else $decimal=0;
...
function LNum2Text($NumCurrency, $plang)
{
	...
	if($plang == "EN")//Ca tieng Viet va tieng Anh deu su dung ngon ngu tieng Viet
	{
		if($NumCurrency<0)
		{
			$NumCurrency=$NumCurrency-2*$NumCurrency;
...
function getInfor($vUserID,$State)
{
	...
	{
		case 1:
			$vsql="	SELECT lv002 UserGroupID 			FROM all_gmacv3_0.lv_lv0007  
					WHERE lv001='$vUserID';";
			...
			break;
		case 2:
			$vsql="	SELECT lv006 EmployeeID 			FROM all_gmacv3_0.lv_lv0007  
					WHERE lv001='$vUserID';";
			$vresult=db_query($vsql);
			$vrow=db_fetch_array($vresult);
			if($vrow)
			...
			break;
		case 4:
			$vsql="	SELECT lv004 NameShift 			FROM all_gmacv3_0.lv_lv0007  
					WHERE lv001='$vUserID';";
			$vresult=db_query($vsql);
			$vrow=db_fetch_array($vresult);
			if($vrow)
			{
				$vReturn=$vrow['NameShift'];
			}
			break;
		case 99:
			$vsql="	SELECT lv099 Themes 			FROM all_gmacv3_0.lv_lv0007  
					WHERE lv001='$vUserID';";
			$vresult=db_query($vsql);
			$vrow=db_fetch_array($vresult);
			if($vrow)
...
{
	$vsql="	update all_gmacv3_0.lv_lv0007 set lv099='$themes' WHERE lv001='$vUserID';";
	$vresult=db_query($vsql);
}
function GetUserName($strUser, $plang){
	//if(is_numeric(substr($strUser,0,1))){
		$strSQL = "SELECT lv002 FirstName, lv003 MiddleName,lv004 LastName from all_gmacv3_0.hr_lv0020 WHERE lv001='$strUser'; ";
		...
		return $strFullName;
	/*} else {
		$strSQL = "SELECT lv004 FROM all_gmacv3_0.lv_lv0007 WHERE lv001='$strUser'; ";
		$arrResult = db_query($strSQL);
		$intRows = db_fetch_array($arrResult);
...
//Hàm công/trừ thêm giá trị ngày
	...
	function AnalysisLink($vBody,$pPara1,$pPara2,$pLink)
	{
		...
		$strTemp=" ".str_replace("<br>"," ",$strTemp)." ";		
		$vArrTemp=split($pPara1,$strTemp);
		$strFinish="";
		for($i=1;$i<count($vArrTemp);$i++)
		{
			$vArrChild=split($pPara2,$vArrTemp[$i]);
	...
	function SplitTo($vBody,$pPara1,$pPara2,$vPara3)
	{
		$strTemp=$vBody;
		$vArrTemp=split($pPara1,$strTemp);
		$strReturn="";
	...
	function ProcessEmail($vBody,$pPara1,$pPara2,$vPara3)
	{
		...
		if(count($vArrTemp)==1) 
		{
			$vArrTemp=split($pPara2,$strTemp."",2);
			if(count($vArrTemp)==1) 
			{
				$vArrTemp=split($vPara3,$strTemp."",2);
...
{
	$arr_uni=array('ẩ','à','ô','ơ','ă','ó','ú','é','õ','ê','ũ','ẹ','ù' ,'è','ì','í','ò','đ','ũ','ư','ý','ố','â','ĩ','ị','á','ứ','ạ','ộ','ể','ụ','ủ','ệ','ã','ờ','ậ','ề','ã','ế','ả','ự','ợ','ễ','ớ','ở','ầ','ẫ','ằ','ổ','ọ','ẳ','ẽ','ỏ','ỡ','ấ','ữ','ồ','ỗ','a','b','c','d','e','f','g','h','i','j','k','l','m','n','o','p','q','r','s','t','w','v','u','x','y');
	  $UNICODE_UPPER_ACCENTS = array('ẩ'=>'Ẩ','à'=>'À' ,'ô'=>'Ô','ơ'=>'Ơ',
  'ă'=>'Ă','ó'=>'Ó','ú'=>'Ú','é'=>'É','õ'=>'Õ','ê'=>'Ê','ũ'=>'Ű','ẹ'=>'Ẹ','ù'=>'Ù' ,'è'=>'È','ì'=>'Ì','í'=>'Í' ,'ò'=>'Ò','đ'=>'Đ' ,'ũ'=>'Ũ','ư'=>'Ư','ý'=>'Ý','ố'=>'Ồ','â'=>'Â','ĩ'=>'Ĩ','ị'=>'Ị' ,'á'=>'Á','ứ'=>'Ứ','ạ'=>'Ạ','ộ'=>'Ộ','ể'=>'Ể','ụ'=>'Ụ','ủ'=>'Ủ','ệ'=>'Ệ','ã'=>'Ã','ờ'=>'Ờ','ậ'=>'Ậ','ề'=>'Ề','ã'=>'Ã','ế'=>'Ế','ả'=>'Ả','ự'=>'Ự','ợ'=>'Ợ','ễ'=>'Ễ','ớ'=>'Ớ','ở'=>'Ở','ầ'=>'Ầ','ẫ'=>'Ẫ','ằ'=>'Ằ','ổ'=>'Ổ','ọ'=>'Ọ','ẳ'=>'Ẳ','ẽ'=>'Ẽ','ỏ'=>'Ỏ','ỡ'=>'Ỡ','ấ'=>'Ấ','ữ'=>'Ữ','ồ'=>'Ồ','ỗ'=>'Ỗ','a'=>'A','b'=>'B','c'=>'C','d'=>'D','e'=>'E','f'=>'F','g'=>'G','h'=>'H','i'=>'I','j'=>'J','k'=>'K','l'=>'L','m'=>'M','n'=>'N','o'=>'O','p'=>'P','q'=>'Q','r'=>'R','s'=>'S','t'=>'T','w'=>'W','v'=>'V','u'=>'U','x'=>'X','y'=>'Y');
	  foreach ($arr_uni as $uni)
	  {
		$fac_contetn=str_replace($uni,$UNICODE_UPPER_ACCENTS[$uni],$fac_contetn);
	  }
...
function unicode_to_upperexist($fac_contetn)
{
	$arr_uni=array('ẩ','à','ô','ơ','ă','ó','ú','é','õ','ê','ũ','ẹ','ù' ,'è','ì','í','ò','đ','ũ','ư','ý','ố','â','ĩ','ị','á','ứ','ạ','ộ','ể','ụ','ủ','ệ','ã','ờ','ậ','ề','ã','ế','ả','ự','ợ','ễ','ớ','ở','ầ','ẫ','ằ','ổ','ọ','ẳ','ẽ','ỏ','ỡ','ấ','ữ','ồ','ỗ');
	  $UNICODE_UPPER_ACCENTS = array('ẩ'=>'Ẩ','à'=>'À' ,'ô'=>'Ô','ơ'=>'Ơ',
  'ă'=>'Ă','ó'=>'Ó','ú'=>'Ú','é'=>'É','õ'=>'Õ','ê'=>'Ê','ũ'=>'Ű','ẹ'=>'Ẹ','ù'=>'Ù' ,'è'=>'È','ì'=>'Ì','í'=>'Í' ,'ò'=>'Ò','đ'=>'Đ' ,'ũ'=>'Ũ','ư'=>'Ư','ý'=>'Ý','ố'=>'Ồ','â'=>'Â','ĩ'=>'Ĩ','ị'=>'Ị' ,'á'=>'Á','ứ'=>'Ứ','ạ'=>'Ạ','ộ'=>'Ộ','ể'=>'Ể','ụ'=>'Ụ','ủ'=>'Ủ','ệ'=>'Ệ','ã'=>'Ã','ờ'=>'Ờ','ậ'=>'Ậ','ề'=>'Ề','ã'=>'Ã','ế'=>'Ế','ả'=>'Ả','ự'=>'Ự','ợ'=>'Ợ','ễ'=>'Ễ','ớ'=>'Ớ','ở'=>'Ở','ầ'=>'Ầ','ẫ'=>'Ẫ','ằ'=>'Ằ','ổ'=>'Ổ','ọ'=>'Ọ','ẳ'=>'Ẳ','ẽ'=>'Ẽ','ỏ'=>'Ỏ','ỡ'=>'Ỡ','ấ'=>'Ấ','ữ'=>'Ữ','ồ'=>'Ồ','ỗ'=>'Ỗ');
	  foreach ($arr_uni as $uni)
	  {
		$fac_contetn=str_replace($uni,$UNICODE_UPPER_ACCENTS[$uni],$fac_contetn);
	  }
