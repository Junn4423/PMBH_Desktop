...
$lvwh_lv0031->objlot=$mowh_lv0020;
if($plang=="") $plang="EN";
	$vLangArr=GetLangFile("../../","WH0016.txt",$plang);
$lvwh_lv0031->lang=strtoupper($plang);
$curPage=(int)$_POST['curPg'];	
$vFlag=(int)$_POST['txtFlag'];
$data = array();
  
  function add_person( $lv003, $lv004, $lv005,$lv006,$lv007,$lv008,$lv009,$lv014,$lv015,$lv088)
  {
  global $data;
	 ...
	 if ( $_FILES['file']['tmp_name'] )
	  ...
	  foreach ($rows as $row)
	  {
		  if ( !$first_row )
		  {
			  ...
			  if ( $index == 9 ) $lv015 = $cell->nodeValue;
			  if ( $index == 10 ) $lv088 = $cell->nodeValue;
			  
			  $index += 1;
		  }
		  if(trim($lv003)!="" && $lv003!=NULL)  	add_person( $lv003, $lv004, $lv005,$lv006,$lv007,$lv008,$lv009,$lv014,$lv015,$lv088);
	  }
	  $first_row = false;
	  }
  }
  $vWHID=$_POST['txtlv099'];
$vII=0;  
foreach( $data as $row )
{
	...
	{
	$lvwh_lv0031->lv001=InsertWithCheckExt('wh_lv0031', 'lv001', '',1);
	$lvwh_lv0031->lv002=getInfor($_SESSION['ERPSOFV2RUserID'],2);
	$lvwh_lv0031->lv003=$lvsl_lv0007->lv001;
	$lvwh_lv0031->lv004=$row['lv004'];
	$lvwh_lv0031->lv005=(trim($row['lv005'])=="" || $row['lv005']==NULL)?$lvsl_lv0007->lv004:$row['lv005'];
	$lvwh_lv0031->lv006=$row['lv006'];
	$lvwh_lv0031->lv007=$row['lv007'];
	$lvwh_lv0031->lv008=$row['lv008'];
	$lvwh_lv0031->lv009=$row['lv009'];
	$lvwh_lv0031->lv014=($row['lv014']==null)?'':$row['lv014'];
	$lvwh_lv0031->lv015=$row['lv015'];
	$lvwh_lv0031->lv016=$lvNow;
	
	if(($lvsl_lv0007->lv012=='FIFO' || $lvsl_lv0007->lv012=='LIFO') && trim($lvwh_lv0031->lv014)==""  )
			$strReturn=$lvwh_lv0031->LV_Insert_FI_LI_FO($lvsl_lv0007->lv012,$vWHID,(int)$row['lv015']);
		else
			$strReturn=$lvwh_lv0031->LV_Insert();
		$vII++;
	}
}
		if($vresult==true || $vII>0) {
				...
				<!--////////////////////////////////////Code add here///////////////////////////////////////////-->
					<form action="#"   name="frmadd" id="frmadd"  method="post" enctype="multipart/form-data" >
						...
						<table width="100%" border="0" align="center" id="table1">
							...
							<tr>
								<td width="166"  height="20px"><?php echo $vLangArr[28];?></td>
								<td  height="20px">
									<input type="file" name="file" />			</td>
								<td  height="20px"><select name="txtlv099" id="txtlv099"   tabindex="7"  style="width:100%" onKeyPress="return CheckKey(event,7)"/><?php echo $lvwh_lv0031->LV_LinkField('lv099',$lvwh_lv0031->lv099);?>
							  </select></td>
							</tr>															
							<tr>
							  <td  height="20px" colspan="2"><input name="txtFlag" type="hidden" id="txtFlag"  /> <a href="MAU_XUAT_KHO_TEMPLATE .zip" title="<?php echo $vLangArr[30];?>"><?php echo $vLangArr[29];?></a></td>
							</tr>
							<tr>
