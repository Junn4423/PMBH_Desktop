...
if (isset($_POST['txtUserName'])) {
	...
	if (intval($vFlag) == 1) {
		...
		if ($vUserName != "" && $vPassword != "") {
			$vsql = "select *,IF(lv095='',1,IF(lv095='$lvIpClient',1,0)) isGood,DATEDIFF(curdate(),lv009) numdate,TIME_TO_SEC(concat(curdate(),' ',curtime())) tos,TIME_TO_SEC(lv009) fros from all_gmacv3_0.lv_lv0007 where lv001='$vUserName' and lv005='" . md5($vPassword) . "' and lv007=0 and ((lv100 like '%cn002%') or lv100='' or ISNULL(lv100))";
			...
			if ($vnum > 0) {
				...
				if ($vrow['isGood'] == 1 || substr($lvIpClient, 0, 8) == '192.168.') {
					if ($vrow['lv008'] != '' && $vrow['lv008'] != null) {
						if ($vrow['tos'] - $vrow['fros'] < 0 && ($vrow['numdate'] == 0)) {
							...
							$vsql = "update all_gmacv3_0.lv_lv0007 set lv008='" . $_SESSION['SOFONLINE'] . "',lv009=concat(CurDate(),' ',CurTime()) where lv001='$vUserName'";
							$vresult = db_query($vsql);
							$vsql = "update all_gmacv3_0.lv_lv0007 set lv095='$lvIpClient' where lv094='$vUserName'";
							$vresult = db_query($vsql);
							$vDate = GetServerDate();
							$vTime = GetServerTime();
			...
			<div class="sof_pages_header">
				<div class="hd_title">
					...
					<div style="float:right;padding-top:20px;padding-right:20px">
						<a href="https://quanlybanhang.gmaccoffee.com/"><img width="60"
								...
								<div class="loginname">
									<div>
										...
										<div style="float:right"><a style="color:blue;"
												href="http://localhost/gmac/">GMAC - CN 1</a></div>
