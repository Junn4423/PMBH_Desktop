...
import {layDsMonNuoc, updateTrangThai} from '../../api';
import {
  NavigationProp,
  useFocusEffect,
  useNavigation,
...
const NhomMonNuoc: React.FC = () => {
  const nav = useNavigation<NavigationProp<RootStackParamList>>();
  const [data, setData] = useState<any>();
  const {mutate, isPending} = useMutation({
    mutationFn: layDsMonNuoc,
    onSuccess: data => {
  ...
  const renderTableGroup = ({item}: {item: any}) => (
    <View style={styles.tableGroup}>
      ...
      <FlatList
        ...
        keyExtractor={(item, index) => `${item.idBan}-${item.tenNuoc}-${index}`}
        scrollEnabled={false}
      />
    </View>
  );
  ...
  return (
    <View style={{flex: 1}}>
      ...
      {/* <WrapHeader title="Theo Nhóm Món Nước"></WrapHeader> */}
      <FlatList
        data={data}
