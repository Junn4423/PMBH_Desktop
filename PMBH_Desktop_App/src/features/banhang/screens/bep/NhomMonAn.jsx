...
import {layDsMonAn, layDsMonNuoc, updateTrangThai} from '../../api';
import {
  NavigationProp,
  useFocusEffect,
  useNavigation,
...
const NhomMonAn: React.FC = () => {
  const nav = useNavigation<NavigationProp<RootStackParamList>>();
  const [data, setData] = useState<any>();
  const {mutate, isPending} = useMutation({
    mutationFn: layDsMonAn,
    onSuccess: data => {
  ...
  const renderTableGroup = ({item}: {item: any}) => (
    <View style={styles.tableGroup}>
      ...
      <FlatList
        ...
        keyExtractor={(item, index) => `${item.idBan}-${item.tenMon}-${index}`}
        scrollEnabled={false}
      />
    </View>
  );
  ...
  return (
    <View style={{flex: 1}}>
      <WrapHeader title={'Món Ăn'} handleBack={() => nav.goBack()}></WrapHeader>
      <FlatList
        data={data}
