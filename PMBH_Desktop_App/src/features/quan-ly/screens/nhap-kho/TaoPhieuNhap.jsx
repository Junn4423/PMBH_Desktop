...
import {loadKho, ThemPhieuNhapChiTietPn} from '../../api/index';
import {ChonSanPham, RenderKhoItem} from '../../components/nhap-kho/index';
import {Controller, useForm} from 'react-hook-form';
import {chiTietPhieuNhap, kho, nguyenLieuKho, phieuNhap} from '../../types';
...
function TaoPhieuNhap() {
  ...
  const [ctPn, setCtPn] = useState<chiTietPhieuNhap[]>([]);
  ...
  const xoaSanPham = (index: number): void => {
    setCtPn(prev => prev.filter((_, i) => i !== index));
  };
  const handleCtPn = (data: chiTietPhieuNhap) => {
    if (!data) {
      return;
    }
  ...
  ///
  const luuPhieuNhap = async (data: phieuNhap) => {
    try {
      const tongTien =
        ctPn?.reduce((sum, sp) => {
          return sum + (sp.thanhTien ?? 0);
        }, 0) ?? 0;
      if (user) {
        const res = await ThemPhieuNhapChiTietPn(
          khoSelected?.maKho ?? '',
          data.maNguoiDung || '',
          data.ghiChu || '',
          data.loaiPhieu || '',
    ...
    <View style={{flex: 1}}>
      ...
      <ScrollView style={styles.containerForm}>
        <View style={styles.boderForm}>
          <Text style={styles.headerForm}>Thông tin phiếu nhập mới</Text>
          <View style={styles.groupInput}>
            <Text style={styles.labelInput}>Kho nhập:</Text>
            <TouchableOpacity
          ...
          <Controller
            control={control}
            name="maNguoiDung"
            rules={{required: 'Mã người dùng không được để trống'}}
          ...
          <Controller
            ...
            render={({field: {value}}) => (
              <View style={styles.groupInput}>
                <Text style={styles.labelInput}>Loại phiếu:</Text>
                <TextInput
                  style={styles.textInput}
                  placeholder="Nhập loại phiếu"
                  value={value}
                  editable={false}
                />
              </View>
          ...
          <Controller
            ...
            render={({field: {value}}) => (
              <View style={styles.groupInput}>
                <Text style={styles.labelInput}>Ngày nhập kho:</Text>
          ...
          <TouchableOpacity
            ...
            onPress={handleSubmit(data => luuPhieuNhap(data as any))}>
